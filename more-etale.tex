...
In Remark \ref{remark-covariance-weighting} if $g$ is flat and we
use the weighting described in
...
Remark \ref{remark-covariance-shriek}. If $f : X \to Y$ is a
flat and quasi-finite morphism of separated schemes of finite type over $k$,
then there is a ``flat'' covariance map
$$
...
\section{The chern class of an effective Cartier divisor}
...
$$
See Section \ref{section-chern-class-invertible}.
...
\noindent
Let $f : X \to Y$ be a separated morphism of finite type with $Y$
...
the two cohomology groups in the displayed equations agree.
...
\noindent
Let $X$ be a separated finite type scheme over a field $k$
with $\dim(X) \leq 1$. We are going to construct a trace map
$$
\text{Tr}_X : H^2_c(X, \mu_n) \longrightarrow \mathbf{Z}/n\mathbf{Z}
$$
for all $n$ prime to the characteristic of $k$.


and $D \subset X$ is an effective Cartier divisor
proper over $k$. Then, if $n$ is invertible in $k$, we have a
first chern class
$$
c_1(D) \in H^2_c(X, \mu_n)
$$
These classes will play an important role in the following.

\begin{lemma}
\label{lemma-generation-by-c1}
Let $k$ be an algebraically closed field. Let $X$ be a separated finite
type scheme over $k$ of dimension $\leq 1$. Then $H^2_c(X, \mu_n)$
is generated by Chern classes of effective Cartier divisors $D \subset X$.
\end{lemma}
