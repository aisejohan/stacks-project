...
and compatible with composition of flat and quasi-finite
...
\begin{proof}
By Lemma \ref{lemma-top-algebraic-scheme}
...
$$
\mathbf{A}^d_Y \xrightarrow{projection} \mathbf{A}^{d - 1}_Y \to Y
$$
Then the spectral sequence of Lemma \ref{lemma-two-ss-complex-functor}
and the vanishing of Lemma \ref{}
...
Let $d \geq 0$.
Let $f : X \to Y$ be a flat, separated morphism of finite presentation
of relative dimension $\leq d$. We will construct a map
$$
\text{Tr}_{f, \mathit{flat}, n} :
R^2f_!\mu_n^{\otimes d}
\longrightarrow
\mathbf{Z}/n\mathbf{Z}
$$
The approach to the construction is to choose a quasi-finite map
$g : X \to \mathbf{A}^d_Y$ and then to use an explicity produced
map for $\mathbf{A}^d_Y$ composed with ``flat'' covariance along $g$.

\begin{lemma}
\label{lemma-covering-right-exact}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $f : X \to Y$ be separated and of finite presentation
with fibres of dimension $\leq d$. Let $U_1, \ldots, U_n \subset X$
be quasi-compact open subschemes such that
$X \setminus (U_1 \cup \ldots \cup U_n)$
...
by the covariance maps of Remark \ref{}.
\end{lemma}

\begin{proof}
By Lemma \ref{} we may replace $X$ by $\bigcup U_i$ and hence
we may assume $X = U_1 \cup \ldots \cup U_n$.
...
0
$$
on $X$ and apply the right exact functor $R^{2d}f_!$ to get
...
Let $(f_1, \ldots, f_n) : X \to \mathbf{A}^d_Y$ and
$(g_1, \ldots, g_n) : X \to \mathbf{A}^d_Y$
be quasi-finite. Let $U_i \subset X$ be the quasi-compact
open subset where $(g_i, f_2, \ldots, f_n) : X \to \mathbf{A}^d_Y$
is quasi-finite. Then $X = U_1 \cup \ldots \cup U_n$.
\end{lemma}
...
(a) why is there a quasi-finite morphism $g : X \to \mathbf{P}^d_Y$?
(b) if such an $g$ exists, why is it flat?
(c) why is the resulting trace map independent of $g$?
The answer to (a) is negative, but we won't use the existence
of global maps in the construction, only local ones.
The answer to (b) is that it requires $X \to Y$ to be Cohen-Macaulay,
see More on Morphisms, Lemma
\ref{more-morphisms-lemma-flat-finite-presentation-characterize-CM}.
The answer to (c) is that this holds because we may check on
stalks where we reduce to the construction of
Section \ref{section-trace-curves}.
