...
the counit $\text{Tr}_j : j_!j^{-1}K \to K$;
...
Rf_!K
$$
Given a second \'etale morphism $j' : X'' \to X'$
for some $f'' : X'' \to Y$ separated and of finite type
the composition
$$
...
This follows from the corresponding statement on trace maps, namely
...
schemes of finite type over $k$. If $f$ is \'etale, then there
is a natural covariance map
$$
R\Gamma_c(X, f^{-1}K) \to R\Gamma_c(Y, K)
$$
Namely, we get this by applying $R\Gamma(\Spec(k), -)$ to the map
in Remark \ref{remark-covariance-shriek}. More generally, suppose
$X$, $Y$, and $f$ satisfy one of the following conditions
\begin{enumerate}
\item $f$ is flat and quasi-finite, or
\item $f$ is quasi-finite and $Y$ is geometrically unibranch, or
\item $f$ is quasi-finite and we are given a weighting
$w : X \to \mathbf{Z}$ of $f$.
\end{enumerate}
In case (3) we obtain a map
$$
R\Gamma_c(X, f^{-1}K) \to R\Gamma_c(Y, K)
$$
depending on the choice of weighting $w$ in exactly the same manner
as above using Remark \ref{remark-covariance-weighting}.
In cases (1) and (2) we can use the weightings discussed
in Examples \ref{example-trace-for-flat-quasi-finite} and
\ref{example-trace-for-quasi-finite-over-normal}.
For example, if $\Lambda$ is a torsion ring, then we obtain an arrow
$$
\text{Tr}_f : R\Gamma_c(X, \Lambda) \longrightarrow R\Gamma_c(Y, \Lambda)
$$
canonical if $f$ is \'etale and depending on the choice of $w$ in general.
...
\begin{enumerate}
\item $\text{Tr}_{f, \alpha, K}$ is additive in $\alpha$,
\item $\text{Tr}_{f, \alpha, K}$ is functorial in $K$,
\item if $\Lambda'$ is a ring annihlated by an integer invertible on $Y$
and $\Lambda' \to \Lambda$ is a ring homomorphism, then the
diagram
$$
\xymatrix{
...
$$
commutes,
\item $\text{Tr}_{f, \alpha, K}$ is equal to
$\text{Tr}_{f, \alpha, \Lambda} \otimes \text{id}_K$ via
the projection formula, Lemma \ref{lemma-projection-formula-Rf-lower-shriek},
\item $\text{Tr}_{f, \alpha, K}$ is compatible with base change
by morphisms $g : Y' \to Y$ where $Y'$ is quasi-compact and quasi-separated,
...
\item if $f$ is \'etale
and $\alpha = [X/X/Y]_0$ then $\text{Tr}_{f, \alpha, K}$
is equal to the counit $f_!f^{-1}K \to K$,
...
$$
\item Let $j : X' \to X$ be an \'etale morphism
of separated schemes of finite type over $Y$ with
structure morphisms $f : X \to Y$ and $f' : X' \to Y$.
...
\item Let $h : X' \to X$ be a proper morphism
of separated schemes of finite type over $Y$ with
structure morphisms $f : X \to Y$ and $f' : X' \to Y$.
Let $\alpha \in z(X'/Y, r)$ and denote $\alpha = h_*\alpha'$.
Then the diagram
$$
\xymatrix{
Rf_!f^{-1}K(r)[2r]
...
\medskip\noindent
Let $Z \subset X$ be a closed subscheme and let $U = X \setminus Z$
be the complementary open subscheme. Let $D \subset X$ be an effective
Cartier divisor which is entirely contained in $U$; equivalently,
$D \subset U$ is an effective Cartier divisor whose image in $X$ is closed.
In this case the invertible module $\mathcal{O}_X(D)$ comes with a
...
Noting that the pullback of $\mathcal{O}_{\overline{X}_2}(D)$ by $g$
is $\mathcal{O}_{\overline{X}_1}(D)$, 
the statement follows from Lemma \ref{lemma-pullback-c1}.
\end{proof}

\noindent
In particular, suppose that $X$ is a separated finite type scheme
over a field $k$ and $D \subset X$ is an effective Cartier divisor
...
\section{Construction of the trace map in the equidimensional case}
