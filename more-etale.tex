...
$$
and the formation of this weighting commutes with base change, see
...
with arbitrary base change. Moreover, combining
More on Morphisms, Lemma
\ref{more-morphisms-lemma-weighting-composition-flat-quasi-finite}
with Lemma \ref{lemma-trace-composition} we see that
we have the compatibility with composition:
$$
\text{Tr}_{g, \mathit{flat}, K} \circ
g_!\text{Tr}_{f, \mathit{flat}, g^{-1}K} =
\text{Tr}_{g \circ f, \mathit{flat}, K}
$$
when $g : Y \to Z$ is another morphism of schemes which is flat,
locally quasi-finite, and locally of finite presentation.
\end{example}
...
map $\text{Tr}_{j, \mathit{flat}, K} : j_!j^{-1}K \to K$ of
Example \ref{example-trace-for-flat-quasi-finite} is equal to the
counit for the adjunction between $j_!$ and $j^{-1}$.
This follows as the weighting for $j$ is the constant function $1$
and thus $\text{Tr}_{j, \mathit{flat}, K}$ agrees with adjunction
by Modules on Sites, Lemma
\ref{sites-modules-lemma-adjunction-on-stalk-j-shriek-j-inverse}.
...
\begin{proof}
If $f$ and $g$ are separated, then we can apply
Lemma \ref{lemma-shriek-proper-and-open} to the square
$$
\xymatrix{
X \ar[r]_f \ar[d]_{\text{id}} & Y \ar[d]^h \\
X \ar[r]^g & Z
}
$$
to obtain this map. We urge the reader to skip the proof in the general case
as we'll mainly use the case where $f$ and $g$ are separated. Let us
point out here that the map constructed below does agree with the map
from Lemma \ref{lemma-shriek-proper-and-open} in case both lemmas apply.
...
\begin{remark}
\label{remark-map-is-pullback}
In Lemma \ref{lemma-shriek-lqf-and-proper} assume that $f$ and $g$
are open immersions and that $h^{-1}(g(X)) = f(X)$ as opens of $Y$.
Then by
\'Etale Cohomology, Lemma \ref{etale-cohomology-lemma-shriek-base-change}
we have a canonical isomorphism of functors $f_! = h^{-1}g_!$
between categories of sheaves of modules. Thus we have a second map
$$
g_!K \longrightarrow Rh_* h^{-1} g_!K = Rh_*f_!K
$$
using the unit of the adjunction in the arrow. Looking at the proof of
Lemma \ref{lemma-shriek-lqf-and-proper} to see the equality it suffices
to prove that the diagram
$$
\xymatrix{
g_!\mathcal{K}^\bullet \ar@{=}[r] \ar[rd] &
h_!f_!\mathcal{K}^\bullet \ar@{=}[r] &
h_*f_!\mathcal{K}^\bullet \\
& h_*h^{-1}g_!\mathcal{K}^\bullet \ar@{=}[ru]
}
$$
commutes. This can be checked on individual sheaves and then
one can check on stalks by unwinding the definitions. We omit
the details. As a consequence we see that for any $i \in \mathbf{Z}$
the map
$$
H^i(Z, g_!K) \to H^i(Z, Rh_*f_!K) = H^i(Y, f_!K)
$$
agrees with pullback by the morphism $h$ via the isomorphism
$f_!K = h^{-1}g_!K$.
\end{remark}
...
Let $f : X \to Y$ be a finite type separated morphism of schemes
with $Y$ quasi-compact and quasi-separated. There is a map
$$
...
\end{enumerate}
To construct our map we choose a compactification $j : X \to \overline{X}$
over $Y$ and denote $\overline{f} : \overline{X} \to Y$ the structure morphism.
...
\begin{lemma}
\label{lemma-proper-Rf-lower-shriek}
Let $f : X \to Y$ be a proper morphism of schemes with $Y$ quasi-compact
and quasi-separated. The map $Rf_!K \to Rf_*K$ of
Remark \ref{remark-shriek-to-star} is an isomorphism.
\end{lemma}

\begin{proof}
If one uses $\overline{X} = X$ to define $Rf_!$, then the result is clear.
If $j : X \to \overline{X}$ is an arbitrary compactification, then
$j$ is both open and closed. Then the transformation of functors
$j_! \to Rj_*$ used in the remark is an isomorphism.
\end{proof}

\begin{lemma}
\label{lemma-Rf-shriek-for-quasi-finite}
Let $f : X \to Y$ be a quasi-finite separated morphism of quasi-compact and
quasi-separated schemes. The functor $Rf_!$ agrees with the functor
$f_! : D(X_\etale, \Lambda) \to D(Y_\etale, \Lambda)$ constructed in
Lemma \ref{lemma-lqf-shriek-derived}.
\end{lemma}
...
\label{remark-covariance-shriek}
Consider a commutative diagram
$$
\xymatrix{
X' \ar[rr]_j \ar[rd]_{f'} & & X \ar[ld]^f \\
& Y
}
$$
of quasi-compact and quasi-separated schemes with $f$ and $f'$
separated and of finite type and with $j$ \'etale.
Let $\Lambda$ be a ring. For $K$ in $D^+_{tors}(X, \Lambda)$
or $K$ in $D(X, \Lambda)$ if $\Lambda$ is a torsion ring, there
is a canonical map
$$
Rf'_!(K|_{X'}) \longrightarrow Rf_!K
$$
in $D(Y_\etale, \Lambda)$. Namely, by Lemmas \ref{lemma-shriek-composition}
and \ref{lemma-Rf-shriek-for-quasi-finite} we have $Rf'_! = Rf_! \circ j_!$.
Since $j_!$ is left adjoint to $j^{-1}$ we have
the counit $\text{Tr}_j : j_!j^{-1}K \to K$ of the adjunction;
we recall that this map agrees with the ``flat'' trace map map for $j$, see
Remark \ref{remark-trace-etale-counit}.
Observing that the restriction $K|_{X'}$ is equal to $j^{-1}K$
the map above is constructed as the composition
$$
Rf'_!(K|_{X'}) = Rf'_!j^{-1}K = Rf_!j_!j^{-1}K
\xrightarrow{Rf_! \text{Tr}_j}
Rf_!K
$$
The covariance map $Rf'_!(K|_{X'}) \longrightarrow Rf_!K$ is compatible
with base change by morphisms $Y' \to Y$ of quasi-compact and quasi-separated
schemes. Furthermore, given a second \'etale morphism $j' : X'' \to X'$
for some $f'' : X'' \to Y$ separated and of finite type the composition
$$
Rf''_!(K|_{X''}) \longrightarrow Rf'_!(K|_{X'}) \longrightarrow Rf_!K
$$
of the maps for $j$ and $j'$ is equal to the map
$Rf''_!K|_{X''} \longrightarrow Rf_!K$ constructed for $j \circ j'$.
This follows from the corresponding statement on counits, namely
``$\text{Tr}_j \circ \text{Tr}_{j'} = \text{Tr}_{j \circ j'}$'',
see Categories, Lemma \ref{categories-lemma-compose-counits}.
\end{remark}

\begin{remark}
\label{remark-covariance-weighting}
We can extend the covariance of Remark \ref{remark-contravariance-shriek}
to the case of a locally quasi-finite morphism endowed with a weigthing.
Namely, consider a commutative diagram
$$
\xymatrix{
X' \ar[rr]_g \ar[rd]_{f'} & & X \ar[ld]^f \\
& Y
}
$$
of quasi-compact and quasi-separated schemes with $f$ and $f'$
separated and of finite type and with $g$ locally quasi-finite.
Assume given a weighting $w : X' \to \mathbf{Z}$ of $g$
(More on Morphisms, Definition \ref{more-morphisms-definition-weighting}).
Let $\Lambda$ be a ring. For $K$ in $D^+_{tors}(X, \Lambda)$
or $K$ in $D(X, \Lambda)$ if $\Lambda$ is a torsion ring, there
is a canonical map
$$
Rf'_!(g^{-1}K) \longrightarrow Rf_!K
$$
in $D(Y_\etale, \Lambda)$. Namely, by Lemmas \ref{lemma-shriek-composition}
and \ref{lemma-Rf-shriek-for-quasi-finite} we have $Rf'_! = Rf_! \circ g_!$.
By the discussion in Section \ref{section-weightings} the weighting
$w$ induces a trace map $\text{Tr}_{g, w, K} : g_!g^{-1}K \to K$.
The displayed arrow above is constructed as the composition
$$
Rf'_!(g^{-1}K) = Rf_!g_!g^{-1}K
\xrightarrow{Rf_! \text{Tr}_{g, w, K}}
Rf_!K
$$
Given a second locally quasi-finite morphism $g' : X'' \to X'$
equipped with a weighting $w'$ for some $f'' : X'' \to Y$ separated
and of finite type the composition
$$
Rf''_!((g')^{-1}g^{-1}K) \longrightarrow Rf'_!(g^{-1}K) \longrightarrow Rf_!K
$$
of the maps for $(g, w)$ and $(g', w')$ is equal to the map
$Rf''_!((g \circ g')^{-1}K) \longrightarrow Rf_!K$
constructed for $(g \circ g', w'')$ where $w''(x'') = w'(x'')w(g'(x''))$.
This follows from the corresponding statement
on trace maps, see Lemma \ref{lemma-trace-composition}.
\end{remark}

\begin{remark}[``Flat'' covariance]
\label{remark-flat-covariance}
In Remark \ref{remark-covariance-weighting} if $g$ is flat
(as well as separated, quasi-finite, and of finite presentation)
and we use the weighting described in
Example \ref{example-trace-for-flat-quasi-finite}, i.e., we use
$Rf_! \text{Tr}_{g, \mathit{flat}, K}$, then we call
$Rf'_!(g^{-1}K) \longrightarrow Rf_!K$ the ``flat'' covariance map.
The construction of this map is compatible with base change by
morphisms $Y' \to Y$ of quasi-compact and quasi-separated schemes
and compatible with composition of flat, quasi-finite
morphisms between schemes separated and of finite presentation over $Y$.
\end{remark}

\begin{remark}[Contravariance]
\label{remark-contravariance-shriek}
Consider a commutative diagram
$$
\xymatrix{
X' \ar[rr]_h \ar[rd]_{f'} & & X \ar[ld]^f \\
& Y
}
$$
of quasi-compact and quasi-separated schemes with $f$ and $f'$
separated and of finite type and with $h$ proper.
Let $\Lambda$ be a ring. For $K$ in $D^+_{tors}(X, \Lambda)$
or $K$ in $D(X, \Lambda)$ if $\Lambda$ is a torsion ring, there
is a canonical map
$$
Rf_!K \longrightarrow Rf'_!h^{-1}K
$$
in $D(Y_\etale, \Lambda)$. Namely, by Lemmas \ref{lemma-shriek-composition}
and \ref{lemma-proper-Rf-lower-shriek} we have $Rf'_! = Rf_! \circ Rh_*$.
Since $Rh_*$ is right adjoint to $h^{-1}$ we have the unit
$\eta : K \to Rh_*h^{-1}K$. The map above is constructed as the composition
$$
Rf_!K
\xrightarrow{Rf_! \eta}
Rf_!Rh_*h^{-1}K = Rf'_!h^{-1}K
$$
Given a second proper morphism $h' : X'' \to X'$
the composition
$$
Rf_!K \longrightarrow Rf'_!h^{-1}K
\longrightarrow Rf''_!(h')^{-1}h^{-1}K = Rf''_!((h \circ h')^{-1}K)
$$
of the maps for $h$ and $h'$ is equal to the map
constructed for $h \circ h'$. Details omitted; hint: use that the
composition
$$
K \xrightarrow{\eta} Rh_*h^{-1}K
\xrightarrow{Rh_*\eta'} Rh_*Rh'_*(h')^{-1}h^{-1}K
$$
is the unit for the pair $R(h \circ h')_*$ and $(h \circ h')^{-1}$
dually to
Categories, Lemma \ref{categories-lemma-compose-counits}.
\end{remark}

\begin{remark}
\label{remark-covariance-and-contravariance-commute}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Consider a cartesian diagram
$$
\xymatrix{
X_1 \ar[r]_{h'} \ar[d]_{g'} & X_2 \ar[d]^g \\
X_3 \ar[r]^h & X_4
}
$$
of schemes over $Y$ with $g$ flat and quasi-finite and
$h$ proper. Assume the structure morphisms $f_i : X_i \to Y$
are separated and of finite presentation. Let $K_4$ in
$D^+_{tors}(X_4, \Lambda)$
or $K_4$ in $D(X_4, \Lambda)$ if $\Lambda$ is a torsion ring.
For $i = 1, 2, 3$ denote $K_i$ the pullback of $K_4$ to $X_i$.
Then the diagram
$$
\xymatrix{
Rf_{1, !}K_1 \ar[d]_{\text{flat covariance}} & &
Rf_{2, !}K_2 \ar[ll]^{\text{contravariance}} \ar[d]^{\text{flat covariance}} \\
Rf_{3, !}K_3 & &
Rf_{4, !}K_4 \ar[ll]_{\text{contravariance}}
}
$$
commutes. Here the arrows are as defined in
Remarks \ref{remark-flat-covariance} and \ref{remark-contravariance-shriek}.
We omit the detailed proof of this.
\end{remark}
...
Let $f : X \to Y$ be a finite type separated morphism of quasi-compact
and quasi-separated schemes. Let $\Lambda$ be a ring. For
$K \in D^+_{tors}(X_\etale, \Lambda)$ there is a spectral sequence
$$
E^{p, q}_2 = R^pf_!H^q(K) \Rightarrow R^{p + q}f_!K
$$
This spectral sequence is bounded, converges, and
induces finite filtrations on $R^nf_!K$.
\end{lemma}

\begin{proof}
Since $Rf_!$ isn't the derived functor of a left exact functor, we need
to say something. Choose a compactification $j : X \to \overline{X}$
of $X$ over $Y$. Since $j_!$ is exact, we see that
$H^q(j_!K) = j_!H^q(K)$. Thus, since $Rf_! = R\overline{f}_* \circ j_!$
we can use the second spectral sequence of
...
Let $f : X \to Y$ be a finite type separated morphism of quasi-compact and
quasi-separated schemes. Let $Z$ be a closed subscheme of $X$ whose
complement $U$ is quasi-compact. Denote $g : U \to Y$ and $h : Z \to Y$
...
Definition \ref{definition-compact-support} in
...
\begin{lemma}
\label{lemma-bounded-algebraic-scheme}
Let $k$ be an algebraically closed field. Let $X$ be a separated
scheme of finite type type over $k$ of dimension $\leq d$.
Let $\mathcal{F}$ be an abelian torsion sheaf on $X$.
Then $H^q_c(X, \mathcal{F}) = 0$ for $q > 2d$.
\end{lemma}

\begin{proof}
This is a consequence of \'Etale Cohomology, Lemma
\ref{etale-cohomology-lemma-cohomological-dimension-proper}.
Namely, choose a compactification $j : X \to \overline{X}$.
After replacing $\overline{X}$ by the scheme theoretic closure
of $X$, we see that we may assume $\dim(\overline{X}) \leq d$.
Then $H^q_c(X, \mathcal{F}) = H^q(\overline{X}, j_!\mathcal{F})$
and the lemma applies.
\end{proof}

\begin{lemma}
\label{lemma-bounded-f-shriek}
Let $f : X \to Y$ be a finite type separated morphism of schemes
with $Y$ quasi-compact and quasi-separated. Assume all fibres
of $f$ have dimension $\leq d$. Let $\mathcal{F}$ be an abelian
torsion sheaf on $X$. Then $R^qf_!\mathcal{F} = 0$ for $q > 2d$.
\end{lemma}

\begin{proof}
Combine Lemmas \ref{lemma-stalk-R-f-shriek} and
\ref{lemma-bounded-algebraic-scheme}.
\end{proof}
...
$D^+_{tors}(X_\etale, \Lambda)$
or of $D(X_\etale, \Lambda)$ in case $\Lambda$ is torsion.
Denote $K'$ the pullback of $K$ to $X'$.
Then the base change map $R\Gamma_c(X, K) \to R\Gamma_c(X'. K')$
is an isomorphism.
\end{lemma}

\begin{proof}
Choosing a compactification $j : X \to \overline{X}$, this
immediately translates into a question on cohomology over $\overline{X}$
and this question is answered positively by
\'Etale Cohomology, Lemma
\ref{etale-cohomology-lemma-base-change-separably-closed}.
\end{proof}

\begin{lemma}
\label{lemma-triangle-compactly-supported}
Let $X$ be a separated finite type scheme over a field $k$.
Let $Z$ be a closed subscheme of $X$ with complement $U$.
There exists a long exact sequence of compactly supported
cohomology
$$
H^i_c(U, K|_U) \to H^i_c(X, K) \to H^i_c(Z, K|_Z) \to H^{i + 1}_c(U, K|_U)
$$
for $K$ in $D^+_{tors}(X_\etale, \Lambda)$ or $K \in D(X_\etale, \Lambda)$
if $\Lambda$ is torsion.
\end{lemma}

\begin{proof}
This follows from Lemma \ref{lemma-relative-triangle-associated-to-open}
and the definitions.
\end{proof}

\begin{remark}[Covariance of compactly supported cohomology]
\label{remark-covariance-compactly-supported}
Let $k$ be a field. Let $j : X \to Y$ be an \'etale morphism of separated
schemes of finite type over $k$. Then there
is a natural covariance map
$$
\text{Tr}_j : R\Gamma_c(X, K|_X) \to R\Gamma_c(Y, K)
$$
which is compatible with extension of ground field.
Namely, we get this by applying $R\Gamma(\Spec(k), -)$ to the map in
Remark \ref{remark-covariance-shriek}. More generally, if $f : X \to Y$
is a flat and quasi-finite morphism of separated schemes of finite type
over $k$, then there is a ``flat'' covariance map
$$
R\Gamma_c(X, f^{-1}K) \to R\Gamma_c(Y, K)
$$
obtained by applying $R\Gamma(\Spec(k), -)$ to the map in
Remark \ref{remark-flat-covariance}. This map is also compatible
with extending ground fields. For example, we obtain a
``flat'' covariance arrow
$$
R\Gamma_c(X, \Lambda) \longrightarrow R\Gamma_c(Y, \Lambda)
$$
which agrees with the natural covariance map $\text{Tr}_f$ when $f$ is \'etale.
\end{remark}

\begin{remark}[Contravariance of compactly supported cohomology]
\label{remark-contravariance-compactly-supported}
Let $k$ be a field. Let $h : X \to Y$ be a proper morphism of separated
schemes of finite type over $k$. Then there is a natural pullback map
$$
R\Gamma_c(Y, K) \longrightarrow R\Gamma_c(X, h^{-1}K)
$$
which is compatible with extension of ground field.
Namely, we get this by applying $R\Gamma(\Spec(k), -)$ to the map in
Remark \ref{remark-contravariance-shriek}.
\end{remark}

\begin{lemma}
\label{lemma-top-algebraic-scheme}
Let $k$ be an algebraically closed field. Let $X$ be a separated
scheme of finite type type over $k$ of dimension $\leq d$. Let $U \subset X$
be an open subscheme which meets every irreducible component of $X$
having dimension $d$. Let $\mathcal{F}$ be an abelian torsion sheaf on $X$.
The map $H^{2d}_c(U, \mathcal{F}|_U) \to H^{2d}_c(X, \mathcal{F})$
is an isomorphism.
\end{lemma}

\begin{proof}
This follows from Lemma \ref{lemma-triangle-compactly-supported}
and the vanishing in Lemma \ref{lemma-bounded-algebraic-scheme}.
\end{proof}

\begin{lemma}
\label{lemma-top-restrict-to-open}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $f : X \to Y$ be a separated morphism of finite presentation
of relative dimension $\leq d$. Let $U \subset X$ be an open
such that $X \setminus U$ has fibres of dimension $< d$ over $Y$.
Then the map $R^{2d}(f|_U)_!\mathcal{F}|_U \to R^{2d}f_!\mathcal{F}$
is an isomorphism for any torsion abelian sheaf $\mathcal{F}$ on $X$.
\end{lemma}

\begin{proof}
Looking at stalks using Lemma \ref{lemma-stalk-R-f-shriek}
we see this follows from
Lemma \ref{lemma-top-algebraic-scheme}.
\end{proof}

\begin{lemma}
\label{lemma-pullback-top-algebraic-scheme}
Let $k$ be an algebraically closed field. Let $f : X \to Y$ be a
proper morphism of separated schemes of finite type type over $k$ both
of dimension $\leq d$. Assume $f$ induces an isomorphism
$U_{red} \to V_{red}$ for open subschemes $U \subset X$ and $V \subset Y$
which meet every irreducible component of $X$ and $Y$ having dimension $d$.
Let $\mathcal{F}$ be an abelian torsion sheaf on $X$.
Then the map $H^{2d}_c(Y, f^{-1}\mathcal{F}) \to H^{2d}_c(X, \mathcal{F})$
of Remark \ref{remark-contravariance-compactly-supported}
is an isomorphism.
\end{lemma}

\begin{proof}
Observe that $X \setminus U$ is a closed subset of $X$ of dimension $< d$.
Hence we may replace $V$ by $V \setminus f(X \setminus U)$ and we may assume
$U = f^{-1}(U)$. In this case, by Lemma \ref{lemma-top-algebraic-scheme}
and the compatibility between the maps of
Remarks \ref{remark-covariance-compactly-supported} and
\ref{remark-contravariance-compactly-supported} given in
Remark \ref{remark-covariance-and-contravariance-commute},
we may replace $X$ by $U$ and $Y$ by $V$.
Then the result holds by the insensitivity to
thickenings given in Lemma \ref{lemma-shriek-and-thickening}.
\end{proof}
...
(Lemma \ref{lemma-derived-lower-shriek-bounded})
we may assume $K$ is bounded below. Using the spectral sequence of
...
separated and of finite type and with $j$ \'etale. As discussed in
Remark \ref{remark-covariance-shriek} the diagram gives rise to a canonical map
$$
Rg_!\Lambda \longrightarrow Rg'_!\Lambda
$$
in $D(Y_\etale, \Lambda)$. Given $g'' : U'' \to Y$ separated of finite type
and a second \'etale morphism $j' : U' \to U''$ the composition
$$
...
\section{Tate twists}
\label{section-tate-twists}

\noindent
Let $S$ be a scheme. Let $n$ be an integer invertible on $S$.
We will use notation as in
\'Etale Cohomology, Section \ref{etale-cohomology-section-kummer}.
The sheaf $\mu_{n, S}$ on $S_\etale$ is an invertible sheaf
of $\mathbf{Z}/n\mathbf{Z}$-modules. Thus for any integer $i \in \mathbf{Z}$
we can consider the $i$th tensor power $\mu_{n, S}^{\otimes i}$
and we have
$\mu_{n, S}^{\otimes i} \otimes_{\mathbf{Z}/n\mathbf{Z}}
\mu_{n, S}^{\otimes j} = \mu_{n, S}^{\otimes i + j}$.

\medskip\noindent
For a sheaf $\mathcal{F}$ of $\mathbf{Z}/n\mathbf{Z}$-modules on $S_\etale$
we set
$$
\mathcal{F}(i) = \mathcal{F} \otimes_{\mathbf{Z}/n\mathbf{Z}}
\mu_{n, S}^{\otimes i}
$$
Then we have $\mathcal{F}(i)(j) = \mathcal{F}(i + j)$. If $K$ is an
object of $D(X_\etale, \mathbf{Z}/n\mathbf{Z})$, then we can similarly
define $K(i)$. We will also use this notation for sheaves of
$\mathbf{Z}/n\mathbf{Z}$-modules or complexes of sheaves of
$\mathbf{Z}/n\mathbf{Z}$-modules on the big \'etale site of $S$.

\medskip\noindent
Additionally, if $\Lambda$ is a ring annihilated by $n$ and
$\mathcal{F}$ is a sheaf of $\Lambda$-modules on $S_\etale$,
then we use the same construction to define $\mathcal{F}(i)$,
but we view it as a sheaf of $\Lambda$-modules. Similarly
for complexes (on $S_\etale$ or on the big \'etale site of $S$).
Given a torsion ring $\Lambda$ the construction of $\mathcal{F}(i)$
is independent of the choice $n$ which annihilates $\Lambda$
(provided $n$ is invertible on $S$).





\section{The trace map associated to a relative cycle}
\label{section-trace-relative-cycle}

\noindent
Let $f : X \to Y$ be a separated morphism of finite presentation
where $Y$ is quasi-compact and quasi-separated. Let $\Lambda$
be a ring annihlated by an integer invertible on $Y$.
Finally, let $r \geq 0$ be an integer and let $\alpha$ be a
relative $r$-cycle on $X/Y$.
The goal of this section is to construct maps
$$
\text{Tr}_{f, \alpha, K} : Rf_!f^{-1}K(r)[2r] \to K
$$
for $K$ in $D(X_\etale, \Lambda)$. These maps should satisfy the
following requirements
\begin{enumerate}
\item The map $\text{Tr}_{f, \alpha, K}$ is additive in $\alpha$.
\item The map $\text{Tr}_{f, \alpha, K}$ is functorial in $K$,
\item If $\Lambda'$ is a ring annihlated by an integer invertible on $Y$
and $\Lambda' \to \Lambda$ is a ring homomorphism, then the diagram
$$
\xymatrix{
Rf_!\Lambda'(r)[2r] \ar[rr]_-{\text{Tr}_{f, \alpha, \Lambda'}} \ar[d] & &
\Lambda' \ar[d] \\
Rf_!\Lambda(r)[2r] \ar[rr]^-{\text{Tr}_{f, \alpha, \Lambda}} & & \Lambda
}
$$
commutes.
\item The map $\text{Tr}_{f, \alpha, K}$ is equal to
$\text{Tr}_{f, \alpha, \Lambda} \otimes \text{id}_K$ via
the projection formula, Lemma \ref{lemma-projection-formula-Rf-lower-shriek}.
\item The formation of $\text{Tr}_{f, \alpha, K}$ is compatible
with base change by morphisms $g : Y' \to Y$ where $Y'$ is
quasi-compact and quasi-separated.
\item If $f$ is locally quasi-finite, $Y$ is Noetherian, and $r = 0$, then
$\text{Tr}_{f, \alpha, \mathcal{F}} =
\text{Tr}_{f, w_\alpha, \mathcal{F}}$, see
Relative Cycles, Lemma \ref{relative-cycles-lemma-weightings}
and Section \ref{section-weightings}.
\item If $f$ is \'etale and $\alpha = [X/X/Y]_0$ then
$\text{Tr}_{f, \alpha, K}$ is equal to the counit $f_!f^{-1}K \to K$.
\item If $f$ is flat and locally quasi-finite and $\alpha = [X/X/Y]_0$ then
$\text{Tr}_{f, \alpha, K} = \text{Tr}_{f, \mathit{flat}, K}$.
\item Given a morphism $f' : X' \to X$ of finite presentation
and $\alpha'$ a relative $r'$-cycle on $X'/X$, then
$$
\text{Tr}_{f \circ f', \alpha \circ \alpha', K} :
R((f \circ f')_!(f \circ f')^{-1}K(r + r')[2r + 2r'] \to K
$$
is equal to the composition
$$
\text{Tr}_{f, \alpha, K} \circ
Rf_!(\text{Tr}_{f', \alpha', f^{-1}K})(r)[2r]
$$
via the identification
$$
R((f \circ f')_!(f \circ f')^{-1}K(r + r')[2r + 2r'] \\
= 
Rf_!\Big(Rf'_!(f')^{-1} f^{-1}K(r')[2r']\Big)(r)[2r]
$$
\item if $j : X' \to X$ is a separated \'etale morphism and
$f' = f \circ j : X' \to Y$.
Let $\alpha \in z(X/Y, r)$ and denote $\alpha' = j^*\alpha$.
Then the diagram
$$
\xymatrix{
Rf_!f^{-1}K(r)[2r]
\ar[rr]_-{\text{Tr}_{f, \alpha, K}} & &
K \ar@{=}[d] \\
Rf'_!(f')^{-1}K(r)[2r] \ar[rr]^-{\text{Tr}_{f', \alpha', K}} \ar[u] & &
K
}
$$
commutes where the vertical arrow is
Remark \ref{remark-covariance-shriek}.
\item Let $g : X' \to X$ be a separated, flat, quasi-finite morphism
of finite presentation and set $f' = f \circ g : X' \to Y$.
Let $\alpha \in z(X/Y, r)$ and denote $\alpha' = g^*\alpha$.
Then the diagram
$$
\xymatrix{
Rf_!f^{-1}K(r)[2r]
...
$$
commutes where the vertical arrow is the ``flat'' covariance
map, see Remark \ref{remark-flat-covariance}.
\item Let $h : X' \to X$ be a proper morphism of finite presentation
and set $f' = f \circ h : X' \to Y$. Let $\alpha \in z(X'/Y, r)$ and
denote $\alpha = h_*\alpha'$. Then the diagram
$$
\xymatrix{
Rf_!f^{-1}K(r)[2r]
\ar[rr]_-{\text{Tr}_{f, \alpha, K}} \ar[d] & &
K \ar@{=}[d] \\
Rf'_!(f')^{-1}K(r)[2r] \ar[rr]^-{\text{Tr}_{f', \alpha', K}} & &
K
}
$$
commutes where the vertical arrow is
...
\section{The chern class of an invertible module}
\label{section-chern-class-invertible}

\noindent
Let $X$ be a scheme. Let $n$ be an integer invertible on $X$.
The {\it first chern class} is the homomorphism
$$
c_1 : \Pic(X) \longrightarrow H^2(X, \mu_n)
$$
which comes from the identification $\Pic(X) = H^1(X, \mathbf{G}_m)$
(\'Etale Cohomology, Theorem \ref{etale-cohomology-theorem-picard-group}) 
and the boundary map in the Kummer exact sequence
(\'Etale Cohomology, Lemma \ref{etale-cohomology-lemma-kummer-sequence}).
Please see \'Etale Cohomology, Section \ref{etale-cohomology-section-kummer}
for more information. In this chapter we will use a slight generalization.

\medskip\noindent
Let $Z \subset X$ be a closed subscheme and let $U = X \setminus Z$
be the complementary open subscheme. Consider pairs
$(\mathcal{L}, \sigma)$ consisting of an invertible
$\mathcal{O}_X$-module together with a trivialization
$\sigma$ of the restriction of $\mathcal{L}$ to $Z$.
Given two such pairs $(\mathcal{L}, \sigma)$ and $(\mathcal{L}', \sigma')$
the tensor product $(\mathcal{L} \otimes \mathcal{L}', \sigma \otimes \sigma')$
is another pair. The set of these pairs up to isomorphism forms a group,
which we will denote $\Pic(X, Z)$, with group law given by tensor product.
We will define a first chern class
$$
c_1 : \Pic(X, Z) \longrightarrow H^2(X, j_!\mu_n)
$$
Here $j : U \to X$ is the inclusion morphism. To do this we generalize both
\'Etale Cohomology, Theorem \ref{etale-cohomology-theorem-picard-group} and
\'Etale Cohomology, Lemma \ref{etale-cohomology-lemma-kummer-sequence}.
Namely, let $\mathcal{I} \subset \mathcal{O}_{X_\etale}$
be the ideal sheaf of $Z$ on the small \'etale site of $X$. Below the
notation $(1 + \mathcal{I})^*$ refers to
$(1 + \mathcal{I}) \cap \mathcal{O}_{X_\etale}^*$.

\begin{lemma}
\label{lemma-picard-group-extended}
We have $\Pic(X, Z) = H^1(X, (1 + \mathcal{I})^*)$.
\end{lemma}

\begin{proof}
Given a pair $(\mathcal{L}, \sigma)$ the sheaf
$$
V \longmapsto \{s \in \mathcal{L}(V) \mid s|_{V \times_X Z} = \sigma|_{V \times_X Z}\}
$$
is a torsor for $(1 + \mathcal{I})^*$. Taking the class of this
torsor using Cohomology on Sites, Lemma \ref{sites-cohomology-lemma-torsors-h1}
we obtain an element of $H^1(X_\etale, (1 + \mathcal{I})^*)$.

\medskip\noindent
Conversely, suppose given an element
$\eta \in H^1(X_\etale, (1 + \mathcal{I})^*)$.
We claim that $\eta$ is Zariski locally trivial.
Namely, consider the short exact sequence
$$
0 \to (1 + \mathcal{I})^* \to \mathcal{O}_{X_\etale}^* \to
i_*\mathcal{O}_{Z_\etale}^* \to 0
$$
where $i : Z \to X$ is the inclusion morphism\footnote{This sequence
provides an alternative definition the sheaf $(1 + \mathcal{I})^*$.}.
By \'Etale Cohomology, Theorem \ref{etale-cohomology-theorem-picard-group}
elements in $H^1(X_\etale, \mathcal{O}_{X_\etale}^*)$ are Zariski
locally trivial. Since $i$ is a closed immersion a section
of $i_*\mathcal{O}_{Z_\etale}^*$ over an object $V$ of $X_\etale$
lifts Zariski locally on $V$ to a section of $\mathcal{O}_{X_\etale}^*$.
These two remarks imply the claim; details omitted. Thus we can find a Zariski
open covering $X = \bigcup V_j$ such that $\eta$ is given by
a $1$-cocycle $(u_{jj'})$ with
$u_{jj'} \in (1 + \mathcal{I})^*(V_j \cap V_{j'})$.
The cocycle $(u_{jj'})$ defines an invertible $\mathcal{O}_X$-module
(on the scheme $X$) which is trivialized after restricting to $Z$.

\medskip\noindent
We omit the proof that the constructions given above are mutually inverse.
\end{proof}

\begin{lemma}
\label{lemma-kummer-sequence-extended}
In the situation above we have a sort exact sequence
$$
0 \to j_!\mu_n \to (1 + \mathcal{I})^*
\xrightarrow{(\cdot)^n} (1 + \mathcal{I})^* \to 0
$$
on $X_\etale$.
\end{lemma}
...
$j_!\mu_n \subset \mu_n \subset \mathbf{G}_m = \mathcal{O}_{X_\etale}^*$ as
sheaves on $X_\etale$. The sequence is exact in the middle as
any local section in the kernel of the right arrow must restrict
to $1$ in an open neighbourhood of $Z$.
The interesting point is to see surjectivity of the right arrow.
We discuss the corresponding algebra problem below.

\medskip\noindent
Let $A$ be a ring with $n$ invertible in $A$. Let $I \subset A$ be an ideal.
Let $u = 1 + i \in A$ be a unit with $i \in I$.
Consider the \'etale extension
$$
B = A[x]/(-1 + nx + {n \choose 2}ix^2 + \ldots + i^{n - 1}x^n)
\left[\frac{1}{n + 2{n \choose 2}ix + \ldots + ni^{n - 1}x^{n - 1}}\right]
$$
In $B$ we have $1 + i = (1 + ix)^n$. Thus $1 + ix$ is a unit in $B$
whose $n$th power is $u$. Moreover, we have $B/IB = A/IA$.
Thus the \'etale morphism $\Spec(B) \to \Spec(A)$, while not necessarily
surjective, covers an open neighbourhood of $V(I)$. On the complement of $V(I)$
we have the surjectivity by the usual Kummer sequence in \'etale cohomology.
\end{proof}

\noindent
Exactly as in the case of chern classes of invertible modules we
define $c_1$ as the boundary map
$$
\Pic(X, Z) = H^1(X, (1 + \mathcal{I})^*) \xrightarrow{\partial}
H^2(X, j_!\mu_n)
$$
in the long exact sequence of cohomology associated to the short
exact (Kummer) sequence of sheaves of
Lemma \ref{lemma-kummer-sequence-extended}.

\begin{lemma}
\label{lemma-n-m-c1}
If $m$ is also invertible on $X$, then the composition
$$
\Pic(X, Z) \xrightarrow{c_1}
H^2_\etale(X, j_!\mu_{nm}) \xrightarrow{(\cdot)^m}
H^2_\etale(X, j_!\mu_n)
$$
is equal to $c_1$.
\end{lemma}
...
\begin{lemma}
\label{lemma-restrict-c1}
If $Z \subset Z' \subset X$ are closed subschemes then the diagram
$$
\xymatrix{
\Pic(X, Z) \ar[r]_{c_1} & H^2(X, j_!\mu_n) \\
\Pic(X, Z') \ar[r]^{c_1} \ar[u] & H^2(X, j'_!\mu_n) \ar[u]
}
$$
is commutative where $j' : X \setminus Z' \to X$ is the inclusion,
where the left vertical arrow sends $(\mathcal{L}, \sigma')$
to $(\mathcal{L}, \sigma'|_Z)$ and where the right vertical arrow
comes from the inclusion $j'_!\mu_n \subset j_!\mu_n$.
\end{lemma}
...
If $f : X' \to X$ is a morphism of schemes and $Z' = f^{-1}(Z)$,
then the diagram
$$
\xymatrix{
\Pic(X', Z') \ar[r]_{c_1} & H^2(X', j'_!\mu_n) \\
\Pic(X, Z) \ar[r]^{c_1} \ar[u] & H^2(X, j_!\mu_n) \ar[u]
}
$$
commutes where the vertical arrows are given by pullback by $f$.
\end{lemma}

\begin{proof}
Note that pullback by $f$ makes sense on the right hand side
because $f^{-1}j_!\mu_n = j'_!\mu_n$. We omit the proof.
\end{proof}
...
\section{The cohomology class of an effective Cartier divisor}
\label{section-chern-class-divisor}

\noindent
Let $X$ be a scheme. Let $n$ be an integer invertible on $X$.
Given an effective Cartier divisor $D \subset X$ we define
$$
c_1(D) = c_1(\mathcal{O}_X(D)) \in H^2(X, \mu_n)
$$
See Section \ref{section-chern-class-invertible}. We are going to
generalize this to ``relative'' cohomology as in the previous section.

\medskip\noindent
Let $U \subset X$ be an open subscheme and denote $j : U \to X$
the inclusion morphism. Let $D \subset U$ be an effective Cartier divisor
whose image in $X$ is closed. Choose any closed subscheme $Z \subset X$
whose underlying closed subset is $X \setminus U$.
The invertible module $\mathcal{O}_X(D)$ comes with a
canonical trivialization $1$ over the open $X \setminus D$,
hence a fortiori $1|_Z$ is a trivialization over the closed subscheme $Z$.
Thus we may define
$$
c_1(D) = c_1(\mathcal{O}_X(D), 1|_Z) \in H^2(X, j_!\mu_n)
$$
See Section \ref{section-chern-class-invertible}. This class is independent
of the choice of the closed subscheme $Z$ by Lemma \ref{lemma-restrict-c1}
(so for example one can replace $Z$ by any infinitesimal neighbourhood
of $Z$).

\begin{lemma}
\label{lemma-additive-c1D}
If $D_1, D_2 \subset U$ are effective Cartier divisors closed in $X$
then so is $D_1 + D_2$ and $c_1(D_1 + D_2) = c_1(D_1) + c_1(D_2)$.
\end{lemma}

\begin{proof}
True because the construction in Section \ref{section-chern-class-invertible}
is additive.
\end{proof}

\begin{lemma}
\label{lemma-pullback-c1D}
Let $f : X' \to X$ is a morphism of schemes and $U' = f^{-1}(U)$.
Assume $D' = f^{-1}(D)$ is an effective Cartier divisor in $U'$.
Then the image of $c_1(D)$ by the pullback map
$H^2(X, j_!\mu_n) \to H^2(X', j'_!\mu_n)$ is $c_1(D')$.
\end{lemma}

\begin{proof}
Note that pullback by $f$ makes sense
because $f^{-1}j_!\mu_n = j'_!\mu_n$.
Choose any closed subscheme $Z \subset X$ 
whose underlying closed subset is $X \setminus U$
and set $Z' = f^{-1}(Z)$. Note that $f^*\mathcal{O}_X(D)$
is canonically isomorphic to $\mathcal{O}_{X'}(D')$
and this isomorphism is compatible with trivializations over
$X \setminus D$ and $X' \setminus D'$. Thus the lemma
follows from Lemma \ref{lemma-pullback-c1}.
\end{proof}

\begin{lemma}
\label{lemma-finite-trivialize-invertible-upstairs-restricts-to-1}
Let $f : X \to Y$ be a finite morphism of schemes. Let $Z \subset X$
be a closed subscheme. Let $(\mathcal{L}, \sigma)$ be an pair.
Let $y \in Y$. There exists an open neighbourhood
$V \subset Y$ of $y$ such that there exists a generator
$s$ of $\mathcal{L}|_{f^{-1}(V)}$ whose restriction to $Z$ is $\sigma$.
\end{lemma}

\begin{proof}
By Divisors, Lemma \ref{divisors-lemma-finite-trivialize-invertible-upstairs}
we may assume $\mathcal{L}$ is trivial. Then it suffices to show
that $f_*\mathcal{O}_X^* \to \mathcal{O}_Z^*$ is surjective.
Looking at stalks, this translates into the following algebra problem:
Let $A \to B$ be a finite ring map with $(A, \mathfrak m)$ local
and let $I \subset B$ be an ideal. If $v$ is a unit of $B/I$, then there
exists a unit $u$ of $B$ mapping to $v$. First take any element $b \in B$
lifing $v$. The ring $B$ has finitely many maximal ideals
$\mathfrak m_1, \ldots, \mathfrak m_r$ all lying over
$\mathfrak m$. By prime avoidance we can pick an element $b' \in B$
contained in $I$ and in all maximal ideals which do not contain $b$ but
not in the maximal ideals containing both $b$ and $I$. Then
$u = b + b'$ is a unit lifting $v$.
\end{proof}

\begin{lemma}
\label{lemma-norm-c1D}
Let $f : X' \to X$ be a finite morphism of Noetherian schemes.
Let $U' \subset X'$ and $U \subset X$ be open subschemes such
that $f(U') \subset U$ and such that $g = f|_{U'} : U' \to U$
is flat. Let $D' \subset U'$ be an effective Cartier divisor
closed in $X'$ and let $D \subset U$ be its norm, see
Discriminants, Lemma \ref{discriminant-remark-norm-divisor}.
Then the image of $c_1(D')$ by the trace map
$$
H^2(X', j'_!\mu_n) = H^2(X, j_!(g_!\mu_n))
\xrightarrow{j_!\text{Tr}_{g, \mathit{flat}, \mu_n}}
H^2(X, j_!\mu_n)
$$
is $c_1(D)$.
\end{lemma}
...
\begin{proof}
(The equality in displayed equation of the lemma follows because
$j_! \circ g_! = f_! \circ j'_!$ and $f_! = f_* = Rf_*$.)
Choose a coherent ideal sheaves
$\mathcal{I}' \subset \mathcal{O}_{X'}$ and
$\mathcal{I} \subset \mathcal{O}_X$ such that
$V(\mathcal{I}) = X \setminus U$ and
$V(\mathcal{I}') = X' \setminus U'$.
Discriminants, Lemmas \ref{discriminant-lemma-norm} and
\ref{discriminant-lemma-multiplicative-norm} produce,
after replacing $\mathcal{I}'$ by a power, a polynomial law
$$
\nu : f_*\mathcal{I}' \longrightarrow \mathcal{I}
$$
satisfying a certain multiplicative property. This produces
a homomorphism of sheaves of groups
$$
N : f_*(1 + \mathcal{I}')^* \longrightarrow (1 + \mathcal{I})^*,\quad
1 + s \longmapsto 1 + \nu(1 + s)
$$
on $X_\etale$. We claim that the following diagram
$$
\xymatrix{
0 \ar[r] &
f_*j'_!\mu_n \ar[r] \ar[d]^{\text{Tr}_{g, \mathit{flat}, \mu_n}} &
f_*(1 + \mathcal{I}')^* \ar[r]^n \ar[d]^N &
f_*(1 + \mathcal{I}')^* \ar[r] \ar[d]^N &
0 \\
0 \ar[r] &
j_!\mu_n \ar[r] &
(1 + \mathcal{I})^* \ar[r]^n &
...
$$
is commutative. The commutativity of the right square follows
from the fact that $N$ is a group homomorphism. The commutativity
of the left hand square can be checked on stalks.
Let $\overline{x}$ be a geometric point of $X$.
Let $\overline{x}'_1, \ldots, \overline{x}'_t$ be the geometric points
of $X'$ lying over $\overline{x}$. If $\overline{x} \not \in U$,
then the stalk of the sheaf $f_*j'_!\mu_n$ at $\overline{x}$ is zero
and the commutativity is clear. If $\overline{x} \in U$, then
we obtain the square
$$
\xymatrix{
\prod_{\overline{x}'_i \in U'} \mu_n(\mathcal{O}_{X', \overline{x}'_i})
\times
\prod_{\overline{x}'_i \not \in U'} 0
\ar[r] \ar[d]^{\text{Tr}_{g, \mathit{flat}, \mu_n}} &
\prod_{\overline{x}'_i \in U'} \mathcal{O}_{X', \overline{x}_i}^*
\times
\prod_{\overline{x}'_i \not \in U'}
(1 + \mathcal{I}')^*_{\overline{x}'_i} \ar[d]^{\prod N_i} \\
\mu_n(\mathcal{O}_{X', \overline{x}'_i})
\ar[r] &
\mathcal{O}_{X, \overline{x}}^*
}
$$
By Discriminants, Lemma \ref{discriminant-lemma-stalks-norm} we see that
$N_i : \mathcal{O}_{X', \overline{x}_i}^* \to \mathcal{O}_{X, \overline{x}}^*$
is the norm map for the finite flat ring map
$\mathcal{O}_{X, \overline{x}}^* \to \mathcal{O}_{X', \overline{x}_i}^*$.
Moreover, the map
$\mathcal{O}_{X, \overline{x}}^* \to \mathcal{O}_{X', \overline{x}_i}^*$
induces a bijection between groups of $n$th roots of unity and for
a family of elements
$\zeta_i \in \mu_n(\mathcal{O}_{X, \overline{x}}^*)$ with image
$\zeta'_i \in \mu_n(\mathcal{O}_{X', \overline{x}_i}^*)$ we have
$$
\text{Tr}_{g, \mathit{flat}, mu_n}((\zeta_1, \ldots, \zeta_r) \times 0) =
\prod \zeta_i^{d_i}
$$
where $d_i$ is the degree of the finite flat ring map
$\mathcal{O}_{X, \overline{x}}^* \to \mathcal{O}_{X', \overline{x}_i}^*$.

\medskip\noindent
Having said this, choose a finite affine open covering $X = \bigcup W_e$
such that $D' \cap f^{-1}W_e$ is principal and cut out by
$h'_e \in \mathcal{O}_{X'}(f^{-1}W_e)$ which restricts to $1$ on 
$Z' \cap W_e$, see
Lemma \ref{lemma-finite-trivialize-invertible-upstairs-restricts-to-1}.
Then $D' \cap W_e$ is cut out by $N(h'_e)$, see
Discriminants, Lemma \ref{discriminant-lemma-norm-divisor}.
Writing $u_e = \epsilon_{ee'} u_{e'}$ we see that the cocycle
$(\epsilon_{ee'})$ defines the class of
$(\mathcal{O}_{X'}(D'), 1|_{Z'})$ in $H^1(X', (1 + \mathcal{I}')^*)$.
What we just said shows that $N$ of this element is the class
of $(\mathcal{O}_X(D), 1|_Z)$ in $H^1(X, (1 + \mathcal{I})^*)$.
Since the diagram above is commutative, we see that the boundaries
match via the trace map and the proof is complete.
\end{proof}


\section{Cohomology classes of divisors in compactly supported cohomology}
\label{section-class-divisor-compactly-supported}

\noindent
Let $f : X \to Y$ be a separated morphism of finite presentation with $Y$
quasi-compact and quasi-separated. Consider an effective Cartier divisor
$D \subset X$ such that $f|_D : D \to Y$ is proper. In this situation
there is a first chern class
$$
c_1(D) \in H^2(Y, Rf_!\mu_n)
$$
defined as follows. Choose a compactification $j : X \to \overline{X}$
of $X$ over $Y$. Since $D$ is proper over $Y$ the image $j(D)$ is
closed in $\overline{X}$. Thus the discussion above determines
an element
$$
c_1(D) \in H^2(\overline{X}, j_!\mu_n)
$$
By construction $Rf_!\mu_n = R\overline{f}_*j_!\mu_n$ and hence
the two cohomology groups in the displayed equations agree. In particular,
if $Y = \Spec(k)$ where $k$ is a field, then we obtain a class
$$
c_1(D) \in H^2_c(X, \mu_n)
$$

\begin{lemma}
\label{lemma-well-defined-c1-divisor}
This is well defined, i.e., independent of the choice of the compactification.
\end{lemma}
...
we have $g_i^{-1}(j_i(X)) = j(X)$. Thus we may assume
we have a morphism $g : \overline{X}_1 \to \overline{X}_2$
between compactifications $j_i : X \to \overline{X}_i$ over $Y$
with $g^{-1}(j_2(X)) = j_1(X)$. Denote
$$
\xi_i \in H^2(\overline{X}_i, j_{i, !}\mu_n)
$$
the cohomology class constructed above from
$D \subset X \subset \overline{X}_i$.
By our construction in the proof of Lemma \ref{lemma-shriek-well-defined}
we have to show the map
$$
H^2(\overline{X}_2, j_{2, !}\mu_n)
\longrightarrow
H^2(\overline{X}_1, j_{1, !}\mu_n)
$$
sends $\xi_2$ to $\xi_1$ where the map comes from the map
$$
j_{2, !}\mu_n \longrightarrow Rg_*j_{1, !}\mu_n
$$
constructed in Lemma \ref{lemma-shriek-lqf-and-proper}.
However, we have $j_{2, !}\mu_n = g^{-1}j_{1, !}\mu_n$ and
the map on cohomology is given by pullback, see
Remark \ref{remark-map-is-pullback}.
Thus the statement follows from Lemma \ref{lemma-pullback-c1D}.
\end{proof}

\begin{lemma}
\label{lemma-additive-c1D-relative}
Let $Y$ be a quasi-compact and quasi-separated scheme. Let $f : X \to Y$
be a separated scheme of finite presentation over $Y$.
If $D_1, D_2 \subset X$ are effective Cartier divisors proper over $Y$,
then so is $D_1 + D_2$ and $c_1(D_1 + D_2) = c_1(D_1) + c_1(D_2)$
in $H^2(Y, Rf_!\mu_n)$.
\end{lemma}

\begin{proof}
Follows from Lemma \ref{lemma-additive-c1D}.
\end{proof}

\begin{lemma}
\label{lemma-flat-covariance-c1D}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $g : X' \to X$ be a flat quasi-finite morphism of
schemes separated and of finite presentation over $Y$.
Let $D' \subset X'$ be an effective Cartier divisor proper over $Y$
and let $D \subset X$ be its norm, see
Discriminants, Lemma \ref{discriminant-remark-norm-divisor}.
Then $c_1(D') \in H^2(Y, Rf'_!\mu_n)$ maps to
$c_1(D) \in H^2(Y, Rf_!\mu_n)$ by the flat covariance map
of Remark \ref{remark-flat-covariance}.
\end{lemma}

\begin{proof}
Omitted but see Lemma \ref{lemma-norm-c1D}.
\end{proof}

\begin{lemma}
\label{lemma-proper-contravariance-c1D}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $h : X' \to X$ be a proper morphism
schemes separated and of finite presentation over $Y$.
Let $D \subset X$ be an effective Cartier divisor
proper over $Y$ such that the inverse image $D' = h^{-1}D$
is an effective Cartier divisor on $X'$.
Then $c_1(D) \in H^2(Y, Rf_!\mu_n)$ maps to
$c_1(D') \in H^2(Y, Rf'_!\mu_n)$ by the contravariance map
of Remark \ref{remark-contravariance-shriek}.
\end{lemma}

\begin{proof}
Omitted but see Lemma \ref{lemma-pullback-c1D}.
\end{proof}

\begin{lemma}
\label{lemma-generation-by-c1}
Let $k$ be an algebraically closed field. Let $X$ be a separated
scheme of finite type over $k$ of dimension $\leq 1$. Let
$C_1, \ldots, C_r$ be the irreducible components of $X$ of dimension $1$.
For each $i$ we may
\begin{enumerate}
\item pick a closed point $c_i \in C_i$ such that
\begin{enumerate}
\item $c_i \not \in C_j$ for $i \not = j$,
\item $C_i$ is smooth at $c_i$, and
\item $\mathcal{O}_{X, c_i}$ is Cohen-Macaulay
\end{enumerate}
\item pick an effective Cartier divisor $D_i \subset X$ such that
$D_i \cap C_j = \emptyset$ for $i \not = j$ and $D_i \cap C_i = c_i$
as a closed subscheme of $C_i$.
\end{enumerate}
Then $H^2_c(X, \mu_n)$ is a free $\mathbf{Z}/n\mathbf{Z}$-module
with basis $c_1(D_1), \ldots, c_1(D_r)$.
\end{lemma}

\begin{proof}
The existence of $c_i$ is a consequence of (a) openness of the smooth
locus of $C_i$, see Varieties, Lemma
\ref{varieties-lemma-dense-smooth-open-variety-over-perfect-field} and
(b) open dense locus where $X$ is CM, see
More on Morphisms, Lemma
\ref{more-morphisms-lemma-flat-finite-presentation-CM-open}.
Pick an $f_i \in \mathcal{O}_{X, c_i}$ which maps to a uniformizer
in the discrete valuation ring $\mathcal{O}_{C_i, c_i}$.
Observe that $f_i$ is a nonzerodivisor for example by
Algebra, Lemma \ref{algebra-lemma-CM-one-g}.
Choose an affine open $c_i \in U_i \subset X$ set theoretically
contained in $C_i$ such that $f_i$ comes from an element
$f_i \in \mathcal{O}_X(U_i)$ which only vanishes at $c_i$ (note that for
dimension reasons, $f_i$ a priori vanishes at only a finite number
of closed points). Then $f_i$ is a nonzerodivisor in $\mathcal{O}_X(U_i)$
too and we can set $D_i = V(f_i) \subset U_i$.
Clearly, $D_i$ is closed in $X$ whence is also an effective Cartier
divisor on $X$. The property $D_i \cap C_i = c_i$ holds by construction.

\medskip\noindent
Consider the morphism $g : C = \coprod C_i \to X$. Away from the finite
set $\bigcup_{i \not = j} C_i \cap C_j$ this morphism induces an
isomorphism from the source onto the reduction of $X$.
Hence pullback by $g$ induces an isomorphism
$H^2_c(X, \mu_n) \to H^2_c(C, \mu_n)$ by
Lemma \ref{lemma-pullback-top-algebraic-scheme}.
Moreover, this is compatible with taking $c_1(D_i)$, see
Lemma \ref{lemma-proper-contravariance-c1D}.
This reduces us to the case discussed in the next paragraph.

\medskip\noindent
Let $C$ be a curve over $k$ and let $c \in C$ be a nonsingular closed
point. Then we have to show that $H^2_c(C, \mu_n)$ is freely generated by
$c_1(c)$. By the exact same argument as before we may replace $C$
by its normalization, i.e., we may assume $C$ is nonsingular.
Then we may choose an embedding $j : C \to \overline{C}$ into a projective
nonsingular curve. Using Lemma \ref{lemma-top-algebraic-scheme}
we see the covariance map along $j$ defines an isomorphism, compatible
with taking cohomology classes of divisors by
Lemma \ref{lemma-flat-covariance-c1D}. Thus we conclude it suffices
to show that if $Y$ is a smooth projective curve, then
$H^2(Y, \mu_n)$ is freely generated by $c_1(\mathcal{O}_Y(y))$ for
any closed point $y$ of $Y$. This was shown in
\'Etale Cohomology, Lemma
\ref{etale-cohomology-lemma-cohomology-smooth-projective-curve}.
\end{proof}

\begin{lemma}
\label{lemma-generation-by-c1-nonperfect}
Let $k$ be a separably closed field. Let $X$ be a separated
scheme of finite type over $k$ of dimension $\leq 1$.
Every element of $H^2_c(X, \mu_n)$ is the cohomology class $c_1(D)$
of an effective Cartier divisor $D$ on $X$.
\end{lemma}

\begin{proof}
We may assume the characteristic of $k$ is $p > 0$ otherwise
the lemma follows from Lemma \ref{lemma-generation-by-c1}.
Choose an algebraic closure $k'$ of $k$. Denote $X'$ the base
change of $X$ to $\Spec(k')$. The base change map gives an isomorphism
$H^2_c(X, \mu_n) \to H^2_c(X', \mu_n)$, see
Lemma \ref{lemma-base-change-separably-closed}.
Let $D_1, \ldots, D_r \subset X'$ be effective Cartier divisors
whose Chern classes generate $H^2_c(X', \mu_n)$ for example
as in Lemma \ref{lemma-generation-by-c1}.
Below we will show that there exist an effective Cartier divisors
$E_i \subset X$ whose pullback to $X'$ is $q_iD_i$
where $q_i$ is a power of $p$. Then $c_1(E_i) \in H^2_c(X, \mu_n)$
pulls back to $c_1(q_iD_i) = q_ic_1(D_i)$
(Lemma \ref{lemma-additive-c1D-relative})
and these still generate as $n$ is prime to $p$.
Thus any $\xi \in H^2_c(X, \mu_n)$ can be written as $\xi = \sum a_ic_1(E_i)$
with integers $a_i \geq 0$. Then the effective Cartier divisor
$D = \sum a_iE_i$ does the job.

\medskip\noindent
We will show that an effective Cartier divisor $D \subset X'$
is up to a $p$-power multiple a pullback of an effective Cartier
divisor $E$ on $X$. Recall that $X' \to X$ is a homeomorphism on
underlying topological spaces. Thus by
Divisors, Lemma \ref{divisors-lemma-characterize-effective-Cartier-divisor}
we can find an affine open covering $X = \bigcup U_i$,
$U_i = \Spec(A_i)$ such that $D \cap (U_i)_{k'}$
is cut out by a single nonzerodivisor $f_i \in A_i \otimes_k k'$.
Since $k'/k$ is a purely inseparable extension, every element
of $k'$ to some large $p$-power is in $k$. It easily follows
that for a sufficiently large power $q$ of $p$ we have
$g_i = f_i^q \in A_i$. Then we can let $E \subset X$ be the
effective Cartier divisor such that $E \cap U_i$ is cut
out by $g_i$.
\end{proof}

\begin{lemma}
\label{lemma-relative-generation}
Let $Y$ be a quasi-compact and quasi-separated scheme. Let $f : X \to Y$
be a separated flat morphism of finite presentation and
of relative dimension $\leq 1$. Let $s$ be a section of $R^2f_!\mu_n$
over an object $V$ of $Y_\etale$. There exists an \'etale covering
$\{V_j \to V\}$ such that $s|_{V_j}$ is the cohomology class of
a relative effective Cartier divisor $D_j \subset X \times_Y V_j$
finite over $V_j$.
\end{lemma}

\begin{proof}
Let $v \in V$ be a point. It suffices to find an \'etale neighbourhood
$(V', v') \to (V, v)$ such that we have such an effective Cartier divisor
over $V'$. Let $k/\kappa(v)$ be a separable algebraic closure. By
Lemma \ref{lemma-generation-by-c1-nonperfect}
we can write the base change of $s$ to $X_k/k$ as the
cohomology class of an effective Cartier divisor $E' \subset X_k$.
Write $k = \colim k_i$ as a filtered colimit of finite separable
extensions of $\kappa(v)$. By
Limits, Lemma \ref{limits-lemma-descend-finite-presentation}
and a very easy case of
Limits, Lemma \ref{limits-lemma-descend-relative-effective-Cartier}
we see that $E'$ is the base change of an effective Cartier
divisor $E_i \subset X_{k_i}$ for some $i$. By
More on Morphisms, Lemma
\ref{more-morphisms-lemma-realize-prescribed-residue-field-extension-etale}
we can replace $V$ by an \'etale neighbourhood of $v$
and assume that we have an effective Cartier divisor $E \subset X_v$
in the fibre whose base change to $X_k$ is $E'$.
Then $c_1(E)$ agrees with the pullback of $s$ to $v = \Spec(\kappa(v))$
(because the map $\Spec(k) \to v$ induces an injective map on sections).
If we can find an elementary \'etale neighbourhood $(V', v') \to (V, v)$
and a relative effective Cartier divisor $D \subset X_{V'}$ finite
over $V'$ with $E = D_v$, then we win. Namely, in this case
$c_1(D)$ will be a section of $R^2f_!\mu_n$ over $V'$ whose value
at the point $v$ is the same as $s|_{V'}$. Then
$s$ and $c_1(D)$ have the same image in the stalk of $R^2f_!\mu_n$ and
thus agree on a further \'etale neighbourhood of $v$.

\medskip\noindent
Proof of the existence of $D$.
Since $\dim(X_v) \leq 1$ we see that $\dim(E) = 0$ and $E$ is finite
over $v$. Thus we can write $E = E_1 + \ldots + E_r$ where
each $E_i$ is set theoretically a single point. Since the category
of elementary \'etale neighbourhoods is cofiltered
(More on Morphisms, Lemma
\ref{more-morphisms-lemma-elementary-etale-neighbourhoods}), it suffices
to prove existence for $E_i$. Thus we may and do
assume $E$ is set theoretically
supported at a single point $x \in X_v$. By
More on Morphisms, Lemma \ref{more-morphisms-lemma-construct-relative-divisors}
we can find an open neighbourhood
$U \subset X_V$ of $c$ and a relative effective Cartier divisor
$D \subset U$ with $D \cap U_v = E \cap U_v$ (which consists set
theoretically of exactly 1 point, namely $x$). Then $D \to V$
has relative dimension $\leq 0$, i.e., is quasi-finite.
By \'etale localization of quasi-finite morphisms
(for example More on Morphisms, Lemma
\ref{more-morphisms-lemma-etale-splits-off-quasi-finite-part-technical})
we may after replacing $V$ by an elementary \'etale neighbourhood
assume $D = D_{fin} \amalg D'$ with $x \in D_{fin}$ and
$D_{fin} \to V$ finite. Then $D_{fin}$ is the divisor we
are looking for.
\end{proof}


\section{Trace map for curves}
\label{section-trace-curves}

\noindent
Let $X$ be a separated finite type scheme over a field $k$
with $\dim(X) \leq 1$. We are going to construct a ``flat'' trace map
$$
\text{Tr}_{X/k, \mathit{flat}, n} :
H^2_c(X, \mu_n)
\longrightarrow
\mathbf{Z}/n\mathbf{Z}
$$
for all $n$ prime to the characteristic of $k$ and prove some of its
properties.

\begin{lemma}
\label{lemma-fixed-by-c1}
Let $k$ be a field. Let $n \geq 1$ invertible in $k$. Let $X$ be a
separated finite type scheme over $k$ of dimension $\leq 1$.
...
\begin{enumerate}
\item formation of $\text{Tr}_{X/k, \mathit{flat}, n}$ is compatible with
change of fields, and
\item $\text{Tr}_{X/k, \mathit{flat}, n}(c_1(D)) = \deg(D)$ when
$D \subset X$ is an effective Cartier divisor.
\end{enumerate}
\end{lemma}

\begin{proof}
Proof of uniqueness. For an algebraically closed field $k$
we have the uniqueness of our map by condition (2) and
Lemma \ref{lemma-generation-by-c1}. For arbitrary fields $k$
the uniqueness then follows by (1) and the case of the base
change of $X$ to the algebraic closure of $k$.

\medskip\noindent
Proof of existence. Choose an open immersion $j : X \to \overline{X}$
into a projective scheme of dimension $1$ over $k$, see
Varieties, Lemma \ref{varieties-lemma-dim-1-projective-completion}.
Consider the map
$\text{Tr}_j : H^2_c(X, \mu_n) \to H^2_c(\overline{X}, \mu_n)$
of Remark \ref{remark-covariance-compactly-supported}.
This map is compatible with change of fields.
Observe that any effective Cartier divisor $D$ in $X$ is finite
(hence proper) over $k$, hence $c_1(D\text{ in }X)$
maps to $c_1(D\text{ in }\overline{X})$ by the map $\text{Tr}_j$, see
Lemma \ref{lemma-flat-covariance-c1D} (although in this case
this compatibility holds more or less by construction).
Thus it suffices to prove existence when $X$ is projective over $k$.

\medskip\noindent
Assume $X$ is projective over $k$. In this case we have
$H^2_c(X, \mu_n) = H^2(X, \mu_n)$, see
Lemma \ref{lemma-proper-Rf-lower-shriek}.
By the Kummer sequence discussed in
\'Etale Cohomology, Section \ref{etale-cohomology-section-kummer}.
we have a short exact sequence
$$
0 \to
\Pic(X)/n\Pic(X) \xrightarrow{c_1}
H^2(X, \mu_n) \to
H^2(X, \mathbf{G}_m)[n] \to 0
$$
By Varieties, Section \ref{varieties-section-divisors-curves}
we have a well defined degree map
$\deg : \Pic(X)/n\Pic(X) \to \mathbf{Z}/n\mathbf{Z}$
compatible with change of field such that
$\deg(\mathcal{O}_X(D)) = \deg(D)$.
Since $c_1(D) = c_1(\mathcal{O}_X(D))$ and using
Divisors, Lemma
\ref{divisors-lemma-quasi-projective-Noetherian-pic-effective-Cartier}
we see that condition (2) forces us to use the degree map
on the image of $c_1$ in the short exact sequence above
and moreover this rule is compatible with change of fields.
However, over the algebraic closure we see that $c_1$ is surjective by
Lemma \ref{lemma-generation-by-c1} and this finishes the proof.
Namely, for a general element $\xi$ of
$H^2(X, \mu_n)$ we compute $\text{Tr}_{X/k, \mathit{flat}, n}(\xi)$
by pulling $\xi$ to the base change of $X$ to the algebraic closure,
writing it as the chern class of an invertible module, and
taking the degree modulo $n$.
\end{proof}

\begin{lemma}
\label{lemma-flat-covariance-trace-curve}
Let $k$ be a field and $n \geq 1$ invertible in $k$.
Let $g : X' \to X$ be a flat quasi-finite morphism of
separated finite type schemes of dimension $\leq 1$ over $k$.
Then $\text{Tr}_{X'/k, \mathit{flat}, n} =
\text{Tr}_{X'/k, \mathit{flat}, n} \circ \text{Tr}_g$
where $\text{Tr}_g$ is the ``flat'' covariance map
of Remark \ref{remark-covariance-compactly-supported}.
\end{lemma}

\begin{proof}
This follows from the uniqueness in
Lemma \ref{lemma-fixed-by-c1}, combined with
Lemma \ref{lemma-flat-covariance-c1D}, and
Discriminants, Lemma \ref{discriminant-lemma-degree-norm}.
\end{proof}

\begin{lemma}
\label{lemma-proper-contravariance-trace-curve}
Let $k$ be a field and $n \geq k$ invertible in $k$. Let $d \geq 1$.
Let $X$ be a separated finite type scheme of dimension $\leq 1$ over $k$.
Let $f : X' \to X$ be a finite flat morphism of degree $d$.
We have
$$
\text{Tr}_{X'/k, \mathit{flat}, n} \circ f^* =
d \text{Tr}_{X/k, \mathit{flat}, n}
$$
where $f^*$ is the contravariance map of
Remark \ref{remark-contravariance-compactly-supported}.
\end{lemma}

\begin{proof}
Any effective Cartier divisor $D \subset X$
pulls back to an effective Cartier divisor $D' \subset X'$
and $D' \to D$ is finite free of degree $d$.
Hence $\deg(D') = d\deg(D)$. Hence the lemma follows from the uniqueness in
Lemma \ref{lemma-fixed-by-c1} combined with
Lemma \ref{lemma-proper-contravariance-c1D}.
\end{proof}

\begin{example}
\label{example-elucidate}
Let $k$ be an algebraically closed field and $n \geq 1$ invertible in $k$.
Let $X$ be a separated finite type scheme of dimension $\leq 1$ over $k$.
Let $C_1, \ldots, C_r \subset X$ be the irreducible components of
dimension $1$, viewed as reduced closed subschemes of $X$.
The inclusion morphisms $h_i : C_i \to X$ induce pullback maps
$$
h_i^* : H^2_c(X, \mu_n) \to H^2_c(C_i, \mu_n)
$$
see Remark \ref{remark-contravariance-compactly-supported}.
Choose $c_i \in C_i$ and effective Cartier divisors $D_i \subset X$ as in
Lemma \ref{lemma-generation-by-c1}. It follows immediately from the
description of the cohomology in the lemma (for both $X$ and the
$C_i$) and our construction that
$$
\text{Tr}_{X/k, \mathit{flat}, n} = 
\sum\nolimits_{i = 1, \ldots, r}
\deg(D_i) \text{Tr}_{C_i/k, \mathit{flat}, n} \circ h_i^*
$$
This suggest that the value of $\deg(D_i)$ is intrinsic (independent
of the choice of $c_i \in C_i$ and $D_i$). Indeed, this follows from
Chow Homology, Lemma \ref{chow-lemma-prepare-geometric-cap} which tells
us that $\deg(D_i)$ is the multiplicity of $C_i$ in $X$.
In other words, if we write
$$
[X]_1 = \sum m_i [C_i]
$$
in the cycle group $Z_1(X)$, see Chow Homology, Definition
\ref{chow-definition-cycle-associated-to-closed-subscheme}, then we have
$$
\text{Tr}_{X/k, \mathit{flat}, n} = 
\sum\nolimits_{i = 1, \ldots, r}
m_i \text{Tr}_{C_i/k, \mathit{flat}, n} \circ h_i^*
$$
This means our trace map for curves agrees with the one in
\cite[Exposee XVII, Equation (1.1.3.3)]{SGA4}.
\end{example}
...
\section{The trace map for flat morphisms of relative dimension 1}
\label{section-construction-dim-1}

\noindent
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $n \geq 1$ be invertible on $Y$.
Let $f : X \to Y$ be a flat, separated morphism of finite presentation
of relative dimension $\leq 1$. We will construct a map
$$
\text{Tr}_{f, \mathit{flat}, n} :
R^2f_!\mu_n
\longrightarrow
\mathbf{Z}/n\mathbf{Z}
$$
which on the stalks reproduces the construction of
Section \ref{section-trace-curves}.

\begin{lemma}
\label{lemma-fixed-by-c1-relative}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $n \geq 1$ be invertible on $Y$. Let $f : X \to Y$
be flat, separated, of finite presentation, and of
relative dimension $\leq 1$. There exists a unique map
$\text{Tr}_{f, \mathit{flat}, n} :
R^2f_!\mu_n
\to
\mathbf{Z}/n\mathbf{Z}$
with the following properties
\begin{enumerate}
\item formation of $\text{Tr}_{f, \mathit{flat}, n}$ is compatible with
base change by morphisms $Y' \to Y$ with $Y'$
quasi-compact and quasi-separated, and
\item $\text{Tr}_{f, \mathit{flat}, n}(c_1(D)) = \deg(D \to Y)$
when $D \subset X$ is a relative effective Cartier divisor
which is finite over $Y$.
\end{enumerate}
\end{lemma}

\begin{proof}
We already know this is true when $Y$ is the spectrum of a field,
see Lemma \ref{lemma-fixed-by-c1}.

\medskip\noindent
Let $Y$ be general as in the statement of the lemma. We introduce
a bit of notation. For an object $V$ of $Y_\etale$, a point $v \in V$,
a sheaf $\mathcal{F}$ on $Y_\etale$, and a section $s \in \mathcal{F}(V)$,
we say the {\it value} of $s$ at $v$ is the element
$s|_v \in \Gamma(v, \mathcal{F}|_v)$ which is the pullback of
$s$ by the morphism $v \to V$.

\medskip\noindent
Let $s$ be a section of $R^2f_!\mu_n$ over an object $V$ of $Y_\etale$.
We claim there exists a unique section $t$ of $\mathbf{Z}/n\mathbf{Z}$
over $V$ such that for every point $v$ of $V$ the value of $t$ at $v$
is the image of the value of $s$ at $v$ via the trace map for $X_v/v$.
Once we've shown this, the rule sending $s$ to $t$ defines a map
$\text{Tr}_{f, \mathit{flat}, n} : R^2f_!\mu_n \to \mathbf{Z}/n\mathbf{Z}$,
it shows uniquess of the map, and it these maps are compatible
with base change as this holds for the maps in Lemma \ref{lemma-fixed-by-c1}.

\medskip\noindent
The uniqueness of $t$ is obvious. To show existence of $t$
choose an \'etale covering $\{V_j \to V\}$ such that $s|_{V_j}$
is the cohomology class of a relative effective Cartier divisor
$D_j \subset X_{V_j}$ finite over $V_j$, see
Lemma \ref{lemma-relative-generation}.
Set $t_j = \deg(D_j \to V_j) \in \Gamma(V_j, \mathbf{Z}/n\mathbf{Z})$.
For any point $v_j \in V_j$ the value of $t_j$ at $v_j$ is the
image of the value of $s$ at $v_j$ by the trace map for $X_{v_j}/v_j$;
this is true because the value of $\deg(D_j \to V_j)$ at $v_j$
is the degree of $(D_j)_{v_j}$ over $v_j$ and the construction
of the map over fields. By uniqueness and compatibility with base
chase, the sections $t_j$ and $t_{j'}$ agree over $V_j \times_V V_{j'}$
(small detail omitted).
Thus these sections glue to a section $t$ over $V$
and this section has the desired property (small detail omitted).

\medskip\noindent
Finally, if $D \subset X$ is as in (2), then
$\text{Tr}_{f, \mathit{flat}, n}(c_1(D)) = \deg(D \to Y)$
as this holds for the values at all points by construction.
\end{proof}

\begin{lemma}
\label{lemma-flat-covariance-trace-relative-curve}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $n \geq 1$ be invertible on $Y$. Let $f : X \to Y$
and $f' : X' \to Y$ be separated morphisms of finite presentation
and of relative dimension $\leq 1$. Let $g : X' \to X$
be a flat quasi-finite morphism over $Y$.
Then $\text{Tr}_{f', \mathit{flat}, n} =
\text{Tr}_{f, \mathit{flat}, n} \circ \text{Tr}_g$
where $\text{Tr}_g$ is the ``flat'' covariance map
of Remark \ref{remark-flat-covariance}.
\end{lemma}

\begin{proof}
Looking at stalks this lemma reduces to the case of fields
which is Lemma \ref{lemma-flat-covariance-trace-curve}
(but one can also directly deduce as in the proof of that lemma).
\end{proof}

\begin{lemma}
\label{lemma-proper-contravariance-trace-relative-curve}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $n \geq 1$ be invertible on $Y$. Let $f : X \to Y$
and $f' : X' \to Y$ be separated morphisms of finite presentation
and of relative dimension $\leq 1$. Let $g : X' \to X$ be
a morphism over $Y$ which is finite locally free of degree $d$.
We have
$$
\text{Tr}_{f', \mathit{flat}, n} \circ g^* =
d \text{Tr}_{f, \mathit{flat}, n}
$$
where $g^*$ is the contravariance map of
Remark \ref{remark-contravariance-compactly-supported}.
\end{lemma}

\begin{proof}
Looking at stalks this lemma reduces to the case of fields
which is Lemma \ref{lemma-proper-contravariance-trace-curve}
(but one can also directly deduce as in the proof of that lemma).
\end{proof}

\begin{lemma}
\label{lemma-trace-iso-relative-curve}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $n \geq 1$ be invertible on $Y$. Let $f : X \to Y$
be a separated flat morphism of finite presentation all
of whose fibres are geometrically integral of dimension $1$.
Then
$\text{Tr}_{f, \mathit{flat}, n} : R^2f_!\mu_n \to \mathbf{Z}/n\mathbf{Z}$
is an isomorphism.
\end{lemma}
...
\begin{proof}
(Recall that an integral scheme is irreducible and hence nonempty.)
Checking in geometric points of $Y$ we see we obtain isomorphisms
by the discussion in Example \ref{example-elucidate}.
\end{proof}

\begin{remark}
\label{remark-cup-product-trace}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $f : X \to Y$ be a proper, flat morphism of finite presentation
with fibres of dimension $\leq 1$. Let $\mathcal{L}$ be an invertible
$\mathcal{O}_X$-module whose degree on every fibre of $f$ is $d$.
Let $n$ be an integer invertible on $Y$ and let
$\mathcal{F}$ be a sheaf of $\mathbf{Z}/n\mathbf{Z}$-modules on $Y_\etale$.
For $\eta \in H^i(Y, \mathcal{F})$ consider the element
$$
c_1(\mathcal{L}) \cup f^*\eta \in
H^{2 + i}(X, \mu_n \otimes f^{-1}\mathcal{F})
$$
All tensor products are over $\mathbf{Z}/n\mathbf{Z}$ in this remark.
Then we can use the sequence of maps
\begin{align*}
H^{2 + i}(X, \mu_n \otimes f^{-1}\mathcal{F})
& =
H^{2 + i}(Y, Rf_*(\mu_n \otimes f^{-1}\mathcal{F})) \\
& =
H^{2 + i}(Y, Rf_*\mu_n \otimes \mathcal{F}) \\
& \to
H^i(Y, R^2f_*\mu_n \otimes \mathcal{F}) \\
& \xrightarrow{\text{Tr}_{f, \mathit{flat}, n}}
H^i(Y, \mathcal{F})
\end{align*}
to get back to an element of $H^i(Y, \mathcal{F})$. It turns out that
the output is the element $d\eta$. This follows from the defining property
of our trace map, namely that the composition
$$
\mathbf{Z}/n\mathbf{Z} \xrightarrow{c_1(\mathcal{L})}
R^2f_*\mu_n \xrightarrow{\text{Tr}_{f, \mathit{flat}, n}} \mathbf{Z}/\mathbf{Z}
$$
is equal to multiplication by $d$ (as one sees on the stalks for example).
\end{remark}




\section{The trace map for an affine space}
\label{section-trace-affine-space}

\noindent
Let $Y$ be a quasi-compact and quasi-separated scheme. By induction
on $d \geq 0$ we are going to construct a canonical isomorphism
$$
\text{Tr}_{d, Y, n} :
R(\mathbf{A}^d_Y \to Y)_!\mu_n^{\otimes d}
\longrightarrow
\mathbf{Z}/n\mathbf{Z}
$$
compatible with base change by morphisms $Y' \to Y$ of quasi-compact
and quasi-separated schemes. For $d = 0$ we use the identity map.

\medskip\noindent
For $d = 1$ we use the canonical map
$\text{Tr}_{\mathbf{A}^1_Y \to Y, \mathit{flat}, n}$,
see Lemma \ref{lemma-trace-iso-relative-curve}. Given any automorphism
$g : \mathbf{A}^1_Y \to \mathbf{A}^1_Y$ over $Y$ the diagram
$$
\xymatrix{
R^2(\mathbf{A}^1_Y \to Y)_!\mu_n
\ar[rd]_{\text{Tr}_{1, Y, n}} \ar[rr]_{covariance} & &
R^2(\mathbf{A}^1_Y \to Y)_!\mu_n \ar[ld]^{\text{Tr}_{1, Y, n}} \\
& \mathbf{Z}/n\mathbf{Z}
}
$$
commutes. Here $covariance$ is the map associated to $g$ in
Remark \ref{remark-covariance-shriek}; the diagram commutes by
Lemma \ref{lemma-flat-covariance-trace-relative-curve}.

\medskip\noindent
Let $d > 1$ we consider the sequence of maps
$$
\mathbf{A}^d_Y \xrightarrow{p} \mathbf{A}^{d - 1}_Y \xrightarrow{q} Y
$$
where $p$ is the projection onto the last $d - 1$ coordinates
and $q$ is the structure morphism.
The isomorphism $R(q \circ p)_! = Rq_! \circ Rp_!$ of
Lemma \ref{lemma-shriek-composition},
the spectral sequence of
Lemma \ref{lemma-two-ss-complex-functor},
and the vanishing of
Lemma \ref{lemma-bounded-f-shriek}
combine to produce an isomorphism
$$
R^{2d}(\mathbf{A}^d_Y \to Y)_!\mathcal{F} =
R^{2d - 2}q_!R^2p_!\mathcal{F}
$$
for any torsion sheaf $\mathcal{F}$ on $\mathbf{A}^d_Y$. Then we construct
our map as the composition
\begin{align*}
R^{2d}(\mathbf{A}^d_Y \to Y)_!\mu_n^{\otimes d}
& =
R^{2d - 2}q_!R^2p_!\mu_n^{\otimes d} \\
& =
R^{2d - 2}q_!((R^2p_!\mu_n) \otimes \mu_n^{\otimes d - 1}) \\
& \xrightarrow{\text{Tr}_{1, \mathbf{A}^{d - 1}_Y, n}}
R^{2d - 2}q_!(\mu_n^{\otimes d - 1}) \\
& \xrightarrow{\text{Tr}_{d - 1, Y, n}}
\mathbf{Z}/n\mathbf{Z}
\end{align*}
Let us prove that this map does not depend on the choice of the
projection; in fact taking $p$ to be any affine linear projection
would give the same map.

\begin{lemma}
\label{lemma-independent-choice-affine-space}
Let $Y$ be a quasi-compact and quasi-separated scheme. Let $d \geq 0$.
For any affine linear automorphism $g : \mathbf{A}^d_Y \to \mathbf{A}^d_Y$
the diagram
$$
\xymatrix{
R^{2d}(\mathbf{A}^d_Y \to Y)_!\mu_n^{\otimes d}
\ar[rd]_{\text{Tr}_{d, Y, n}} \ar[rr]_{covariance} & &
R^{2d}(\mathbf{A}^d_Y \to Y)_!\mu_n^{\otimes d}
\ar[ld]^{\text{Tr}_{d, Y, n}} \\
& \mathbf{Z}/n\mathbf{Z}
}
$$
commutes. Here $covariance$ is the map associated to $g$ in
...
\begin{proof}
The idea of the proof is as follows: the diagram defines two
arrows
$$
\varphi, \psi :
R^{2d}(\mathbf{A}^d_Y \to Y)_!\mu_n^{\otimes d}
...
\mathbf{Z}/n\mathbf{Z}
$$
The source and target of these arrows are both isomorphic to the
constant sheaf $\mathbf{Z}/n\mathbf{Z}$, hence if $\varphi$ and $\psi$ agree
in a point $y \in Y$, then they agree at every point of the connected
component of $Y$ that $y$ is in. Since the space of all affine
linear automorphisms is connected, this proves the lemma. Below
we give a bit more detail.

\medskip\noindent
An affine linear automorphism is one of the form
$$
x \longmapsto Ax + b
$$
were $A$ is an invertible $d \times d$ matrix and $b$ is a vector
both with coefficients in $\Gamma(Y, \mathcal{O}_Y)$. There exists
a smooth scheme $G \to Y$ and a universal affine linear automorphism
$$
\mathbf{A}^d_G = G \times_Y \mathbf{A}^d_Y
\longrightarrow
G \times_Y \mathbf{A}^d_Y = \mathbf{A}^d_G
$$
Namely, we set $G = \text{GL}_{d, Y} \times_Y \mathbf{A}^d_Y$
and use the formula above to define the arrow. Thus we get two
arrows $\varphi_{univ}, \psi_{univ}$ over $G$ with the following
property: given $(A, b)$ as above, these define a morphism
$\tau : Y \to G$ over $Y$ and the pullbacks of $\varphi_{uni}, \psi_{univ}$ 
are $\varphi$ and $\psi$. Consider the morphism
$\sigma : Y \to G$ corresponding to the identity
affine linear automorphism. By the same token as above the pullbacks of
$\varphi_{univ}$ and $\psi_{univ}$ agree! Thus every fibre of $G \to Y$
contains a point where $\varphi_{univ}$ and $\psi_{univ}$ agree.
Since the fibres of $G \to Y$ are connected we conclude.
\end{proof}

\begin{remark}
\label{remark-trace-P1d}
Let $k$ be an algebraically closed field. Let $n \geq 1$ be invertible in $k$.
Let us give a little bit more insight into the map
$\text{Tr}_{d, \Spec(k), n}$ which we may view as a map
$$
H^{2d}_c(\mathbf{A}^d_k, \mu_n^{\otimes d}) \to \mathbf{Z}/n\mathbf{Z}
$$
It follows from Lemma \ref{lemma-flat-covariance-trace-relative-curve}
that the diagram
$$
\xymatrix{
H^{2d}_c(\mathbf{A}^d_k, \mu_n^{\otimes d}) \ar[r] \ar[d] &
H^{2d}((\mathbf{P}^1_k)^d, \mu_n^{\otimes d}) \ar[d] \\
H^{2d}_c(\mathbf{A}^{d - 1}_k, \mu_n^{\otimes d - 1}) \ar[r] &
H^{2d}((\mathbf{P}^1_k)^{d - 1}, \mu_n^{\otimes d - 1})
}
$$
commutes. Here the horizontal arrows are covariance along the open
embeddings $\mathbf{A}^w_k \to (\mathbf{P}^1_k)^w$ and are isomorphisms
and the vertical maps are the ``flat'' trace maps for the
flat families of curves defined by the projections
$p : \mathbf{A}_k^d \to \mathbf{A}_k^{d - 1}$ and
$p' : (\mathbf{P}_k^1)^d \to (\mathbf{P}_k^1)^{d - 1}$ onto the last
$d - 1$ coordinates. Denote
$\xi_i \in H^2((\mathbf{P}^1_k)^d, \mu_n)$ the pullback
of $c_1(\mathcal{O}(1))$ from the $i$ projection onto $\mathbf{P}^1_k$.
By Remark \ref{remark-cup-product-trace} we see that given
$\eta \in H^{2d - 2}((\mathbf{P}^1_k)^{d - 1}, \mu_n^{\otimes d - 1})$
the cup product
$\xi_1 \cup (p')^*\eta$ maps to $\eta$ by the right most arrow.
Repeating this $d - 1$ more times and using that our map
is constructed by successively using these traces along projections,
we see that our map sends
$$
\xi_1 \cup \ldots \cup \xi_d \in
H^{2d}((\mathbf{P}^1_k)^d, \mu_n^{\otimes d}) =
H^{2d}_c(\mathbf{A}_k^d, \mu_n^{\otimes d})
$$
to $1$. The result of Lemma \ref{lemma-independent-choice-affine-space}
says in particular that if we permute the coordinates, we get the same result.
This can also be seen, since we know that the displayed cohomology
group is free on $\xi_1 \cup \ldots \cup \xi_d$, by proving that
the cup product is independent of the ordering of the $\xi_i$.
Indeed, this follows from Cohomology on Sites, Lemma
\ref{sites-cohomology-lemma-cup-product-commutative}
and the fact that our classes have even degrees.
\end{remark}
...
\section{The trace map for flat morphisms}
\label{section-construction-flat}

\noindent
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $n \geq 1$ be invertible on $Y$. Let $d \geq 0$.
Let $f : X \to Y$ be a flat, separated morphism of finite presentation
of relative dimension $\leq d$. In this section we construct the map
$$
\text{Tr}_{f, \mathit{flat}, n} :
R^{2d}f_!\mu_n^{\otimes d}
\longrightarrow
\mathbf{Z}/n\mathbf{Z}
$$
The integer $d$ does not appear in the notation and has to be
inferred from context.

\begin{lemma}
\label{lemma-covering-right-exact}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $f : X \to Y$ be separated and of finite presentation
with fibres of dimension $\leq d$. Let $U_1, \ldots, U_r \subset X$
be quasi-compact open subschemes such that
$X \setminus (U_1 \cup \ldots \cup U_r)$
has fibres of dimension $\leq d - 1$ over $Y$.
For any torsion sheaf $\mathcal{F}$ on $X$ there is an
exact sequence
$$
\bigoplus\nolimits_{i, i'} R^{2d}f_{ii'!}\mathcal{F}|_{U_{ii'}}
\to
\bigoplus\nolimits_i R^{2d}f_{i!}\mathcal{F}|_{U_i}
\to
R^{2d}f_!\mathcal{F}
\to
0
$$
of sheaves on $Y_\etale$ where the arrows are given up to sign
by the covariance maps of Remark \ref{remark-covariance-shriek}.
\end{lemma}

\begin{proof}
By Lemma \ref{lemma-top-restrict-to-open} we may replace $X$ by $\bigcup U_i$.
Thus we may assume $X = U_1 \cup \ldots \cup U_n$.
Consider the exact complex of sheaves
$$
\bigoplus\nolimits_{i, i'} j_{ii'!}j_{ii'}^{-1}\mathcal{F}
\to
...
0
$$
on $X$; this is a special case of Lemma \ref{lemma-lqf-colimit-f-shriek}.
Apply the right exact functor $R^{2d}f_!$ (by the vanishing of
Lemma \ref{lemma-bounded-f-shriek}) to get
the result of the lemma. Details omitted.
\end{proof}

\begin{lemma}
\label{lemma-good-covering}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $f : X \to Y$ be separated and of finite presentation
with fibres of dimension $\leq d$. There exists a finite
affine open covering $X = \bigcup U_i$ such that $f|_{U_i} : U_i \to Y$
factors as $U_i \to \mathbf{A}^d_Y \to Y$ where the first
arrow is quasi-finite.
\end{lemma}

\begin{proof}
Reducing to affines, this follows from More on Morphisms, Lemma
\ref{more-morphisms-lemma-weak-relative-noether-normalization}.
It can also be deduced from the easier
Algebra, Lemma \ref{algebra-lemma-quasi-finite-over-polynomial-algebra}.
\end{proof}

\begin{lemma}
\label{lemma-switch-coordinates}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $X$ be separated and of finite presentation over $Y$.
Let $(g_1, \ldots, g_d) : X \to \mathbf{A}^d_Y$ and
$(h_1, \ldots, h_d) : X \to \mathbf{A}^d_Y$
be quasi-finite. Let $U_i \subset X$ be the quasi-compact
open subset where $(h_i, g_2, \ldots, g_n) : X \to \mathbf{A}^d_Y$
is quasi-finite. Then $X \setminus (U_1 \cup \ldots \cup U_n)$
has fibres of dimension $< d$ over $Y$.
\end{lemma}

\begin{proof}
Observe that $U_i$ is open and quasi-compact by Morphisms, Lemma
\ref{morphisms-lemma-openness-bounded-dimension-fibres-finite-presentation}.
To prove the final statement, we may base change to a geometric
point of $Y$ to reduce to the case discussed in the next paragraph.

\medskip\noindent
Let $Y = \Spec(k)$ where $k$ is an algebraically closed field.
Suppose that $x \in X$ is a closed point which
is not contained in any of the $U_i$.
Let $t$ be the image of $x$ by the morphism
$G = (g_2, \ldots, g_d) : X \to \mathbf{A}^{d - 1}_k$.
Let $X_t$ be the scheme theoretic fibre of $F$ over $t$.
Since $(g_1, \ldots, g_d)$ is quasi-finite, it follows
that $g_1|_{X_t} : X_t \to \mathbf{A}^1_k$ is quasi-finite.
Hence $X_t$ is a finite type scheme of dimension $\leq 1$ over $k$.
By assumption for every $i$ the morphism
$h_i|_{X_t} : X_t \to \mathbf{A}^1_k$ is not quasi-finite at $x$.
This means there is an irreducible component $x \in C_i \subset X_t$
of dimension $1$ such that $h_i$ collapses $C_i$ to a point.
It cannot be the case that $C_1 = \ldots = C_d$, since that
would imply that $(h_1, \ldots, h_d)$ sends $C_1$ to a point,
contradiction the assumption that $(h_1, \ldots, h_d)$ is quasi-finite.
Thus $x$ is contained in an intersection of irreducible components
of $X_t$. It follows that the fibres of the restricition of $G$ to
$X \setminus (U_1 \cup \ldots \cup U_n)$ are finite (set theoretically).
Hence the dimension of $X \setminus (U_1 \cup \ldots \cup U_n)$
is at most $d - 1$.
\end{proof}

\noindent
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $f : X \to Y$ be a separated flat morphism of finite presentation
which is in addition Cohen-Macaulay (More on Morphisms, Definition
\ref{more-morphisms-definition-CM}) and has relative dimension $d$
(Morphisms, Definition \ref{morphisms-definition-relative-dimension-d}).
Let $g : X \to \mathbf{A}^d_Y$ be a quasi-finite morphism over $Y$;
note that by More on Morphisms, Lemma
\ref{more-morphisms-lemma-flat-finite-presentation-characterize-CM}
the condition that $f$ is Cohen-Macaulay of relative dimension $d$
implies that $g$ is flat (and the converse holds true as well).
In this case we can consider the map
\begin{equation}
\label{equation-trace-by-qf}
Rf_!\mu_n^{\otimes d} \xrightarrow{\text{Remark \ref{remark-flat-covariance}}}
R(\mathbf{A}^d_Y \to Y)_!\mu_n^{\otimes d}
\xrightarrow{\text{Tr}_{d, Y, n}}
\mathbf{Z}/n\mathbf{Z}
\end{equation}
Thus the first arrow is ``flat'' covariance and the second arrow is
the map construction in Section \ref{section-trace-affine-space}.

\begin{lemma}
\label{lemma-trace-by-qf}
The map (\ref{equation-trace-by-qf}) does not depend on the choice of $g$.
\end{lemma}

\begin{proof}
We have to show: given two quasi-finite morphisms
$g = (g_1, \ldots, g_d) : X \to \mathbf{A}^d_Y$ and
$h = (h_1, \ldots, h_d) : X \to \mathbf{A}^d_Y$, we
obtain the same arrow.

\medskip\noindent
Note that we already know that $(g_1, g_2, g_3, \ldots, g_d)$ and
$(g_2, g_1, g_3, \ldots, g_d)$ produce the same map by
Lemma \ref{lemma-independent-choice-affine-space} (this also
uses the compatibility of the ``flat'' covariance maps with compositions
discussed in Remark \ref{remark-contravariance-shriek}).
The same goes for any permutation of the $g_i$. (See also
discussion in Remark \ref{remark-trace-P1d} for why this holds.)

\medskip\noindent
Suppose we have quasi-compact open subschemes $U_i \subset X$,
$i = 1, \ldots, r$ such that $X \setminus \bigcup U_i$ has
fibres of dimension $< d$ over $Y$. Then it suffices to
show that the two arrows, composed with the covariance maps
$$
R^{2d}(f|_{U_i})_!\mu_n^{\otimes d}
\to
R^{2d}f_!\mu_n^{\otimes d}
$$
agree, see Lemma \ref{lemma-covering-right-exact}.
By the compatibility of the ``flat'' covariance maps
with compositions, this reduces us to proving the result
for $g|_{U_i}$ and $h|_{U_i}$ for $i = 1, \ldots, r$.
In this sense the problem is local on $X$.

\medskip\noindent
Note that if for some
$i \not = j$ we have $g_i = g_j$. Then we see that
the relative dimension of $X$ over $Y$ is at most $d - 1$.
In this case the source of our arrow is $0$ and the
lemma holds in this case. Similarly for the functions $h_i$.
Thus we may assume the functions $g_i$ are pairwise distinct
and the same is true for the functions $h_i$.
(This step is not necessary but it simplifies the arguments
below.)

\medskip\noindent
We argue by descending induction on the number $r$
of indices $i$ such that $g_i$ occurs in the set
$\{h_1, \ldots, h_d\}$. If $r = d$, then $g = h$ up to a
permutation of the coordinates and the result is true.
If $r < d$, then we may, after permuting the coordinates,
assume $g_1$ is not equal to any of the $h_i$.
Let $U_i \subset X$ be the open subset where
$(h_i, g_2, \ldots, g_d) : X \to \mathbf{A}^d_Y$ is quasi-finite.
By Lemma \ref{lemma-switch-coordinates} and the discussion above,
we may replace $X$ by $U_i$, in other words, we may assume
$(h_i, g_2, \ldots, g_d)$ is quasi-finite for some $i$.

\medskip\noindent
In the last paragraph of the proof we will show that $(g_1, \ldots, g_d)$ and
$(h_i, g_2, \ldots, g_d)$ define the same arrow.
Since may assume $h_i$ is not equal to $g_2, \ldots, g_d$ (see above),
a simple counting argument shows that $\{h_i, g_2, \ldots, g_d\}$
and $\{h_1, \ldots, h_d\}$ have at least $r + 1$ elements in common
and we conclude by induction.

\medskip\noindent
Suppose that $g = (g_1, \ldots, g_d) : X \to \mathbf{A}^d_Y$
is quasi-finite. We will show that (\ref{equation-trace-by-qf})
only depends on $G = (g_2, \ldots, g_d) : X \to \mathbf{A}^{d - 1}_Y$.
Observe that $G$ is a flat morphism of relative dimension $1$
(as the composition of the two flat morphisms $g$ and projection onto
the last $d - 1$ coordinates).
Consider the commutative diagram
$$
\xymatrix{
X \ar[rr]_{(g_1, \ldots, g_d)} \ar[rd]_G & & \mathbf{A}^d_Y \ar[ld]^p \\
& \mathbf{A}^{d - 1}_Y \ar[d]^q \\
& Y
}
$$
where $p$ is the projection onto the last $d - 1$ coordinates. 
By construction the map (\ref{equation-trace-by-qf}) is constructed
by writing the source as
$$
R^{2d}f_!\mu_n^{\otimes d} =
R^{2d - 2}q_!(R^2p_!(g_!\mu_n) \otimes \mu_n^{\otimes d - 1})
$$
and then using
$$
\text{Tr}_{d - 1, Y, n}
\circ
R^{2d - 2}q_!\left(
\left(\text{Tr}_{p, \mathit{flat}, n} \circ R^2p_!(\text{Tr}_g)\right)
\otimes \text{id}_{\mu_n^{\otimes d - 1}}\right)
$$
By Lemma \ref{lemma-flat-covariance-trace-relative-curve} this is equal to
$$
\text{Tr}_{d - 1, Y, n}
\circ
R^{2d - 2}q_!\left(\text{Tr}_{G, \mathit{flat}, n} \otimes
\text{id}_{\mu_n^{\otimes d - 1}}\right)
$$
and this finishes the proof.
\end{proof}

\begin{proposition}
\label{proposition-flat-trace-map}
Let $Y$ be a quasi-compact and quasi-separated scheme.
Let $n \geq 1$ be invertible on $Y$. Let $d \geq 0$.
Let $f : X \to Y$ be a flat, separated morphism of finite presentation
of relative dimension $\leq d$. There exists a unique map
$$
\text{Tr}_{f, \mathit{flat}, n} :
R^{2d}f_!\mu_n^{\otimes d}
\longrightarrow
...
\begin{enumerate}
\item formation of $\text{Tr}_{f, \mathit{flat}, n}$ is compatible with
base change by morphisms $Y' \to Y$ with
$Y'$ quasi-compact and quasi-separated, and
\item given a flat, quasi-finite morphism $g : X' \to X$ of
finite presentation, setting $f' = f \circ g$, we have
$\text{Tr}_{f', \mathit{flat}, n} =
\text{Tr}_{f, \mathit{flat}, n} \circ \text{Tr}_g$ where
$\text{Tr}_g$ is the ``flat'' covariance map
of Remark \ref{remark-flat-covariance}, and
\item if $X = \mathbf{A}^d_Y$ and $f$ is the structure morphism,
then $\text{Tr}_{f, \mathit{flat}, n}$ equals the map $\text{Tr}_{d, Y, n}$
constructed in Section \ref{section-trace-affine-space}.
\end{enumerate}
\end{proposition}

\begin{proof}
Let $W \subset X$ be the locus where $f$ is Cohen-Macaulay. By
More on Morphisms, Lemma
\ref{more-morphisms-lemma-flat-finite-presentation-CM-open}
this is a quasi-compact open dense in every fibre.
If $g : X' \to X$ is as in (2) and $W' \subset X'$ is the
locus where $f'$ is Cohen-Macaulay, then
$W' = g^{-1}(W)$ by
More on Morphisms, Lemma \ref{more-morphisms-lemma-composition-CM}
(combined with the fact that a quasi-finite flat morphism
is Cohen-Macaulay, see More on Morphisms, Lemma
\ref{more-morphisms-lemma-quasi-finite-flat-CM}).
Thus the covariance map for $W \subset X$ induces
an isomorphism
$R^{2d}(f|_W)_!\mu_n^{\otimes d} \to R^{2d}f_!\mu_n^{\otimes d}$
by Lemma \ref{lemma-top-restrict-to-open}
and this is compatible with base change and with ``flat'' covariance
along morphisms $g$ as in part (2) of the statement of the lemma.
It follows that it suffices to prove the existence and uniqueness
of the maps for Cohen-Macaulay morphisms.

\medskip\noindent
If $f$ is a Cohen-Macaulay morphism, then we can
write $X = X_d \amalg X_{<d}$ as a disjoint union of schemes
where $X_d \to Y$ has relative dimension $d$ and $X_{< d} \to Y$
has relative dimension $< d$, see 
Morphisms, Lemma
\ref{morphisms-lemma-flat-finite-presentation-CM-fibres-relative-dimension}.
If $g : X' \to X$ is as in (2), then $g^{-1}(X_d)$ is the open of $X'$
where $f'$ has relative dimension $d$.
As in the previous paragraph, we reduce to proving the existence
and uniqueness of the maps for Cohen-Macaulay morphisms of
relative dimension $d$.

\medskip\noindent
Assume $f$ is Cohen-Macaulay of relative dimension $d$.
Choose a finite open covering $X = U_1 \cup \ldots \cup U_r$
such that each $U_i$ admits a quasi-finite morphism
$g_i : U_i \to \mathbf{A}^d_Y$, see Lemma \ref{lemma-good-covering}.
Set $f_i = f|_{U_i}$ and $f_{ii'} = f|_{U_{ii'}}$ where
$U_{ii'} = U_i \cap U_{i'}$.
By conditions (2) and (3) we are forced to let
$\text{Tr}_{f_i, \mathit{flat}, n}$
be the map (\ref{equation-trace-by-qf})
constructed using $g_i$.
Then we have two quasi-finite maps
$g_i|_{U_{ii'}}, g_{i'}|_{U_{ii'}} : U_{ii'} \to \mathbf{A}^d_Y$
but by Lemma \ref{lemma-switch-coordinates} the map
(\ref{equation-trace-by-qf})
constructed using either of these produces the same
$\text{Tr}_{f_{ii'}, \mathit{flat}, n}$. It follows that the diagrams
$$
\xymatrix{
R^{2d}f_{ii', !}\mu_n^{\otimes d} \ar[r] \ar[d]
\ar[rd]^{\text{Tr}_{f_{ii'}, \mathit{flat}, n}} &
R^{2d}f_{i', !}\mu_n^{\otimes d}
\ar[d]^{\text{Tr}_{f_{i'}, \mathit{flat}, n}} \\
R^{2d}f_{i, !}\mu_n^{\otimes d} \ar[r]^{\text{Tr}_{f_i, \mathit{flat}, n}} &
\mathbf{Z}/n\mathbf{Z}
}
$$
commute (this uses that ``flat'' covariance is compatible with compositions
of flat quasi-finite morphisms). Thus we can use the canonical exact
sequence of Lemma \ref{lemma-covering-right-exact}
to get a unique map $\text{Tr}_{f, \mathit{flat}, n}$
such that the diagrams
$$
\xymatrix{
R^{2d}f_{i, !}\mu_n^{\otimes d} \ar[rr]
\ar[rd]_{\text{Tr}_{f_i, \mathit{flat}, n}} & &
R^{2d}f_!\mu_n^{\otimes d}
\ar[ld]^{\text{Tr}_{f, \mathit{flat}, n}} \\
&
\mathbf{Z}/n\mathbf{Z}
}
$$
commute. Moreover, this choice of $\text{Tr}_{f, \mathit{flat}, n}$
is forced.

\medskip\noindent
The only remaining issue is to prove (2) for 
flat, quasi-finite morphisms $g : X' \to X$ when both $f$ and $f'$
are Cohen-Macaulay of relative dimension $d$.
Let $X = U_1 \cup \ldots \cup U_r$ and $g_i : U_i \to \mathbf{A}^d_Y$
be the data used to construct $\text{Tr}_{f, \mathit{flat}, n}$
and let
$X' = U'_1 \cup \ldots \cup U'_{r'}$ amd $g'_i : U'_i \to \mathbf{A}^d_Y$
be the data used to construct $\text{Tr}_{f', \mathit{flat}, n}$.
It is easy to see that we may refine the open covering
of $X'$ without changing the resulting arrow
$\text{Tr}_{f', \mathit{flat}, n}$.
Thus we may assume that for every $i'$ there is an $i(i')$ such that
$g(U'_{i'}) \subset U_{i(i')}$. It follows that we have
commutative diagrams
$$
\xymatrix{
R^{2d}f'_{i', !}\mu_n^{\otimes d} \ar[r]  \ar[d] &
R^{2d}f_{i(i'), !}\mu_n^{\otimes d} \ar[d] \\
R^{2d}f'_!\mu_n^{\otimes d} \ar[r] &
R^{2d}f_!\mu_n^{\otimes d}
}
$$
by the compatibility of flat covariance with composition.
Here the top arrow is flat covariance with respect to the restriction
$U'_{i'} \to U_{i(i')}$ of $g$. Since we have
Lemma \ref{lemma-switch-coordinates}
we know that the top horizontal arrow composed with
$\text{Tr}_{f_{i(i')}, \mathit{flat}, n}$ is equal to
$\text{Tr}_{f'_{i'}, \mathit{flat}, n}$.
...
\begin{lemma}
\label{lemma-composition-flat-trace}
Let $Z$ be a quasi-compact and quasi-separated scheme.
Let $g : Y \to Z$ be a separated flat morphism of finite presentation
and of relative dimension $\leq e$.
Let $f : X \to Y$ be a separated flat morphism of finite presentation
and of relative dimension $\leq d$.
Then the composition
\begin{align*}
R^{2e + 2d}(g \circ f)_!\mu_n^{\otimes d + e}
& =
R^{2e}g_!\left(R^{2d}f_!\mu_n^{\otimes d} \otimes \mu_n^{\otimes e}\right) \\
& \xrightarrow{R^{2e}g_!(\text{Tr}_{f, \mathit{flat}, n} \otimes \text{id})}
R^{2e}g_!\mu_n^{\otimes e} \\
& \xrightarrow{\text{Tr}_{g, \mathit{flat}, n}}
\mathbf{Z}/n\mathbf{Z}
\end{align*}
is equal to $\text{Tr}_{g \circ f, \mathit{flat}, n}$.
\end{lemma}
...
\section{The trace map in the equidimensional case}
\label{section-construction-equidimensional}
...
\section{Construction of the trace map in general}
\label{section-construction-general}

\noindent
Let $f : X \to Y$ be a separated morphism of finite presentation
with $Y$ quasi-compact and quasi-separated. Let $r \geq 0$
and $\alpha \in z(X/Y, r)$. If $\alpha$ is not equidimensional, then
we use a descent argument to define the trace map
$\text{Tr}_{f, \alpha, \Lambda} : Rf_!\Lambda(r)[2r] \to \Lambda$.

\medskip\noindent
First we reduce to the case where $Y$ is Noetherian. Namely, by
our definition of $z(X/Y, r)$ in
Relative Cycles, Definition
\ref{relative-cycles-definition-relative-cycles-general}
...
$$
where $X' \to Y'$ is a finite type morphism of Noetherian schemes
and $\alpha' \in z(X'/S', r)$ such that $\alpha$ is the base change
of $\alpha'$ by $Y \to Y'$. Moreover, by Relative Cycles, Lemma
\ref{relative-cycles-lemma-relative-r-cycle-general-bis}
we may assume $f'$ is separated.
Below we construct the map $\text{Tr}_{f', \alpha', \Lambda}$
and we obtain $\text{Tr}_{f, \alpha, \Lambda}$ by base change.

\medskip\noindent
Assume $Y$ is Noetherian.
Choose a compactification $j : X \to \overline{X}$ over $Y$.
Choose a proper, completely decomposed morphism
$b_0 : Y_0 \to Y$ such that the pullback $\alpha_0 = a_0^*\alpha$
is equidimensional, see
Relative Cycles, Lemma \ref{relative-cycles-lemma-get-equidimensional-cycles}.

\medskip\noindent
For $m \geq 1$ denote $Y_m$ the $(m + 1)$-fold fibre product of $Y_0$ over $Y$.
We obtain a simplicial scheme $Y_\bullet$ which comes with an augmentation
$b : Y_\bullet \to Y$. This is a proper hypercovering, see
Simplicial Spaces, Section
\ref{spaces-simplicial-section-proper-hypercovering-spaces}
for the definition and compare with
Hypercoverings, Example \ref{hypercovering-example-cech}.
Denote $X_m$, $\overline{X}_m$, and $\alpha_m$ the pullback of
$X$, $\overline{X}$, and $\alpha$ to $Y_m$. Let $W_m \subset X_m$
be the closure of the support of $\alpha_m$ (viewed as a reduced
closed subscheme of $X_m$). Since $\alpha_0$ and hence $\alpha_m$
is equidimensional, the fibres of $W_m \to Y_m$ all have dimension $\leq r$.
Denote $\overline{W}_m$ the scheme theoretic closure of $W_m$ in
$\overline{X}_m$. The schemes $X_m$, resp.\ $\overline{X}_m$
form a proper hypercovering of $X$, resp.\ $\overline{X}$ (by base change).
Denote $j_\bullet : X_\bullet \to \overline{X}_\bullet$ the inclusion
morphism. Moreover, the $W_m$, resp.\ $\overline{W}_m$ form a simplicial
scheme which comes with a closed immersion
$i_\bullet : W_\bullet \to X_\bullet$,
resp.\ $\overline{i}_\bullet : \overline{W}_\bullet \to \overline{X}_\bullet$.
Namely, since taking the support of a cycle is compatible with base
change (Relative Cycles, Lemma \ref{relative-cycles-lemma-support-family}),
and since given any morphism $\varphi : [n] \to [m]$ in $\Delta$ the diagram
$$
\xymatrix{
X_m \ar[r]_{X_\varphi} \ar[d]_{f_m} & X_n \ar[d]^{f_n} \\
Y_m \ar[r]^{Y_\varphi} & Y_n
}
$$
is cartesian, it follows that the top horizontal morphism sends
$W_m$ into $W_n$ and consequently the morphism $\overline{X}_\varphi$ sends
$\overline{W}_m$ into $\overline{W}_n$. Denote
$j'_\bullet : W_\bullet \to \overline{W}_\bullet$ the inclusion morphism.

\medskip\noindent
By the equivalence of Simplicial Spaces, Lemma
\ref{spaces-simplicial-lemma-ph-hypercovering-equivalence-bounded}
to define our map $\text{Tr}_\alpha : Rf_!\Lambda(r)[2r] \to \Lambda$,
it suffices to define a map
$$
b^{-1}Rf_!\Lambda(r) \longrightarrow \Lambda[-2r]
$$
in the derived category of sheaves of $\Lambda$-modules on the \'etale
site of $Y_\bullet$. Now we have
$$
b^{-1}Rf_!\Lambda(r) =
b^{-1}R\overline{f}_*j_!\Lambda(r) =
R\overline{f}_{\bullet, *}\overline{a}^{-1}j_!\Lambda(r) =
R\overline{f}_{\bullet, *}j_{\bullet, !}\Lambda(r) 
$$
Here the first equality is the definition of $Rf_!$. The second
equality follows from the cartesian diagram
$$
\xymatrix{
\overline{X}_\bullet \ar[d]_{\overline{f}_\bullet} \ar[r]_{\overline{a}} &
X \ar[d]^f \\
Y_\bullet \ar[r]^a & Y
}
$$
and the fact that the base change map is an isomorphism in this case
(details omitted; use that we may check on $Y_m$ for each $m$ to reduce
to the usual proper base change theorem in \'etale cohomology).
Consider the commutative diagram
$$
\xymatrix{
W_\bullet \ar[r]_{i_\bullet} \ar[d]_{j'_\bullet} &
X_\bullet \ar[r]_a \ar[d]_{j_\bullet} & X \ar[d]^j \\
\overline{W}_\bullet \ar[r]^{\overline{i}_\bullet} &
\overline{X}_\bullet \ar[r]^{\overline{a}} & \overline{X}
}
$$
with cartesian squares. The third equality follows by looking
at the right square and using that
$\bar{a}^{-1} \circ j_! = j_{\bullet, !} \circ a^{-1}$
on abelian sheaves. By the same token, looking at the left square,
we have $\overline{i}_\bullet^{-1} j_\bullet\Lambda =
j'_{\bullet, !}\Lambda$ as sheaves on $\overline{W}_\bullet$.
We use this to produce a canonical map
$$
j_{\bullet, !}\Lambda(r) \longrightarrow
\overline{i}_{\bullet, *}j'_{\bullet, !}\Lambda(r)
$$
Applying the functor $R\overline{f}_{\bullet, *}$ we obtain a map
$$
R\overline{f}_{\bullet, *}j_{\bullet, !}\Lambda(r) 
\longrightarrow
R\overline{f}_{\bullet, *}\overline{i}_{\bullet, *}j'_{\bullet, !}\Lambda(r)
=
R\overline{g}_{\bullet, *}j'_{\bullet, !}\Lambda(r)
$$
where $\overline{g}_\bullet : \overline{W}_\bullet \to Y_\bullet$
is the structure morphism. In conclusion, it suffices to define
$$
R\overline{g}_{\bullet, *}j'_{\bullet, !}\Lambda(r)
\longrightarrow
\Lambda[-2r]
$$
Now the left hand side is an object of $D^+(Y_\bullet, \Lambda)$
with nonzero cohomology sheaves only in degrees $0, \ldots, 2r$
because we arranged it so that the fibres of $W_n$ over $Y_n$
have dimension $\leq r$. Hence it suffices to define maps
$$
R^{2r}g_{m, !}\Lambda(r) \to \Lambda
$$
over $Y_m$ compatible with the morphisms in the simplicial scheme.
This was done in the earlier sections.
