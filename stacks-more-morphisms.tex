...
$$
\beta = (\gamma \star \id_j) \circ (\alpha \star \text{id}_f)
$$
...
Let $y' = x' \circ f'$ and $\gamma = \id : x' \circ f' \to y'$.
Set
$$
\beta = \alpha \star \id_f :
...
and $\gamma_k = \gamma \star \id_{\delta_k}$ (more succinctly
...
$p' \circ \delta' = \id_{U'}$. To be explicit,
$(\id_U, q' \circ \delta', (\delta')^*\gamma) : \xi \to \xi$
...
automorphism is $(\id_V, h', (\delta')^*\gamma)$.
Here $h' : V' \to V'$ is the unique (iso)morphism such that
$h'|_V = \id_V$ and such that
$$
\xymatrix{
...
$p' \circ \delta'_i = \id_{U'}$. Then the composition
in $\mathit{Aut}(\xi)$ is
$$
...
such that
$\gamma = (\id_f \star \alpha) \circ (\beta \star \text{id}_i)$,
in other words such that
$$
\xymatrix{
& f \circ x' \circ i \ar[rd]^{\id_f \star \alpha} \\
y \circ i \ar[ru]^{\beta \star \id_i} \ar[rr]^\gamma & &
f \circ x
}
$$
...
$\alpha_1 = \alpha_2 \circ (\theta \star \id_i)$ and
$\beta_2 = (\id_f \star \theta) \circ \beta_1$.
...
$Z = W$, $\overline{g} = \id_Y$, $g = \text{id}_W$,
$h = x$.
\end{proof}
...
Observe that $g_i = h \circ g'_i$ is
