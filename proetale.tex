...
$u \circ v = \id$ by construction, see
...
to $\id_A : A \to A$ we find a faithfully flat, ind-\'etale ring map
...
$$
such that $\pi_S \circ i_S = \id$. Moreover, $i_S = i_{\text{id}_S}$
with $i_{\id_S}$ as in Lemma \ref{lemma-put-in-T}. In particular the
...
\item $\id \to i^{-1}i^{Sh}_!$, $\text{id} \to i^{-1}i_!$, and
$i^{-1}i_* \to \id$ are isomorphisms, and
...
(Sites, Lemma \ref{sites-lemma-sheaf-coequalizer-representable})
we conclude $\id \to i^{-1}i^{Sh}_!$ is an isomorphism.
Similarly for the map $\id \to i^{-1}i_!$. Then
$(i^{-1}i_*\mathcal{H})(V) = i_*\mathcal{H}(v(V)) =
\mathcal{H}(u(v(V))) = \mathcal{H}(V)$ and we find that
$i^{-1}i_* \to \id$ is an isomorphism.
...
\item $i_\proetale^{-1}i_{\proetale, *} \cong \id_{\Sh(Z_\proetale)}$.
\end{enumerate}
\end{lemma}
...
Then $\id \cong j^{-1}j_!$ and $j^{-1}j_* \cong \text{id}$
