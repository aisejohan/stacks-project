...
\end{reference}
Let $X$ be a scheme. An {\it envelope} is a morphism $f : Y \to X$
of schemes which is proper and completely decomposed
...
Let $X' \to X$ be a morphism of schemes. If $f : Y \to X$ is an envelope,
then the base change $f' : Y' \to X'$ of $f$ is an envelope too.
\end{lemma}
...
\begin{lemma}
\label{lemma-category-envelopes}
Let $X$ be a scheme. Any two envelopes of $X$ are dominated by a third.
\end{lemma}

\begin{proof}
If $p : Y \to X$ and $q : Z \to X$ are envelopes, then
so is $r : Y \times_X Z \to X$. Namely, $r$ is the composition
of a base change of $p$ and $q$, whence is an envelope
by Lemmas \ref{lemma-base-change-envelope} and
\ref{lemma-composition-envelope}.
\end{proof}
