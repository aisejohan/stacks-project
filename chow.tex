...
\begin{proof}
The cases $i = 0, n$ are trivial as $\pi^0 = \id_M$ by convention.
...
\medskip\noindent
As a first step we may replace $A$ by the ring constructed in
Lemma \ref{lemma-Noetherian-domain-dim-1-two-elements}
for $a_1, a_2 \in A$. Thus, after possibly switching $a_1$
and $a_2$, we may and do assume that $a_2 \in (a_1)$.
...
$$
we get a welldefined map $Q(A)^* \times Q(A)^* \to \kappa(\mathfrak m)^*$
...
Then $p \circ o = \id_X$ and $o(X)$ is an effective
Cartier divisor on $L$. Hence by Lemma \ref{lemma-relative-effective-cartier}
we see that $o^* \circ p^* = \id$ and we conclude that $p^*$ is
injective too.
\end{proof}
...
the bivariant cohomology classes for $\id_X$. Namely, that is where
...
$\det\nolimits_\kappa(u_M) = f_M \id_{\det\nolimits_\kappa(M)}$.
