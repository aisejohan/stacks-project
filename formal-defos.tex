...
morphisms are those of $\mathcal{F}$ lying over $\id_U$.
...
$p'(t_x) = \id_{p(x)}$ for all $x \in \Ob(\mathcal{F})$.
\item
...
over $\id: B \to B$, such that the diagram
$$
\xymatrix{
...
$\varphi(x_n) \to y$ lying over  $\id : B_n \to B_n$ so that the diagram
$$
\xymatrix{
...
By construction $f \circ s = \id$. Then $s$ is injective.
...
Then there exists a morphism $s : x \to y$ with $a \circ s = \id_x$
...
$$
\sigma = (\id_A, p(s_\epsilon)) : A \to A \times_k k[\epsilon].
$$
...
$\sigma_*x \to x$ such that $x \to \sigma_*x \to x$ is $\id_x$.
...
\begin{proof}
Note that
$\id_{A_1} \times g : A_1 \times_A A_2 \to A_1 \times_k k[I]$
...
$\id : A_1 \times_A A_1 \to A_1 \times_A A_1$, hence it is an
isomorphism.
\end{proof}
...
\xymatrix{
\text{Lift}(x, A') \times (T\mathcal{F} \otimes_k I)
\ar[d]_{(\varphi, d\varphi \otimes \id_I)} \ar[r] &
...
$\mathcal{F}(k)$.
\begin{enumerate}
\item $T_{\id_{x_0}} \mathit{Aut}(x)$ has a natural $k$-vector
space structure such that addition agrees with composition in
$T_{\id_{x_0}} \mathit{Aut}(x)$. In particular, composition in
$T_{\id_{x_0}} \mathit{Aut}(x)$ is commutative.
\item There is a canonical isomorphism
$T_{\id_{x_0}} \mathit{Aut}(x) \to
T_{\id_{x_0}} \mathit{Aut}(x_0)$
of $k$-vector spaces.
\end{enumerate}
\end{lemma}
...
and the element $\id_{x_0} \in \mathit{Aut}(x)(k)$ to get
a predeformation functor $F = \mathit{Aut}(x)_{\id_{x_0}}$. By
Lemmas \ref{lemma-Aut-functor-RS} and \ref{lemma-localize-RS}
$F$ is a deformation functor. By definition
$T_{\id_{x_0}} \mathit{Aut}(x) = TF = F(k[\epsilon])$
which has a natural
...
is a homomorphism where $F(A) = \mathit{Aut}(x)_{\id_{x_0}}(A)$ and
$F(B) = \mathit{Aut}(x)_{\id_{x_0}}(B)$ are groups under
...
group under composition. With $\id \in F(k[\epsilon])$ the
unit element we see that $+(v, \id) =
+(\id, v) = v$ for any $v \in F(k[\epsilon])$ because
$(\id, v)$ is the pushforward of $v$ along the ring map
...
$$
\text{Inf}(x'/x) = \text{Lift}(\id_x, A')
$$
where the liftings are of $\id_x$ as an object of
...
$T_{\id_{x_0}} \mathit{Aut}(x_0)$, and so has a natural $k$-vector
...
$T_{\id_{x_0}} \mathit{Aut}(x') \otimes_k I
...
of $TF \otimes_k I$ on $\text{Lift}(\id_x, A')$, because as
$\text{Lift}(\id_x, A')$ is a group it is always nonempty.
Note that we have equalities of vector spaces
$$
TF = T_{\id_{x_0}} \mathit{Aut}(x') \otimes_k I =
\text{Inf}_{x_0}(\mathcal{F}) \otimes_k I
$$
by
Lemma \ref{lemma-Aut-functor-tangent-space}.
The equality $\text{Inf}(x'/x) = \text{Lift}(\id_x, A')$ of
Remark \ref{remark-infaut-lifting-equalities}
finishes the proof.
\end{proof}
...
$(f, \id_{U(f)(x)}): (T, x) \to (T, U(f)(x))$ lying over
$f : T \to T'$.
\end{enumerate}
\end{enumerate}
\end{definition}
...
But since $h \bmod (t_1, \ldots, t_r) = \id_R$ by
