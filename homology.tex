...
determined by the rules $p \circ i = \id_x$,
$q \circ j = \id_y$, $p \circ j = 0$, $q \circ i = 0$.
...
$q : z \to y$ such that $p \circ i = \id_x$,
$q \circ j = \id_y$, $p \circ j = 0$, $q \circ i = 0$
and $i \circ p + j \circ q = \id_z$, then $z$
...
$q : z \to y$ such that $p \circ i = \id_x$,
$q \circ j = \id_y$, $p \circ j = 0$, $q \circ i = 0$
and $i \circ p + j \circ q = \id_z$, according
...
\right.
$$
The map $f : V \to W$ corresponding to $\id_k$ on the underlying
...
$i$ is a monomorphism this implies $u\circ e=\id_{\Ker(f)}$.
Furthermore, we have $j\circ e\circ u=g\circ i\circ u=g\circ t=j$. 
Since $j$ is a monomorphism this implies $e\circ u=\id_{\Ker(k)}$.
...
However, since $ba \cong \id$ we see that every morphism of
...
= 0$. Then $\overline{F}(\id_X) = \text{id}_{\overline{F}(X)} = 0$, thus
$\id_X = 0$ in $\mathcal{A}/\mathcal{C}$ by faithfulness of
...
$a \circ b$ and $\id_A$
and there exists a homotopy between $b \circ a$ and $\id_B$.
...
$\id_{v(P)}$ in $\Hom_\mathcal{A}(v(P), v(P))$. Functoriality
