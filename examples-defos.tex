...
$\gamma \bmod \epsilon = \id$. Then we can write
$\gamma = \id_M + \epsilon \psi$ where
...
and a homomorphism $\rho : \Gamma \to \GL_A(M)$, and
...
\begin{matrix}
\GL_n(A)\text{-conjugacy classes of}\\
\text{homomorphisms }\rho : \Gamma \to \GL_n(A)
\end{matrix}
$$
...
Consider an object
$$
((A_1, M_1, \rho_1), (A_2, M_2, \rho_2), (\id_A, \varphi))
$$
...
\GL_{A_1 \times_A A_2}(M_1 \times_\varphi M_2)
$$
...
\item an object is a triple $(A, M, \rho)$ consisting of an
object $A$ of $\mathcal{C}_\Lambda$, a finite projective $A$-module $M$,
and a continuous homomorphism $\rho : \Gamma \to \GL_A(M)$
where $\GL_A(M)$ is given the discrete topology\footnote{An alternative
...
continuous representation $\rho_0 : \Gamma \to \GL_k(V)$,
let $x_0 = (k, V, \rho_0)$ be the corresponding object of $\mathcal{F}(k)$.
We set
$$
\Deformationcategory_{V, \rho_0} = \mathcal{F}_{x_0}
...
\begin{matrix}
\GL_n(A)\text{-conjugacy classes of}\\
\text{continuous homomorphisms }\rho : \Gamma \to \GL_n(A)
\end{matrix}
$$
...
$\rho_0 : \Gamma \to \GL_k(V)$ be a finite dimensional
...
$\gamma = \id + \epsilon \delta$ as above.
Thus we see that
$$
\text{Inf}(\Deformationcategory_P) = \text{Der}_k(P, P)
$$
