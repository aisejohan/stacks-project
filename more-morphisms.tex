...
\begin{lemma}
\label{lemma-quasi-finite-flat-CM}
Let $f : X \to Y$ be a locally quasi-finite morphism of schemes.
If $f$ is flat at $x \in X$, then $f$ is Cohen-Macaulay at $x$.
If $f$ is flat, then $f$ is Cohen-Macaulay.
\end{lemma}

\begin{proof}
Let $y = f(x)$. Then $\mathcal{O}_{X_y, x}$
is a zero dimensional Noetherian local ring
and hence Cohen-Macaulay by definition.
\end{proof}
...
\item $W$ is (a) open in $X$ and (b) retrocompact in $X$,
...
by definition. Parts (2) (a) and (3) follow from
...
\medskip\noindent
Proof of (2)(b). It is enough to show that $W \cap U$ is
quasi-compact for any affine open of $X$ which maps into
and affine open of $S$ (since $X$ is covered by such $U$).
Thus reduce to the affine case. Say $X \to S$ corresponds to
the flat and finitely presented ring map $A \to B$.
By Algebra, Lemma \ref{algebra-lemma-flat-finite-presentation-limit-flat}
part (1) there exists a flat finite type ring homorphism $A_0 \to B_0$
of Noetherian rings and a ring map $A_0 \to A$ such that
$B = B_0 \otimes_{A_0} A$. Then the open $W_0 \subset X_0 = \Spec(B_0)$
of the lemma for $X_0 \to S_0 = \Spec(A_0)$
is quasi-compact (as $X_0$ is a Noetherian topological space).
By (4) the open $W$ is the pullback of $W_0$ by the quasi-compact
map $X \to X_0$. Thus $W$ is quasi-compact.
\end{proof}
...
\begin{lemma}
\label{lemma-construct-relative-divisors}
Let $f : X \to S$ be a flat morphism of finite presentation.
Let $x \in X$ with image $s \in S$. Let $D \subset X_s$ be an
effective Cartier divisor. There exists an open neighbourhood
$U \subset X$ of $x$ and a relative effective Cartier divisor
$E \subset U$ such that $E_s = D \cap U_s$ as effective Cartier
divisors on $U_s$.
\end{lemma}

\begin{proof}
Let $V \subset X_s$ be an affine open neighbourhood of $x$
such that $D \cap V$ is the zero scheme of a nonzerodivisor
$h' \in \Gamma(V, \mathcal{O}_V)$, see
Divisors, Lemma \ref{divisors-lemma-characterize-effective-Cartier-divisor}.
Let $\overline{h} \in \mathcal{O}_{X_s, x}$ be the image of $h'$.
Since $\mathcal{O}_{X_s, x}$ is a quotient of $\mathcal{O}_{X, x}$
(Schemes, Lemma \ref{schemes-lemma-local-ring-fibre})
we may choose an element $h \in \mathcal{O}_{X, x}$ lifting $\overline{h}$.
By Lemma \ref{lemma-slice-given-element} we see that
$h$ defines an effective Cartier divisor $E$ on an open $U$
with $E \to S$ flat and locally of finite presentation.
Thus $E$ is a relative effective Cartier divisor by
Divisors, Definition
\ref{divisors-definition-relative-effective-Cartier-divisor}.
Finally, after shrinking $U$ we may assume that $U_s \subset V$
and that $h'|_{U_s}$ and $h|_{U_s}$ agree; this is true because
the topology on $X_s$ is induced from $X$, see
Schemes, Lemma \ref{schemes-lemma-fibre-topological}.
Thus we'll have $E_s = D \cap U_s$ as desired.
\end{proof}
...
Let $f : X \to Y$ be a locally quasi-finite morphism of schemes.
Consider the function $w : X \to \mathbf{Z}_{> 0}$ sending
$x \in X$ with image $y \in Y$ to
$$
w(x) =
...
(Fields, Definition \ref{fields-definition-insep-degree}). Then
\begin{enumerate}
\item formation of this function is compatible with base change, and
\item if $f$ is flat and locally of finite presentation, then
$w$ is a weighting of $f$.
\end{enumerate}
\end{lemma}

\begin{proof}
Let $x \in X$ map to $y \in Y$. Then
$\mathcal{O}_{X, x}/\mathfrak m_y\mathcal{O}_{X, x} = \mathcal{O}_{X_y, x}$
is a finite dimensional $\kappa(y)$-algebra.
Suppose that $Y' \to Y$ is a morphism of schemes and $y' \in Y'$
maps to $y$ and $x' \in X' = X \times_Y Y'$ is a point mapping to
$x$ and $y'$. Then $x'$ corresponds to a prime ideal
$$
\mathfrak q \subset \mathcal{O}_{X_y, x} \otimes_{\kappa(y)} \kappa(y')
$$
and
$$
\mathcal{O}_{X'_{y'}, x'} =
(\mathcal{O}_{X_y, x} \otimes_{\kappa(y)} \kappa(y'))_{\mathfrak q}
$$
Thus the function $w' : X' \to \mathbf{Z}_{> 0}$ constructed in the
same manner is the pullback of $w$ by
Varieties, Lemma \ref{varieties-lemma-base-change-and-length}.
This proves (1).

\medskip\noindent
Since the construction of $w$ is compatible with base change,
in order to prove (2) it suffices to show: if $f$ is a finite,
flat morphism of finite presentation, then $\int_f w$
is locally constant on $Y$. In fact, $f$ is finite locally free
(Morphisms, Lemma \ref{morphisms-lemma-finite-flat})
...
\begin{lemma}
\label{lemma-weighting-composition-flat-quasi-finite}
Let $f : X \to Y$ and $g : Y \to Z$ be flat, locally quasi-finite,
and locally of finite presentation. Let $w_f : X \to \mathbf{Z}$
be the weighting of $f$ constructed in
Lemma \ref{lemma-weighting-flat-quasi-finite} and let
$w_g : Y \to \mathbf{Z}$ be a weighting of $g$ constructed in
Lemma \ref{lemma-weighting-flat-quasi-finite}.
Then $g \circ f$ is flat, locally quasi-finite, and
locally of finite presentation and the function
$$
X \longrightarrow \mathbf{Z},\quad
x \longmapsto w_f(x) w_g(f(x))
$$
is the weighting of $g \circ f$ constructed in
Lemma \ref{lemma-weighting-flat-quasi-finite}.
\end{lemma}

\begin{proof}
Let $x \in X$ have images $y \in Y$ and $z \in Z$. We have to show that
$$
\text{length}_{\mathcal{O}_{X, x}}
(\mathcal{O}_{X, x}/\mathfrak m_z \mathcal{O}_{X, x})
[\kappa(x) : \kappa(z)]_i
$$
...
[\kappa(x) : \kappa(y)]_i
\cdot
\text{length}_{\mathcal{O}_{Y, y}}
(\mathcal{O}_{Y, y}/\mathfrak m_z \mathcal{O}_{Y, y})
[\kappa(y) : \kappa(z)]_i
$$
This follows from the multiplicativity of inseparable degrees
(Fields, Lemma \ref{fields-lemma-multiplicativity-all-degrees})
and
Algebra, Lemma \ref{algebra-lemma-pullback-module}
applied to the flat ring map
$A = \mathcal{O}_{Y, y} \to \mathcal{O}_{X, x} = B$
and the $A$-module
$M = \mathcal{O}_{Y, y}/\mathfrak m_z \mathcal{O}_{Y, y}$.
\end{proof}
