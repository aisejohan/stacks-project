...
\end{enumerate}
These data have to satisfy the condition that $\rho^U_U = \id$
...
the identity morphisms $\id_U : U \to U$ are the identity.
...
$$
commutes where $\varphi_{01} = \varphi \otimes \id_B$ and similarly
...
$\{\id : \Spec(R) \to \Spec(R)\}$. We will see below that
...
\prod_{j = 1, \ldots, n} \mathcal{O}_X \ar[d]^{\id}\\
f_{small}^{-1}h_V \ar[r]^-{f^{-1}h_i} &
\prod_{j = 1, \ldots, n} f_{small}^{-1}\mathcal{O}_Y
\ar[r]^-{\prod f^\sharp} &
\prod_{j = 1, \ldots, n} \mathcal{O}_X
...
\ar[rr]^{t_i \star \id_{j_{i, i', small}}}
\ar@{=}[d] & &
f_{small} \circ j_{i, small} \circ j_{i, i', small} \ar@{=}[d] \\
g \circ j_{i', small} \circ j_{i', i, small}
\ar[rr]^{t_{i'} \star \id_{j_{i', i, small}}} & &
...
$$
1 \to (K^{sep})^* \to \GL_d(K^{sep}) \to \text{PGL}_d(K^{sep}) \to 1,
$$
...
\in (K^{sep})^*
\end{equation}
where $\tilde g_\sigma \in \GL_d(K^{sep})$ is a lift of $g_\sigma$.
...
$$
H^1(\text{Gal}(K^{sep}/K), \GL_d(K^{sep})) =
H_\etale^1(\Spec(K), \GL_d(\mathcal{O}))
$$
...
adjunction map $\id \to f_* f^{-1}$ is called {\it restriction}.
The adjunction map $f_! f^{-1} \to \id$ is often
called the {\it trace map}. If $f$ is finite, then $f_* = f_!$ and
we can view this as a map $f_*f^{-1} \to \id$.

\begin{definition}
\label{definition-trace-map}
Let $f : Y \to X$ be a finite \'etale morphism of schemes.
The map $f_* f^{-1} \to \id$ described above is called the {\it trace}.
\end{definition}
...
with $\pi_S \circ i_S = \id$ and $\pi_{S, *} = i_S^{-1}$.
...
\begin{proof}
The functor $a_X^{-1}$ is fully faithful (as $a_{X, *}a_X^{-1} = \id$ by
...
(\mathcal{G}_1, \mathcal{H}_1, \alpha_1)
$$
which uses the unit $\id \to f_{big, fppf, *}f_{big, fppf}^{-1}$
...
$$
\xymatrix{
X_{\bar k} \ar[rd] \ar[rr]^{\Spec(\sigma) \times \id_X} & &
X_{\bar k} \ar[ld] \\
& X
}
$$
...
k}})$
$$
\sigma \cdot \xi := (\Spec(\sigma) \times \id_X)^*\xi \in
H^j(X_{\bar k}, (\Spec(\sigma) \times \id_X)^{-1}
\mathcal{F}|{X_{\bar k}})
...
\GL_m(\mathbf{Z}/\ell^n\mathbf{Z}))$ (for some fixed choice of $\bar c$)
...
under $\varphi - \id_C$. Proofs of this theorem can be found in
...
An {\it unramified cusp form on $\GL_2(\mathbf{A})$ with values in
...
such that
\begin{enumerate}
\item $f(x\gamma) = f(x)$ for all $x\in \GL_2(\mathbf{A})$ and all
$\gamma\in \GL_2(K)$
\item $f(ux) = f(x)$ for all $x\in \GL_2(\mathbf{A})$ and all
$u\in \GL_2(O)$
...
complete theorems like this two above for $\GL_n$
and allowing ramification!
In other words, the full global Langlands correspondence for $\GL_n$
...
\pi_1(X)\to \GL_n(R_{\text{univ}})$ (Univ. Defo ring)
$R_{\text{univ}}$ loc.
...
$\rho : \pi_1(X)\to \GL_n(R)$ then there exists
