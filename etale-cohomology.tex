...
$$
at least if $n$ is invertible on $S$. The map
$$
c_1 = c^n_1 : \Pic(S) \longrightarrow H^2_\etale(S, \mu_{n, S})
$$
in the long exact sequence above is called the first chern class map.
If $m$ is also invertible on $S$, then the composition
$$
\Pic(S) \xrightarrow{c^{nm}_1}
H^2_\etale(S, \mu_{nm, S})
\xrightarrow{(\cdot)^m}
H^2_\etale(S, \mu_{n, S})
$$
is equal to $c^n_1$. Thus for $\ell$ a prime invertible on $S$
we get an $\ell$-adic chern class map
$\Pic(S) \to \lim H^2_\etale(S, \mu_{\ell^e})$.
Finally, given a morphism $f : S' \to S$ of schemes the diagram
$$
\xymatrix{
\Pic(S') \ar[r]_-{c_1} & H^2_\etale(S', \mu_{n, S'}) \\
\Pic(S) \ar[r]^-{c_1} \ar[u]^{f^*} &
H^2_\etale(S, \mu_{n, S}) \ar[u]_{f^{-1}}
}
$$
is commutative.

\medskip\noindent
When $n$ is not invertible on $S$ we can apply the following lemma.
...
\begin{enumerate}
\item If $\mathcal{F}$ is a torsion abelian sheaf on $X_\etale$, then
the map $H^q_\etale(X, \mathcal{F}) \to H^q_\etale(X_K, \mathcal{F}|_{X_K})$
is an isomorphism for $q \geq 0$.
\item If $E \in D^+(X_\etale)$ has torsion cohomology sheaves,
then the maps $H^q_\etale(X, E) \to H^q_\etale(X_K, E|_{X_K})$
are isomorphisms.
\item If $E \in D(X_\etale, \mathbf{Z}/n\mathbf{Z})$, then the 
maps $H^q_\etale(X, E) \to H^q_\etale(X_K, E|_{X_K})$
are isomorphisms.
\end{enumerate}
\end{lemma}

\begin{proof}
Looking at stalks we see that (1) is a special case of
Theorem \ref{theorem-proper-base-change}, (2) is a special case of
Lemma \ref{lemma-proper-base-change}, and (3) is a special case of
Lemma \ref{lemma-proper-base-change-mod-n}.
\end{proof}
