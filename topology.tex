...
Let $f : X \to Y$ be a continuous map of topological spaces.
Let $s : Y \to X$ be a continuous map such that $f \circ s = \id_Y$.
...
If $f$ is not $\id_X$, then there exists a proper closed subset
...
$f|_E : E \to E$. This map has to be $\id_E$ as otherwise
Lemma \ref{lemma-rainwater} shows that $E$ isn't minimal.
Thus the $\id_E$ factors through the extremally disconnected
