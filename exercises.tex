...
such that the operator $T - \lambda \id_V$ is not invertible.
\begin{enumerate}
...
if there exists a morphism $g : Y \to X$ such that $f \circ g = \id_Y$
and $g \circ f = \id_X$. An isomorphism in the category of sets
...
such that $\pi \circ \sigma = \id_S$.
\item Show there does exist a nonempty open $U \subset S$ and
a morphism $\sigma : U \to X$ such that $\pi \circ \sigma = \id_U$.
\end{enumerate}
\end{exercise}
