...
\tau \star \id_{\text{pr}_R} = \text{id}_g \star \tau'.
\end{equation}
...
$(\id_{f_2} \star \alpha) \circ \beta = \beta'$.
\end{enumerate}
...
and the morphism $\id : \mathcal{X} \to \mathcal{X}$ to
...
the base change $\id_\mathcal{Z} \times f$ has property $\mathcal{P}$.
\end{enumerate}
\end{lemma}
...
the map $\id_U : U \to U$ factors through $Z \to U$, and hence
