...
$[n] \circ [m] = [n + m]$  and $[0] = \id$ (equality as functors).
...
$[n] \circ [m] = [n + m]$ and $[0] = \id$ (equality as functors), and
...
\item the triple $(\id_X, g, a)$ is
a morphism of triangles
$(X, Y, Q_1, f, p_1, d_1) \to (X, Z, Q_2, g \circ f, p_2, d_2)$, and
\item the triple $(f, \id_Z, b)$ is a morphism of triangles
...
$(0, Z, Z, 0, \id_Z, 0)$ is distinguished by TR1 and TR2 and
...
\begin{proof}
Since $F$ is fully faithful the adjunction map $\id \to G \circ F$
...
such that $[n] \circ [m] = [n + m]$ and $[0] = \id$.
...
such that $\pi \circ s = \id_{L^\bullet}$ and such that
$s \circ \pi$ is homotopic to $\id_{\tilde L^\bullet}$.
\end{enumerate}
...
\right)
$$
so that $\pi \circ s = \id_{L^\bullet}$. Finally,
...
such that $s \circ i = \id_{K^\bullet}$ and such that
$i \circ s$ is homotopic to $\id_{\tilde K^\bullet}$.
\end{enumerate}
Moreover, if both $K^\bullet$ and $L^\bullet$ are in
$K^{+}(\mathcal{A})$, $K^{-}(\mathcal{A})$, or $K^b(\mathcal{A})$,
...
\right)
$$
so that $s \circ i = \id_{K^\bullet}$. Finally,
...
$\id_{A^\bullet}$ which is acyclic. Hence we see that
...
$$
(\id_{X^i}, s^i, 0) : X^i \oplus Y^i \oplus Y^{i + 1} \to X^i
$$
Calculation omitted.
\end{proof}
...
because $vuv \cong v$ by the assumption that $vu \cong \id$.
...
$\id_{F(X'')}$ of the colimit we find a unique morphism
...
$\id : I^\bullet \to I^\bullet$. Thus also the ind-object
$$
...
such that $s \circ inj = \id$.
\item For any functor
$inj : \text{Comp}^{+}(\mathcal{A}) \to \text{InjRes}(\mathcal{A})$
such that $s \circ inj = \id$ we obtain a resolution functor, see
Definition \ref{definition-localization-functor}.
\end{enumerate}
\end{lemma}
...
\medskip\noindent
Suppose we have a functor $inj$ such that $s \circ inj = \id$.
...
\begin{remark}
\label{remark-match}
Suppose $inj$ is a functor such that $s \circ inj = \id$
as in part (2) of
...
Z_0 \ar[r] & B \ar[r] & 0 \\
0 \ar[r] &
A \ar[r] \ar[u]^{\id} \ar[d]_{\text{id}} &
Z''_{i - 1} \ar[r] \ar[u] \ar[d] &
\ldots \ar[r] &
Z''_0 \ar[r] \ar[u] \ar[d] &
B \ar[r] \ar[u]_{\id} \ar[d]^{\text{id}} & 0 \\
...
$$
\xymatrix{
K_{n_i} \ar[r] \ar[d]_{\id} & \text{hocolim} K_{n_i} \ar[d] \\
...
$$
Then the reader verifies that $(1 - f) \circ h = \id - a \circ c$
and $h \circ (1 - f) = \id - b \circ d$. This means that
$\id - \psi \circ \varphi$ has square zero by
...
$\mathcal{D} = \langle E \rangle$ we conclude that $\id_K = 0$,
i.e., $K = 0$.
\end{proof}
