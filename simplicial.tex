...
$\sigma^{n - 1}_j \circ \delta^n_j = \id_{[n - 1]}$
and
$\sigma^{n - 1}_j \circ \delta^n_{j + 1} = \id_{[n - 1]}$.
In other words the diagram
$$
\xymatrix{
& [n] \ar[rd]^{\sigma^{n - 1}_j} & \\
...
\item We have $\id = d_j \circ s_j = d_{j + 1} \circ s_j$.
...
\begin{proof}
This is true because $d_i^{n + 1} \circ s^n_i = \id_{U_n}$.
\end{proof}
...
\item We have
$\id = \sigma_j \circ \delta_j = \sigma_j \circ \delta_{j + 1}$.
\item If $i > j + 1$, then
...
$\sigma_i^{n - 1} \circ \delta^n_i = \id_{U_n}$
for $j < n$.
\end{proof}
...
$$
\xymatrix{
X \ar[r]^{\gamma_\alpha} \ar[d]^{\id_X} & V_n \ar[d]^{V(\varphi)} \\
X \ar[r]^{\gamma_{\alpha \circ \varphi}} & V_m
}
$$
commute. Taking $\alpha = \id_{[k]}$, we see that
for any $\varphi : [m] \to [k]$ we have $\gamma_\varphi =
V(\varphi) \circ \gamma_{\id_{[k]}}$. Thus the morphism
$\gamma$ is determined by the value of $\gamma$ on the
component corresponding to $\id_{[k]}$. Conversely,
...
the projection onto the factor corresponding to $\id_{[k]}$.
...
Taking $\alpha' = \id_{[k]}$, we see that
for any $\varphi : [k] \to [n]$ we have $\gamma_\varphi =
\gamma_{\id_{[k]}} \circ V(\varphi)$. Thus the morphism
$\gamma$ is determined by the component of $\gamma_k$
corresponding to $\id_{[k]}$. Conversely,
...
$$
\xymatrix{
X \ar[r]^{f_u} \ar[d]_{\id_X} & V_n \ar[d]^{V(\varphi)} \\
...
\ar[rr]_{W(\varphi)\times \id \times \text{id}}
\ar[d]_{\id \times \text{id} \times \varphi} & &
...
as $f'_n = (f_n)_{n, \id}$, in other words, as
the restriction of
$(f_n)_n : W_n \times U_n \times (\Delta[n])_n \to V_n$
to $W_n \times U_n \times \id_{[n]}$.
...
\ar[r]_-{(f_n)_n}
\ar[d]_{\id \times U(\varphi) \times \varphi}
& V_n \ar[d]^{V(\varphi)} \\
...
equal to $V(\varphi) \circ (f_n)_{n, \id}$.
Altogether we obtain
$
V(\varphi) \circ (f_n)_{n, \id}
=
(f_n)_{m, \varphi} \circ U(\varphi)
=
(f_m)_{m, \id} \circ W(\varphi)\circ U(\varphi)
=
(f_m)_{m, \id} \circ W(\varphi)\times U(\varphi)
$
as desired.
...
$\id : W_n \to W_n$ corresponds to a unique
...
Taking $n = k$, $\varphi = \alpha'$, and $\alpha = \psi = \id_{[k]}$
we deduce that $\gamma(\alpha') = \gamma(\id_{[k]}) \circ V(\alpha')$.
In other words, the morphisms $\gamma(\id_{[k]})$, $k \leq m$
...
\gamma(\id_{[k]}) \circ V(\alpha)$. These morphisms
...
has an initial object, namely $\id : [n] \to [n]$.
\end{proof}
...
mapped by $\id_A$ to the summand with
...
$\alpha \circ \beta = \id_{[k]}$. Similarly for the
...
to $\alpha$ via $\id_A$ to the summand
...
group are $\id_A$, in other words $K(A, 0)$
is the constant simplicial object with value $A$.
The boundary maps $d_n = \sum_{i = 0}^n (-1)^i \id_A
= 0$ if $n$ odd and $ = \id_A$ if $n$ is even.
...
$d^n_j \circ s^{n - 1}_j = d^n_{j + 1} \circ s^{n - 1}_j = \id$
...
\item there is an isomorphism $g : N \circ S \to \id_\mathcal{B}$
...
isomorphism $S \circ N \to \id_\mathcal{A}$.
\end{proof}
...
the summand $A_{k + 1} \cdot \id_{[k + 1]}$.
And by the definition of $S(A_\bullet)$ above the
map $d^{k + 1}_{k + 1}$ maps $A_{k + 1} \cdot \id_{[k + 1]}$
into $A_k \cdot \id_{[k]}$ by $(-1)^{k + 1}d_{A, k + 1}$.
...
$N \circ S = \id$. Hence it suffices to show that
...
corresponding to $\id_A$ on the right hand side
...
such that $a \circ b$ is homotopic to $\id_V$
and $b \circ a$ is homotopic to $\id_U$.
...
We have $f \circ g = \id$ and we will give a
homotopy $h : \Delta[m] \times \Delta[1] \to \Delta[m]$
between $\id_{\Delta[m]}$ and $g \circ f$. Namely $h$ given by
the maps
$$
...
we could find a homotopy from $g \circ f$ to $\id_{\Delta[m]}$.
...
\begin{enumerate}
\item We have $\pi \circ e_1 = \pi \circ e_0 = \id_U$, and
\item The morphisms $\id_{U \times \Delta[1]}$,
...
is a homotopy between $\id_{U \times \Delta[1]}$ and $e_0 \circ \pi$.
...
$h_{n, 0} = \id$, $h_{n, n + 1} = (s \circ f)^{n + 1}$ and
$h_{n, i} = \id_Y^{i + 1} \times (s \circ f)^{n + 1 - i}$.
...
$h = \id_{U \times \Delta[1]}$ is mapped via
...
with homotopy $\id_{U \times \Delta[1]}$.
...
$$
Here the top horizontal arrow is given by $\id_X$ and $g \circ f$
where we use that
...
$\id_X$, and a homotopy $h' : Y \times \Delta[1] \to Y$ between
$f \circ g$ and $\id_Y$. During this proof we will write
...
$\xi = [\id_{[n]}] - [*] \in E_n$ satisfies $d_i\xi = 0$ and
...
is degenerate. Thus $\varphi = \id_{[n]}$, which is a trivial case.
\end{proof}
...
$k : F \circ i \to \id_\mathcal{A}$ and
$t : \id_\mathcal{S} \to i \circ F$
...
silently identify $i$ with $\id_\mathcal{S} \circ i$ and
$1$ stands for $\id_i : i \to i$. We will use this notation and
...
with itself. We also set $X_{-1} = \id_\mathcal{A}$.
...
to the constant simplicial object with value $X_{-1} = \id_\mathcal{A}$.
\end{lemma}
...
F \circ i \ar[rr]^k & & \id_\mathcal{A}
}
$$
is commutative. More precisely we should write this as the equality
$$
(k \star 1_{\id_\mathcal{A}}) \circ (1_{F \circ i} \star k) =
(1_{\id_\mathcal{A}} \star k) \circ (k \star 1_{F \circ i})
$$
as morphisms
$(F \circ i) \circ (F \circ i) \to
\id_\mathcal{A} \circ \text{id}_\mathcal{A}$.
...
$F \circ \id_\mathcal{A} \circ \text{id}_\mathcal{A} \circ i$
...
$F \circ (i \circ F) \circ i \circ \id_\mathcal{A}$
to $F \circ \id_\mathcal{A} \circ i \circ (F \circ i)$.
...
Recall that $t : \id_\mathcal{S} \to i \circ F$ is the
...
$j$ pairs $(F \circ i)$ to $\id_\mathcal{S}$ using $k^{\star (j)}$,
then adding a $\id_\mathcal{S}$ in front and expanding this
...
we can think of the morphism $h_{n, j}$ as collapsing the first
$j$ pairs $(F \circ i)$ to $\id_\mathcal{S}$ using $k^{\star (j)}$,
then inserting a $\id_\mathcal{S}$ just after the first $F$ and
