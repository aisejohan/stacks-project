...
$$
and point $x' \in X'$. We set $Z' = X'$, $i = \id$, and
...
$$
f' + \sum x_j g'_j = \id_{M_1} \in \text{End}_R(M_1)
$$
By Nakayama's lemma
(Algebra, Lemma \ref{algebra-lemma-NAK})
...
hence $\xi \leadsto \xi_0$. This means that $(\id_S, s' \to s, \xi)$
...
$u \otimes \id_{\kappa(\mathfrak p)}$ is injective for all
...
the map $u \otimes \id_{A/(\mathfrak p + f)}$ is universally
