...
The final assumption is that $ba \cong \id_\mathcal{A}$ which is
...
$\id_{\Sh(\mathcal{D})} \to f_*f^{-1}$ is a
...
is a $\mathcal{O}_1$-algebra map such that $\pi \circ s = \id$.
...
f^{-1}\Omega_{X/Y} \ar[rr]^{1 \otimes \id} & &
f^*\Omega_{X/Y}
}
$$
...
equals $\id_A$. Hence a functorial direct sum decomposition
...
ringed site, then setting $f^\sharp = \id$
