...
\noindent
The following lemma doesn't fit anywhere else, so we've put it here.

\begin{lemma}
\label{lemma-base-change-and-length}
Let $K/k$ be an extension of fields. Let $A$ be a finite dimensional
$k$-algebra and set $B = A \otimes_k K$. For any prime ideal
$\mathfrak q \subset B$ lying over $\mathfrak p \subset A$ we have
$$
\text{length}(A_\mathfrak p)[\kappa(\mathfrak p) : k]_i
=
\text{length}(B_\mathfrak q)[\kappa(\mathfrak q) : K]_i
$$
where $[\kappa':\kappa]_i$ denotes the inseparable degree
(Fields, Definition \ref{fields-definition-insep-degree}).
\end{lemma}

\begin{proof}
By Algebra, Lemma \ref{algebra-lemma-finite-dimensional-algebra}
the rings $A$ and $B$ are Artinian. By 
Algebra, Lemma \ref{algebra-lemma-artinian-finite-length} the ring $A$
is the product of local rings, hence we may assume
$A$ is local with maximal ideal $\mathfrak p$.
Since $A \to B$ is flat, also $A \to B_\mathfrak q$ is
flat. By Algebra, Lemma \ref{algebra-lemma-pullback-module} we have
$\text{length}(B_\mathfrak q) =
\text{length}(A)\text{length}(B_\mathfrak q/\mathfrak pB_\mathfrak q)$.
Since $B_\mathfrak q/\mathfrak pB_\mathfrak q =
(A/\mathfrak p \otimes_k K)_\mathfrak q$ this reduces us to the case 
$A = A/\mathfrak p = \kappa(\mathfrak p)$. In other words, we
may assume $A$ is a field.

\medskip\noindent
Assume $A = k'$ is a field and $\mathfrak q \subset k' \otimes_k K$.
Let $F/K$ be an algebraic closure. Choose a prime
$\mathfrak q' \subset k' \otimes_k F$ lying over $\mathfrak q$.
Suppose we can show that
$$
[k' : k]_i =
\text{length}((k' \otimes_k F)_{\mathfrak q'})
\quad\text{and}\quad
[\kappa(\mathfrak q) : K]_i =
\text{length}((\kappa(\mathfrak q) \otimes_K F)_{\mathfrak q'})
$$
Then we win because
\begin{align*}
[k' : k]_i
...
\text{length}((k' \otimes_k F)_{\mathfrak q'}) \\
& =
\text{length}((k' \otimes_k K)_\mathfrak q)
\text{length}((\kappa(\mathfrak q) \otimes_K F)_{\mathfrak q'}) \\
& =
\text{length}((k' \otimes_k K)_\mathfrak q)
[\kappa(\mathfrak q) : K]_i
\end{align*}
The second equality by Algebra, Lemma \ref{algebra-lemma-pullback-module}
and flatness of
$(k' \otimes_k K)_\mathfrak q \to (k' \otimes_k F)_{\mathfrak q'}$.

\medskip\noindent
To show the two equalities, it suffices to prove the first.
Let $k'/k_{sep}/k$ be the subfield constructed in
Fields, Lemma \ref{fields-lemma-separable-first}. Then we see that
$$
k' \otimes_k F =
\prod\nolimits_{\sigma : k_{sep} \to F}
k' \otimes_{k_{sep}, \sigma} F
$$
...
See also Lemma \ref{lemma-geometrically-reduced-dense-smooth-open},
...
\begin{example}
\label{example-degree-P1}
The degree of $\mathcal{O}(n)$ on $\mathbf{P}^1_k$ is $n$. For example
$\deg(\mathcal{O}(1) = \chi(\mathcal{O}(1)) - \chi(\mathcal{O}) = 2 - 1 = 1$;
this uses the computation of cohomology of $\mathcal{O}(n)$ in
Cohomology of Schemes, Section
\ref{coherent-section-cohomology-projective-space}.
Another way to see $\deg(\mathcal{O}(1)) = 1$ is to use that it
has a nonzero section vanishing to order $1$ in a rational point
and use Lemma \ref{lemma-degree-if-regular-section}.
\end{example}
...
\begin{lemma}
\label{lemma-degree-if-regular-section}
Let $k$ be a field.
Let $X$ be a proper scheme of dimension $\leq 1$ over $k$.
Let $\mathcal{L}$ be an invertible $\mathcal{O}_X$-module
and let $s$ be a regular section of $\mathcal{L}$ (for example
a nonzero section if $X$ is a curve). Then
$\deg(\mathcal{L}) = \deg(D)$ where $D = Z(s) \subset X$.
\end{lemma}

\begin{proof}
This follows from Lemma \ref{lemma-degree-effective-Cartier-divisor}
and the fact that $\mathcal{L} \cong \mathcal{O}_X(D)$ by
