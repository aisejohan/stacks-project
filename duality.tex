...
\end{equation}
where the first arrow comes from $\id \to Rj_* \circ j^*$
and the final arrow from the isomorphism $(j')^* \circ Rj'_* \to \id$.
...
$$
tensored with $\id_K$. Here $\epsilon$ is the isomorphism from
Derived Categories of Schemes, Lemma
\ref{perfect-lemma-cohomology-base-change} and
$\beta$ comes from the counit map
$g_*b \to \id$. Similarly, the composition of the lower
horizontal arrows is adjoint to $\id_K$ tensored with the composition 
$$
...
\mathcal{O}_Z
$$
where $\gamma$ comes from the counit map $f_*a \to \id$
...
counit $Ri_*R\SheafHom(\mathcal{O}_Z, K) \to K$ tensored with $\id_M$.
...
\ar[rr]^-{\id_K \otimes \text{Tr}_{f, \mathcal{O}_Y}} & & K
}
$$
...
\begin{enumerate}
\item The first equality holds as $\id = c \circ b$ because
$\id_X = p \circ \Delta$.
...
Lemma \ref{lemma-compare-with-pullback-flat-proper-noetherian}.
The third equality because $p \circ \Delta = \id_X$.
...
open immersion $X \to \overline{X}$ because $f_1|_X = f_2|_X = \id_X$.
...
$$
is commutative. Let $\eta_Y : \id \to D_Y^2$
and $\eta_Z : \id \to D_Z^2$ be the canonical isomorphisms
...
applied to $\id : X \to X$ we get a unique morphism
