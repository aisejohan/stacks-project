...
Observe that $(x_i, \id)$ and $(x'_i, \text{id})$ define
...
$$
\xymatrix{
U \ar[d] \ar[rr] & & U_i \ar[d]^{(x'_i, \id)} \\
U_i \ar[r]^{(x_i, \id)} & X_{y_i} \ar[r]^{\beta_i} & X_{y'_i}
}
$$
commutes. Namely, this is equivalent to the condition that
the pairs $(x_i|_U, \beta_i|_U)$ and $(x'_i|_U, \id)$
...
$$
\xymatrix{
U_{i'} \ar[d] \ar[rr] & & U_i \ar[d]^{(x'_i, \id)} \\
U_i \ar[r]^{(x_i, \id)} & X_{y_i} \ar[r]^{\beta_i} & X_{y'_i}
}
$$
