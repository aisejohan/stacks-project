...
$\id_{small, \etale, Zar} : X_\etale \to X_{Zar}$
discussed in
...
is equal to $\id_{small, \etale, Zar, *}$, since
$\id_{small, \etale, Zar}^*$ is left adjoint to
$\id_{small, \etale, Zar, *}$ and since
$(\Omega_{X/Y})^a = \id_{small, \etale, Zar}^*\Omega_{X/Y}$
...
to $\id : X \to X$ over $Y$ we see that (1) translates into
...
morphism such that $f \circ s = \id_U$. Assume
\begin{enumerate}
\item $f$ is proper,
...
$\id_Y : Y \to Y$ as our \'etale covering to see the
...
$$
(\id_U, j) : U \longrightarrow X \times_A \mathbf{P}^n_A
$$
...
Then we see that (2) holds, that $W = (\id_U, j)(U)$, and hence
