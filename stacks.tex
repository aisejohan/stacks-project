...
is the descent datum $(X, \id_X)$ with respect to the family
$\{\id_U : U \to U\}$.
...
descent datum $(X, \id_X)$ via the
obvious map $\{f_i : U_i \to U\} \to \{\id_U : U \to U\}$.
...
over $\id_V$. Similarly, morphisms in the fibre category
$\mathcal{S}_{2, V}$ are exactly those morphisms between $F(x|_V)$ and
$F(y|_V)$ in $\mathcal{S}_2$ lying over $\id_V$. Thus we find that $F$
...
$\gamma = \alpha^{-1} \circ \beta$ we see that $G(\gamma) = \id_{G(y)}$
and we have to show that $\gamma = \id_y$. By assumption we
can find a covering $\{U_i \to U\}$ such that $G(y)|_{U_i}$ is in the
essential image of $F :(\mathcal{S}_2)_{U_i} \to (\mathcal{S}_1)_{U_i}$.
Since it suffices to show that $\gamma|_{U_i} = \id$ for each $i$,
...
over $U$ whose image under $G'$ in $\mathcal{S}_1$ is $\id_x$.
As $G'$ is faithful we conclude that $\gamma = \id_y$ and we win.
\end{proof}
...
Then we can think of $\xi = (x, \beta)$ and $\xi' = (x', \id_{F(x')})$
...
$F(\alpha) = \id_{F(x)}$. There is also an absolute version,
...
defined morphism $(p(a) : U \to V, \{\id_U\}, a)$.
...
with $G(\alpha_{ijk}) = \id_{x'|_{U_{ijk}}}$. Set $y_i = F(x_i)$.
...
because $F(x_i) = y_i$. Because $F(\varphi_{ij}) = \id$ (as we started
...
$$
(a_i, \id) : (U, x, y, f)|_{U_i} \to (U, x', y, f')|_{U_i}
$$
...
such that $F'(a'_{ij}) = \id_{y'_i}|_{U_{ij}}$. Unwinding the definition
...
$$
f_1 =
((a_1, b_1, \alpha_1) : X' \to X_1, (c, \id_V, \gamma) : X' \to X) :
...
(a_2, b_2, \alpha_2).
$$
Write $r = (c, \id_V, \gamma) : (U', \phi' : V \to u(U'), x')$,
...
$(c', \id_V, \gamma')$ of $R$. After doing this, i.e., replacing
...
$p_p(f) = \id_V$. We are done if we can show that $f$ is invertible, see
Categories, Lemma \ref{categories-lemma-fibred-groupoids}.
Write $f$ as the equivalence class of
a pair $((a, b, \alpha), r)$ with $r \in R$. Then $p_p(r) = \id_V$,
hence $p_{pp}((a, b, \alpha)) = \id_V$. Hence $b = \text{id}_V$.
...
$$
H(x/U) = (U, G'(U, \id_{u(U)} : u(U) \to u(U), x))
$$
on objects and by
$$
...
Lemma \ref{lemma-fibred-category-pushforward}.
Hence in this case the morphism $\beta$ has $q(\beta) = \id_V$
...
$$
with $\gamma$ strongly cartesian and $b = \id_{u(U)}$.
But since $u$ is fully faithful we can write $\phi = u(c')$ for some
morphism $c' : U \to U'$
and then we see that $a \circ c' = \id_U$ and
$c \circ c' = \id_{U'}$. Because $\gamma$ is strongly cartesian
we can find a morphism $\gamma' : x \to x'$ lifting $c'$ such that
$\gamma \circ \gamma' = \id_x$. By definition of the equivalence
classes defining morphisms in $u_p\mathcal{S}$ it follows that
the morphism
$$
\xymatrix{
(U, \id : u(U) \to u(U), x)
\ar[rr]_{(\id, \text{id}, \alpha \circ \gamma')} & &
(U, \id : u(U) \to u(U), y)
}
$$
