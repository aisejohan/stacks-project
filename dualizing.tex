...
Then the map $\id_E : E \to E$ can be extended
...
\begin{proof}
We have seen that $D \circ D = \id$ on $\text{Mod}^{fg}_R$
...
\right\}
$$
and we have $D \circ D = \id$ on either side of the equivalence.
\end{proposition}
...
existence of the natural transformation $R\Gamma_Z \to \id$
...
For this we can choose $can \otimes^\mathbf{L} \id_{R'}$
...
and the fact that $D_B \circ D_B \cong \id$ by
Lemma \ref{lemma-dualizing}.
...
\medskip\noindent
Assume $B = A[x_1, \ldots, x_n]$. Since $D_A \circ D_A \cong \id$,
...
$D_A \circ D_A = \id$. The final equality by adjointness again.
