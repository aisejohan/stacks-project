...
$$
\xymatrix{
\mathcal{O} \times \mathcal{F} \ar[r] \ar[d]_{\id \times \varphi} &
...
$\id_{f_*\mathcal{O}}$ corresponds to $c_\mathcal{O}$
under the adjunction described in the proof of
Lemma \ref{lemma-pullback-presheaves}
and thus
$\id_{f_*\mathcal{O}} = f_*c_\mathcal{O} \circ i_{f_*\mathcal{O}}$.
\end{proof}
...
$$
\item On the category of presheaves of $U$ we have $j_pj_* = \id$.
\item On the category of sheaves of $U$ we have $j^{-1}j_* = \id$.
\end{enumerate}
...
\right.
$$
\item On the category of presheaves of $U$ we have $j_pj_{p!} = \id$.
\item On the category of sheaves of $U$ we have $j^{-1}j_! = \id$.
\end{enumerate}
\end{lemma}
...
\begin{proof}
Fully faithfulness follows formally from $j^{-1} j_! = \id$.
...
$i^{-1}i_* = \id$ on the category of sheaves
...
$i^{-1}i_* = \id$ consider the canonical map
...
\begin{proof}
Fully faithfulness follows formally from $i^{-1} i_* = \id$.
We have seen that any sheaf in the image of the functor has
the property on the stalks mentioned in the lemma. Conversely, suppose
that $\mathcal{G}$ has the indicated property.
Then it is easy to check that
...
$$
\xymatrix{
\mathcal{F}|_{U_i \cap U_j} \ar[r]_{\varphi_i} \ar[d]_{\id} &
