...
morphism $\id_x\in \Mor_\mathcal{C}(x, x)$ such that
$\id_x \circ \phi = \phi$ and $\psi \circ \text{id}_x = \psi $ whenever
...
speak of {\it the} identity morphism $\id_x$ of $x$.
\end{remark}
...
such that $\phi \circ \psi = \id_y$ and
$\psi \circ \phi = \id_x$.
\end{definition}
...
morphisms of $\mathcal{A}$ and $F(\id_x) = \text{id}_{F(x)}$.
\end{definition}

\noindent
Note that every category $\mathcal{A}$ has an
{\it identity} functor $\id_\mathcal{A}$.
...
$\id_F : F \to F$. In addition, given a morphism of
...
identity functors $\id_\mathcal{B}$,
respectively $\id_\mathcal{A}$.
...
$\id_\mathcal{B} \to F \circ j$. Moreover, $j$ and $F$
...
\quad
s \longmapsto s_U(\id_U).
$$
\end{lemma}

\begin{proof}
For the first statement, just take
$\phi = s_U(\id_U) \in \Mor_\mathcal{C}(U, V)$.
...
Set $F(y_i) = F(z_i) = x_i$. Set $F(g_j) = f_j$ and $F(h_i) = \id_{x_i}$.
...
$$
M(F(h_i)) = \id : M(F(y_i)) = M(x_i) \longrightarrow M(x_i) = M(F(z_i))
$$
...
{\it filtered} if $\id : \mathcal{I} \to \mathcal{I}$ is filtered
...
{\it cofiltered} if $\id : \mathcal{I} \to \mathcal{I}$ is
...
= \id$ and such that $f_{ii''} = f_{i'i''} \circ f_{i i'}$
...
arrow $\id_{i_\infty}$ and the arrows $f_i$.
...
that the composition $X \to M_i \to X$ is $\id_X$. A stronger condition
is the following.
...
\begin{proof}
Assume (2) holds. Then $\id_X \in \Mor_\mathcal{C}(X, X)$
...
\colim_i \Mor_\mathcal{C}(W, M_i)
$$
is the identity too. Setting $W = M_j$ and starting with $\id_{M_j}$
...
$\id_{u(X)}$. Similarly, for any object $Y$ of $\mathcal{D}$ we obtain
a morphism $u(v(Y)) \to Y$ corresponding to $\id_{v(Y)}$.
...
$$
\id_\mathcal{C} \to v \circ u\quad (\text{unit})
\quad\text{and}\quad
u \circ v \to \id_\mathcal{D}\quad (\text{counit}).
$$
...
Then
\begin{enumerate}
\item $u$ is fully faithful $\Leftrightarrow \id \cong v \circ u$.
\item $v$ is fully faithful $\Leftrightarrow u \circ v \cong \id$.
\end{enumerate}
\end{lemma}
...
Conversely, if $\id \cong v \circ u$ then $u$ has to be fully
...
equivalence class of the pair $(\id : X \to X,
\id : X \to X)$.
\end{enumerate}
...
Namely, LMS1 implies that $\id_Y : Y \to Y$
...
$(f : X \to Y) \mapsto (f : X \to Y, \id_Y : Y \to Y)$
...
the equivalence class of the pair $(\id_Y, s)$.)
...
\begin{enumerate}
\item The rules $X \mapsto X$ and
$(f : X \to Y) \mapsto (f : X \to Y, \id_X : X \to X)$
define a functor $Q : \mathcal{C} \to S^{-1}\mathcal{C}$.
\item For any $s \in S$ the morphism $Q(s)$ is an isomorphism in
$S^{-1}\mathcal{C}$.
\item If $G : \mathcal{C} \to \mathcal{D}$ is any functor such that
...
The relation $\id_X = s^{-1}gf$ in $S^{-1}\mathcal{C}$ means
there exists a commutative diagram
$$
\xymatrix{
 & X' \ar[d]^u & \\
X \ar[ru]^{gf} \ar[r]^{f'} \ar[rd]_{\id_X} &
X'' &
X \ar[lu]_s \ar[l]_{s'} \ar[ld]^{\id_X} \\
& X \ar[u]_v &
}
$$
for some morphisms $f', u, v$ and $s' \in S$. Hence $ugf = s' \in S$.
Similarly, using that $\id_Y = fht^{-1}$ one proves that
...
${}_G(\id_F) = \text{id}_{G \circ F}$ and that
${}_G(t_1 \circ t_2) = {}_Gt_1 \circ {}_Gt_2$.
Of course we also have that ${}_{\id_\mathcal{A}}t = t$.
...
$$
To see this you just have to check that
$(\id_G)_F = \text{id}_{G\circ F}$ and that
$(s_1 \circ s_2)_F = s_{1, F} \circ s_{2, F}$.
Of course we also have that $s_{\id_\mathcal{B}} = s$.
...
$ {}_Gt = \id_G \star t $ and $ s_F = s \star \text{id}_F $.
...
\item the identity transformations $\id_F$
are units for $\circ$,
\item the identity transformations of the identity functors
$\id_{\text{id}_\mathcal{A}}$
...
\item The identity $2$-morphism $\id_{\text{id}_x}$
of the identity $1$-morphism $\id_x$ is a unit for
...
$2$-isomorphic to $\id_y$ and $G \circ F$ is $2$-isomorphic to
$\id_x$.
\end{definition}
...
$\alpha_x : \id_{\varphi(x)} \to \varphi(\text{id}_x)$, and
...
$\alpha_{\id_y, f} = \alpha_y \star \text{id}_{\varphi(f)}$:
$$
\xymatrix{
\varphi(x)
\rrtwocell^{\varphi(f)}_{\varphi(f)}{\ \ \ \ \id_{\varphi(f)}}
& &
\varphi(y)
\rrtwocell^{\id_{\varphi(y)}}_{\varphi(\text{id}_y)}{\alpha_y}
& &
\varphi(y)
}
=
\xymatrix{
\varphi(x)
\rrtwocell^{\varphi(f)}_{\varphi(\id_y) \circ \varphi(f)}{\ \ \ \ \alpha_{\text{id}_y, f}}
& &
\varphi(y)
}
$$
\item for any morphism $f : x \to y$ in $\mathcal{A}$ we have
$\alpha_{f, \id_x} = \text{id}_{\varphi(f)} \star \alpha_x$,
\item for any triple of composable morphisms
$f : w \to x$, $g : x \to y$, and $h : y \to z$ of $\mathcal{A}$
we have
$$
(\id_{\varphi(h)} \star \alpha_{g, f})
\circ
\alpha_{h, g \circ f}
=
(\alpha_{h, g} \star \id_{\varphi(f)})
\circ
\alpha_{h \circ g, f}
$$
...
\ar[d]^{\alpha_{h, g} \star \id_{\varphi(f)}} \\
\varphi(h) \circ \varphi(g \circ f)
\ar[rr]^{\id_{\varphi(h)} \star \alpha_{g, f}}
& &
...
(k \circ k',
(\alpha \star \id_{k'}) \circ \alpha',
(\beta \star \id_{k'}) \circ \beta'),
$$
...
\ar[r]_{\alpha_1} &
a \circ k_1
\ar[d]^{\id_a \star \delta} &
&
b \circ k_1
\ar[d]_{\id_b \star \delta} &
b'
\ar[l]^{\beta_1}
\ar[ld]^{\beta_2}
\\
...
\xymatrix@C=12pc{
(w'', a'', b'', \phi'')
\rtwocell^{(k_1 \circ k'_1, (\alpha_1 \star \id_{k'_1}) \circ \alpha'_1, (\beta_1 \star \text{id}_{k'_1}) \circ \beta'_1)}_{(k_2 \circ k'_2, (\alpha_2 \star \text{id}_{k'_2}) \circ \alpha'_2, (\beta_2 \star \text{id}_{k'_2}) \circ \beta'_2)}{\ \ \ \delta \star \delta'}
&
(w, a, b, \phi)
}
$$
...
$((X_1, X_2), G_2(X_2), (\psi, \id_{G_2(X_2)}))$.
...
$z = x$ shows that $(\id_x, p(\varphi)^{-1})$ comes from a unique
...
$\id_U^*x = x$, although in practice this is a useless property
...
$$
\item If $f = \id_U$, then there is a canonical isomorphism
$\alpha_U : \id \to (\text{id}_U)^*$ as functors
...
since $\id_x : x \to x$ is clearly strongly cartesian
over $\id_U$ (with $U = p(x)$) we see that there exists
an isomorphism $(\alpha_U)_x : x \to (\id_U)^*x$.
...
$i : F \circ G \to \id_{\mathcal{S}_2}$,
$j : G \circ F \to \id_{\mathcal{S}_1}$ be isomorphisms of
...
$x \mapsto (p(x), x, F(x), \id)$. This is fully faithful.
...
$$
\Mor_\mathcal{X}(x, x') =
\Mor_{\mathcal{X}'}((U, x, y, f), (p(x'), x', F(x'), \id)),
$$
...
and $\alpha : x \to x$ is an automorphism with $F(\alpha) = \id$,
...
$\mathcal{S}$, and
$(\iota, \kappa) : (x, x, \id_{F(x)}) \to (y, y, \text{id}_{F(y)})$
...
$(x, x, (\alpha, \id_x))$ of the right hand side
...
\end{equation}
defined by the rules $x \mapsto (x, \id_x)$ and
...
and $\id_W$ we conclude that there is a unique morphism
...
$fg = \id_x$. By a similar argument there is a unique $h : y \to x$ so
that $gh = \id_y$. Then $fgh = f : y \to x$.  We have $fg = \text{id}_x$,
...
$\alpha : x' \to G(f^*x)$ such that $p'(\alpha) = \id_{U'}$
...
to the triple $(x, x, \id_{G(x)})$. Note that $(x, x', \alpha)$
...
$\mathcal{X}' = \mathcal{X} \times_{F, \mathcal{Y}, \id} \mathcal{Y}$
...
$x \mapsto (p(x), x, F(x), \id_{F(x)})$ on objects and
...
we can find $f' : F(x') \to y'$ lying over $\id_{U'}$ such that
...
$\mathcal{X} \times_{F, \mathcal{Y}, \id} \mathcal{Y}$
...
\mathcal{X} \times_{F, \mathcal{Y}, \id} \mathcal{Y}$,
$x \mapsto (p(x), x, F(x), \id_{F(x)})$
constructed in the proof of
Lemma \ref{lemma-ameliorate-morphism-categories-fibred-groupoids}.
Let $\alpha : F \to g \circ b$ and $\beta : b^{-1} \circ b \to \id$
...
$(\varphi, \id_V) : (x_1, f_1) \to (x_2, f_2)$
between morphisms in $\mathcal{S}'_V$ we set $g^\ast(\varphi, \id_V) =
(g^\ast\varphi, \id_{V'})$ where we use the unique identifications
...
of automorphisms of $x$. Hence taking $\alpha' = \id_x$ we conclude
...
$j' \circ t' \cong \id_{\mathcal{C}/X'}$ and
$t' \circ j' \cong \id_\mathcal{S}$. Then
...
\mathcal{S}_U
$$
given by $G \mapsto G(\id_U)$ is an equivalence.
\end{lemma}
...
a final object namely $\id : \mathcal{C} \to \mathcal{C}$.
