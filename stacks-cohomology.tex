...
equivalences of categories. The assertion $Q \circ i \cong \id$
...
\item the adjunction maps $g^{-1}g_* \to \id$ and
$\id \to g^{-1}g_!^{Sh}$ are isomorphisms,
\item the functor $g^{-1}$ has a left adjoint $g_!$ on abelian sheaves,
\item the adjunction map $\id \to g^{-1}g_!$ is an isomorphism, and
...
on abelian sheaves and $g^*g_! = \id$.
\item There exists a functor
$$
g_! :
...
\medskip\noindent
In both cases the equality $g^*g_! = \id$ follows from
$g^* = g^{-1}$ and the
