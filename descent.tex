...
is given by $\id_S$ a descent datum is necessarily
of the form $(\mathcal{F}, \id_\mathcal{F})$. The cocycle
...
$$
\xymatrix{
N \otimes_R A \ar[r]_\varphi \ar[d]_{\psi \otimes \id_A} &
A \otimes_R N \ar[d]^{\id_A \otimes \psi} \\
...
$\varphi_{01} = \varphi \otimes \id_A$,
$\varphi_{12} = \id_A \otimes \varphi$, and
...
$\varphi^n_{ij} = \id$. Since these are all isomorphisms they allow us
...
We first verify the two properties $\sigma^0_0 \circ \delta^1_0 = \id$
and $\sigma^0_0 \circ \delta^1_1 = \id$.
The first one, $\sigma^0_0 \circ \delta^1_0 = \id$, is clear from
...
$\sigma_j \circ \delta_j = \sigma_j \circ \delta_{j + 1} = \id$.
...
$\varphi' = \id_{R'} \otimes \varphi$ the base change
...
$\sigma^0_0 \circ \delta^1_1 = \id$ in any cosimplicial object).
...
\end{enumerate}
Here $f_\sigma = \id_X \times \Spec(\sigma) : X_{k'} \to X_{k'}$.
\end{lemma}
...
\begin{enumerate}
\item $(\id_{\tau, Zar})^*\mathcal{F}$ is the $\tau$-sheafification
of the Zariski sheaf
$$
(f : T \to S) \longmapsto \Gamma(T, f^*\mathcal{F})
$$
on $(\Sch/S)_\tau$, and
\item $(\id_{small, \etale, Zar})^*\mathcal{F}$ is the
...
on $S_\etale$. Then
\begin{enumerate}
\item[(3)] $(\id_{\tau, \etale})^*\mathcal{G}$ is the
...
such that $\id_{\tau, Zar} \circ i_f = f_{small}$ as morphisms
$T_{Zar} \to S_{Zar}$, see
Topologies, Lemmas \ref{topologies-lemma-put-in-T} and
...
we see that
$i_f^*(\id_{\tau, Zar})^*\mathcal{F} = f_{small}^*\mathcal{F}$
as desired. Hence, by the remark preceding this lemma we see that
$(\id_{\tau, Zar})^*\mathcal{F}$ is the $\tau$-sheafification of
...
$\id_{\tau, Zar} : ((\Sch/S)_\tau, \mathcal{O})
\to (S_{Zar}, \mathcal{O})$
or the morphism
$\id_{small, \etale, Zar} :
...
$f = \id_S$ and the proposition is proved.
...
$\id_Y : Y \to Y$ is an \'etale morphism, and $h_i$ is as in part (3) of
...
$$
\xymatrix{
V \times_S X \ar[r]_{\varphi} \ar[d]_{f \times \id_X} &
X \times_S V \ar[d]^{\id_X \times f} \\
...
$$
\xymatrix{
V_i \times_S X_j \ar[r]_{\varphi_{ij}} \ar[d]_{\psi_i \times \id} &
X_i \times_S V_j \ar[d]^{\id \times \psi_j} \\
...
$\id : S \to S$, namely the identity morphism on $U$.
...
$$
V \times_S X \xrightarrow{\psi \times \id_X}
X \times_S U \times_S X =
X \times_S X \times_S U
\xrightarrow{\id_X \times \psi^{-1}}
X \times_S V
$$
...
descent datum for $U$ relative to $\{\id : S \to S\}$.
...
$$
\xymatrix{
V' \times_{S'} X' \ar[r]_{\varphi'} \ar[d]_{\alpha' \times \id} &
X' \times_{S'} V' \ar[d]^{\id \times \alpha'} \\
...
$$
\xymatrix{
V \times_S X \ar[r]_{\varphi} \ar[d]_{\alpha \times \id} &
X \times_S V \ar[d]^{\id \times \alpha} \\
...
$\varphi_{ij} = \id_{U_i \cap U_j}$, see
Schemes, Lemma \ref{schemes-lemma-glue}.
...
(\pi \times \id_X)_*\mathcal{O}_{V \times_S X}, \quad
\text{pr}_1^*\pi_*\mathcal{O}_V =
(\id_X \times \pi)_*\mathcal{O}_{X \times_S V}
$$
by flat base change
(Cohomology of Schemes, Lemma \ref{coherent-lemma-flat-base-change-cohomology}).
