...
the condition ``$\alpha = \id_x$'' defines an algebraic space over $V$.
...
if and only if $f^*\alpha = \id_{f^*x}$.)
...
(1) implies that ``$\alpha^{-1} \circ \beta = \id_{h^*x}$''
defines an algebraic space. The implication (2) $\Rightarrow$ (1)
follows by taking $h = \id_V$ and $\beta = \text{id}_x$.
\end{proof}
...
and
$$
p'(w_{i'}) = z_i|_{U_{i'}} \xrightarrow{\id} z_i|_{U_{i'}}.
$$
...
\text{Res}_{Z/B}(X) \ar[r] \ar[u] & B \ar[u]_{\id_Z}
}
$$
...
$(U, Z, H(y'), G(x'), \gamma \star \id_H \star \alpha')$
...
assume
\begin{enumerate}
\item $\mathcal{Y}' = \mathcal{Y}$ and $H = \id_\mathcal{Y}$,
...
$$
G(T) = \{g \in \GL_d(\Gamma(T, \mathcal{O}_T)) \mid g(e_1) = e_1\}
$$
It is clear that $G \subset \GL_d$ (see
