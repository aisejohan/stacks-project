...
\end{eqnarray*}
Hence we see that $\GL_n$ is a group scheme over $\mathbf{Z}$.
For any scheme $S$ the base change $\GL_{n, S}$ is a
group scheme over $S$ whose functor of points is
$$
T/S
\longmapsto
\GL_{n, S}(T) = \text{GL}_n(T) =\text{GL}_n(\Gamma(T, \mathcal{O}_T))
$$
as before.
\end{example}
...
$$
\det : \GL_n \longrightarrow \mathbf{G}_m
$$
over $\mathbf{Z}$. By base change it gives a morphism
of group schemes $\GL_{n, S} \to \mathbf{G}_{m, S}$
...
$Z_1$ and $Z_2$ since $m|_{e \times G} = \id_G$ and
$m|_{G \times e} = \id_G$. We conclude $Z_1 = Z_2$ as desired.
\end{proof}
...
$$
G \to \text{Aut}(G) \to \text{Aut}(G_n) \to \GL(A_n)
$$
...
$$
H_n = \Ker(G \to \text{Aut}(G_n)) = \Ker(G \to \GL(A_n)).
$$
...
$$
\xymatrix{
G \times_S X \ar[r]_-{\id \times f} \ar[d]_a &
G \times_S Y \ar[d]^a \\
X \ar[r]^f & Y
}
$$
...
If $G = \GL_{n, S}$ then the answer to (3) is yes
and in fact any $\GL_{n, S}$-torsor is locally trivial
(insert future reference here).
\end{remark}
...
such that
\begin{enumerate}
\item $s \circ e = t \circ e = \id$ as maps $\text{Ob} \to \text{Ob}$,
...
R \times_{s, U} V
$$
is an isomorphism over $R$ such that $e^*\varphi = \id_V$ and such that
$$
...
s \times \id_{\mathbf{A}^1},
t \times \id_{\mathbf{A}^1},
c \times \id_{\mathbf{A}^1})
$$
