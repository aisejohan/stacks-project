...
given by $\id : [n] \to [n]$, and $U = \bigcup U_i$.
\end{enumerate}
...
Condition (1) holds because $(X_n, \id_{[n]})$ is an object.
...
$(X(\varphi)^{-1}(U), \id_{[n]}) \to (U, \varphi)$ given
by setting $\psi = \varphi$. Moreover, given a pair of objects
of the form $(U, \id_{[n]})$ and $(U', \text{id}_{[n]})$
we see there exists an object, namely $(U \cap U', \id_{[n]})$,
...
\item a covering $\{(\id, f_i) :  U_i \to U\}$ in $\mathcal{C}_{total}$
...
for notation. Since $\sigma^n_j \circ \delta_j^{n + 1} = \id_{[n]}$
the composition
$$
\mathcal{F}_n =
(f_{\sigma^n_j})^{-1}
...
$\id : g_{n!}\mathbf{Z} \to g_{n!}\mathbf{Z}$.
...
which in turn is adjoint to
$\id : g_{n!}\mathcal{O}_n \to g_{n!}\mathcal{O}_n$.
Using the description of $g_{n!}$
...
and letting $\{(\id, f_j) : U_j/U_{n, i} \to U/U_{n, i}\}$
...
$$
Taking $i = (U, V, U, a, \id_U) \in \Omega$ for example, we find
...
$d^1_0 \circ s^0_0 = \id_{X_0} = d^1_1 \circ s^0_0$, see
Simplicial, Lemma \ref{simplicial-lemma-characterize-simplicial-object}.
...
$(s_0^0)^*\varphi = \id_V$ and such that
$$
...
Because $a^{-1}$ is fully faithful we have $\id \cong a_*a^{-1}$ by
Categories, Lemma \ref{categories-lemma-adjoint-fully-faithful}.
