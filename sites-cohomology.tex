...
the induced map
$$
\text{Tot}(\id_{\mathcal{F}^\bullet} \otimes \alpha) :
...
$$
of ringed tpoi. Then the base change maps of
Remark \ref{remark-base-change}
for the two squares compose to give the base
change map for the outer rectangle. More precisely,
the composition
...
coming from the counit $Lf^* \circ Rf_* \to \id$.
\end{remark}
...
is exact,
\item the unit $\id \to \pi_{X, *} \circ \pi_X^{-1}$
...
\begin{proof}
Proof of (1).
The equality $H^n_{Zar}(X, \pi_X^{-1}\mathcal{F}) = H^q(X, \mathcal{F})$
...
\noindent
Our next goal is to prove Lemmas \ref{lemma-compare-cohomology-LC} and
\ref{lemma-cohomological-descent-LC}. We will do this by an induction
argument using the following two statements
\begin{enumerate}
\item[($V_n$)] For $X \in \Ob(\textit{LC}_{qc})$ and
abelian sheaf $\mathcal{F}$ on $X$ we have
$\epsilon_{X, *}a_X^{-1}\mathcal{F} = \pi_X^{-1}\mathcal{F}$ and
$R^i\epsilon_{X, *}(a_X^{-1}\mathcal{F}) = 0$
for $1 \leq i \leq n$.
\item[($C_n$)] For a proper morphism $f : X \to Y$ in $\textit{LC}_{qc}$
and abelian sheaf $\mathcal{F}$ on $X$ we have
$a_Y^{-1}(R^if_*\mathcal{F}) = R^if_{qc, *}(a_X^{-1}\mathcal{F})$
for $i = 0, \ldots, n$.
\end{enumerate}
Observe that $V_0$ holds by Lemma \ref{lemma-describe-pullback-pi}
and that $C_0$ holds by Lemma \ref{lemma-push-pull-LC}.

\begin{lemma}
\label{lemma-V-implies-C}
...
Lemma \ref{lemma-push-pull-LC} without further mention. In particular have
$$
R\epsilon_{Y, *}(Rf_{qc, *}(a_X^{-1}\mathcal{F})) =
Rf_{Zar, *}(R\epsilon_{X, *}(a_X^{-1}\mathcal{F}))
$$
Using the spectral sequences of Lemma \ref{lemma-relative-Leray}
on both sides, the description of $R^if_{qc, *}a_X^{-1}\mathcal{F}$
for $i \leq n - 1$ given to us by $C_{n - 1}$, and
the vanishing furnished to us by $V_n$
(for both $X$ and $Y$) we obtain 
$$
\epsilon_{Y, *}(R^nf_{qc, *}(a_X^{-1}\mathcal{F})) =
R^nf_{Zar, *}(\epsilon_{X, *}(a_X^{-1}\mathcal{F}))
$$
The sheaf on the right is equal to $R^nf_{Zar, *}(\pi_X^{-1}\mathcal{F})$
by $V_0$. This is equal to $\pi_Y^{-1} R^nf_*\mathcal{F}$
by Lemma \ref{lemma-collect-true-things-Zar} part (7b).
If we rewrite this as $\epsilon_{Y, *}(a_Y^{-1}R^nf_*\mathcal{F})$
then we get
$$
\epsilon_{Y, *}(R^nf_{qc, *}(a_X^{-1}\mathcal{F})) =
\epsilon_{Y, *}(a_Y^{-1}R^nf_*\mathcal{F})
$$
This finishes the proof as $\epsilon_{Y, *}$ reflects
isomorphisms (see proof of Lemma \ref{lemma-push-pull-LC}).
\end{proof}

\noindent
If $\mathcal{H}$ is an abelian sheaf on $\textit{LC}_{qc}/X$,
then we will write $H^n_{qc}(U, \mathcal{H})$ for the cohomology
of $\mathcal{H}$ over an object $U$ of $\textit{LC}_{qc}/X$.

\begin{lemma}
\label{lemma-V-implies-cohomology}
With notation as above, if $V_n$ holds, then for
$X \in \Ob(\textit{LC}_{qc})$ and $L \in D(X)$
with $H^i(L) = 0$ for $i < 0$
we have $H^n(X, L) = H^n_{qc}(X, a_X^{-1}L)$
\end{lemma}

\begin{proof}
We have $H^n(X, L) = H^n_{Zar}(X, \pi_X^{-1}L)$ by
Lemma \ref{lemma-collect-true-things-Zar}.
By Lemma \ref{lemma-Leray-unbounded} we have
$H^n_{qc}(X, a_X^{-1}L) = H^n_{Zar}(X, R\epsilon_{X, *}a_X^{-1}L)$.
Represent $L$ by a complex $\mathcal{F}^\bullet$ of abelian
sheaves on $X$ with $\mathcal{F}^i = 0$ for $i < 0$. We have
...
\begin{lemma}
\label{lemma-induction-step-V-C}
With notation as above, we have $V_n + C_n \Rightarrow V_{n + 1}$.
\end{lemma}

\begin{proof}
Let $X \in \Ob(\textit{LC}_{qc})$ and $\mathcal{F} \in \Ob(\textit{Ab}(X))$.
Let $\xi \in H^{n + 1}_{qc}(U, a_X^{-1}\mathcal{F})$ for some $U/X$.
We have to show that $\xi$ restricts to zero on the members of an
open covering of $U$. See
Lemma \ref{lemma-higher-direct-images}.
Pick $u \in U$. We have to find an open neighbourhood $u \in V \subset U$
such that $\xi$ maps to zero in $H^{n + 1}_{qc}(V, a_X^{-1}\mathcal{F})$.

\medskip\noindent
By locality of cohomology
(Lemma \ref{lemma-kill-cohomology-class-on-covering})
we can choose a qc covering $\{U_i \to U\}$
such that $\xi$ restricts to zero on $U_i$.
Pick $i_1, \ldots, i_m \in I$ and quasi-compact subsets
$E_j \subset U_{i_j}$ such that
$\bigcup f_{i_j}(E_j)$ is a neighbourhood of $x$.
...
Observe that $Y = \coprod E_j$ is a disjoint union (in the sense
that $E_j \times_Y E_{j'}$ is empty if $j \not = j'$) and hence
$\xi$ restricts to zero on $Y$.
Choose an open neighbourhood $u \in V$ contained in $\bigcup f_{i_j}(E_j)$.
Then $Y \times_U V \to V$ is surjective and proper and
the restriction of $\xi$ to $Y \times_U V$ is zero.
This observation, changing notation, and combining
with Lemma \ref{lemma-cohomology-of-open}
reduces us to the problem described in the next paragraph.

\medskip\noindent
Let $f : X \to Y$ be a surjective proper morphism.
Let $\mathcal{F}$ be an abelian sheaf on $Y$.
Let $\xi \in H^{n + 1}_{qc}(Y, a_Y^{-1}\mathcal{F})$
which maps to zero in $H^{n + 1}_{qc}(X, a_Y^{-1}\mathcal{F})$.
Problem: show that $\xi$ restricts to zero on an open covering of $Y$.
The assumption implies that $\xi$ maps to zero under the map
$$
a_Y^{-1}\mathcal{F} \longrightarrow
Rf_{qc, *}f_{qc}^{-1}a_Y^{-1}\mathcal{F} =
Rf_{qc, *}a_X^{-1}f^{-1}\mathcal{F}
$$
...
$$
a_Y^{-1}\mathcal{F} \longrightarrow
\tau_{\leq n}Rf_{qc, *}a_X^{-1}f^{-1}\mathcal{F}
$$
Set $K = \tau_{\leq n}Rf_*f^{-1}\mathcal{F}$ in $D^+(Y)$ and
choose a distinguished triangle $\mathcal{F} \to K \to L$.
Note that $\mathcal{F} \to f_*f^{-1}\mathcal{F}$ is injective
as $f : X \to Y$ is surjective, hence $L$ only has nonzero
cohomology sheaves in degrees $0, \ldots, n$. By $C_n$ the canonical map
$a_Y^{-1}Rf_*f^{-1}\mathcal{F} \to Rf_{qc, *}a_X^{-1}f^{-1}\mathcal{F}$
induces an isomorphism
$$
a_Y^{-1}K \longrightarrow \tau_{\leq n}Rf_{qc, *}a_X^{-1}f^{-1}\mathcal{F}
$$
Pulling back the distinshed triangle $\mathcal{F} \to K \to L$
by $a_Y$ we find that $\xi$ is the image of an element
$\xi' \in H^n_{qc}(Y, a_Y^{-1}L)$.
By induction, more precisely by $V_n$, we have
$$
H^n(Y, L) = H^n_{qc}(Y, a_Y^{-1}L)
$$
see Lemma \ref{lemma-V-implies-cohomology}.
Thus we may lift $\xi'$ to an element of $H^n(Y, L)$
and take the boundary into $H^{n + 1}(Y, \mathcal{F})$
to see that $\xi$ is in the image of the canonical map
$H^{n + 1}(Y, \mathcal{F}) \to H^{n + 1}_{qc}(Y, a_Y^{-1}\mathcal{F})$.
By locality of cohomology for $H^{n + 1}(Y, \mathcal{F})$, see
...
\begin{proof}
Recall that $V_0$ holds by Lemma \ref{lemma-describe-pullback-pi}
and that $C_0$ holds by Lemma \ref{lemma-push-pull-LC}.
Then we get $V_n$ and $C_n$ for all
$n$ by Lemmas \ref{lemma-V-implies-C} and \ref{lemma-induction-step-V-C}.
Finally, the reader immediately proves (1) and (2) from
$V_n$ and $C_n$ by representing $K$ by a bounded below complex
of abelian sheaves. We omit the details.
\end{proof}
...
and by Lemma \ref{lemma-collect-true-things-Zar}
...
$Lf^*Rf_* \to \id$ and the second arrow is the canonical map
...
$$
such that $\pi_U \circ \sigma = \id$ and $\sigma^{-1} = \pi_{U, *}$.
\end{lemma}
...
it follows that $\sigma^{-1} \circ \pi_U^{-1} = \id$
...
$\pi'_{U'} \circ \sigma' = \id$, $\pi_U \circ \sigma = \text{id}$,
...
by mapping $*$ to $(U \to \ldots \to U, \id_U)$, and the
maps
$$
...
(U_n \to \ldots \to U_0, f)
\longmapsto
(U_n \to \ldots \to U_i \to U \to \ldots \to U, \id)
$$
for $i > 0$ and $h_{n, 0} = \id$. Verifications omitted.
...
$L\pi_! \circ \pi^{-1} = \id$ on $D(\textit{Ab})$, resp.\ $D(B)$.
\end{lemma}
...
object, hence $R\pi_* \circ \pi^{-1} \cong \id$ on
...
Then the same lemma implies that $L\pi_! \circ \pi^{-1} = \id$
as desired.
\end{proof}
...
with $p(\varphi) = \id_V$, i.e., $\varphi$ is a morphism of the
...
(U_0 \to \ldots \to U_n, f)
\longmapsto
(U \to \ldots \to U \to U_i \to \ldots \to U_n, \id)
$$
for $i > 0$ and $h_{n, 0} = \id$. Verifications omitted.
