...
apply the identifications above to $\id_{\Spec(\mathbf{Z}[x])}$.
...
$\varphi_{ij} \circ \varphi_{ji} = \id_{U_{ji}}$
...
morphism $\id_{S'} \times_{\text{id}_S} f$).
...
$\text{pr}_1 \circ \Delta_{X/S} = \id_X$ and
$\text{pr}_2 \circ \Delta_{X/S} = \id_X$ (it exists in
...
Let $f : X \to S$ be a morphism of schemes.
Let $s : S \to X$ be a section of $f$ (in a formula $f \circ s = \id_S$).
