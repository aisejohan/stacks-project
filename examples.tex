...
that $i \circ \psi = \id$ and such that $D \circ i = 0$
...
\xymatrix{
C_1
\ar@<1ex>[rr]^{\id} \ar@<-1ex>[rr]_{\varphi}
& &
Y \ar[r]^\pi & X
}
$$
...
$m \circ (m \times \id_G) = m \circ (\text{id}_G \times m)$
...
that $m \circ (m \times \id_G) = m \circ (\text{id}_G \times m)$.
...
inside the scheme $T \times S$. Consider the group scheme $\SL_2$ which
comes with a surjective smooth affine morphism $\SL_2 \to \mathbf{P}^1$.
Next, consider $U = (\SL_2)^\infty$ with its canonical (product) morphism
...
fibre product of the schemes $Z_i = T \times_{f_i, \mathbf{P}^1} \SL_2$
...
system, namely
$$
\Phi_0 = (K, J_i = (0), (V_i) \xrightarrow{\id} (V_i))
$$
