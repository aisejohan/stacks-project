...
$f \circ s$ is $2$-isomorphic to $\id_\mathcal{T}$. Then
\begin{enumerate}
...
that $x$ lives in with $f(\alpha) = \id$. The composition
$$
c :
\mathcal{I}_{\mathcal{X}/\mathcal{Y}}
...
functor $x \mapsto (x, \id_x)$. We omit the proof that the
...
and $\alpha'$ is an automorphism of $x'$ with $\pi'(\alpha') = \id$, see
Categories, Section \ref{categories-section-inertia}.
...
$\pi'(\alpha') = \id$ signifies that $\gamma = \text{id}$
whereupon the condition that $\alpha, \beta, \gamma$ are compatible
is exactly the condition $\pi(\alpha) = \id$, i.e., means
...
This exactly means that $\Delta(x_1) = (x_1, x_1, \id)$
...
such that $\beta = \id_f \star \alpha$.
\end{proof}
...
scheme $(U, R, s, t, c)$. As
$s \circ e = t \circ e = \id_{\Spec(k)}$ we see that $A$
...
such that
$\gamma = (\id_f \star \alpha) \circ (\beta \star \text{id}_j)$,
in other words such that
$$
\xymatrix{
& f \circ a \circ j \ar[rd]^{\id_f \star \alpha} \\
y \circ j \ar[ru]^{\beta \star \id_j} \ar[rr]^\gamma & &
f \circ x
}
$$
...
$\alpha = \alpha' \circ (\theta \star \id_j)$ and
$\beta' = (\id_f \star \theta) \circ \beta$.
\end{definition}
...
$\alpha' = (\id_{y' \circ j}, \alpha)$ and
$\beta' = \id_{y'}$. Details omitted.
\end{proof}
...
then $(f \circ a, \id_f \star \alpha, \beta)$ is an object
...
$(a, \alpha, (\id_g \star \beta) \circ \epsilon)$ is an
object of $\mathcal{C}$.
\end{proof}
...
Note that $f \circ \theta = \id$; we will use this below.
Observe that $e \circ x = (x, \id_x)$ and
$(a, \theta) \circ j = (a \circ j, \theta \star \id_j)$.
Thus we see that $\alpha$ is a $2$-arrow $\alpha : a \circ j \to x$
compatible with $\theta \star \id_j$ and $\text{id}_x$.
Set $y = f \circ x$ and $\beta = \id_{f \circ a}$.
...
\id_f \star \alpha : f \circ a \circ j \to f \circ x
$$
On the other hand, $\id_a$ is an automorphism too, hence
we conclude $\theta = \id_a$ from the assumption on $f$.
Then we can take as dotted arrow for the displayed diagram above
the morphism $a : \Spec(A) \to \mathcal{X}$ with $2$-morphisms
$(a, \id_a) \circ j \to (x, \text{id}_x)$ given by $\alpha$
and $(a, \theta) \to e \circ a$ given by $\id_a$.
...
$(a_1, \alpha_1, \id)$ and $(a_2, \alpha_2, \varphi)$ in
the diagram
$$
\xymatrix{
\Spec(K) \ar[r]_-x \ar[d]_j & \mathcal{X} \ar[d]^f \\
...
$(a_1, \alpha_1, \id) \to (a_2, \alpha_2, \varphi)$ in
the category of dotted arrows.
\end{enumerate}
\end{lemma}
...
(a \circ j, a' \circ j,
(\beta' \star \id_j) \circ (\beta \star \text{id}_j)^{-1})
\xrightarrow{(\alpha, \alpha')} (x, x, \id) = \Delta_f \circ x
$$
...
$\gamma$ of $G_x$. Then $(\gamma, \id) : (x, y', \alpha) \to
