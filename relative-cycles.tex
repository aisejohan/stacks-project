...
\medskip\noindent
Conversely, assume that (2) holds. Choose an envelope $g : S' \to S$
of finite presentation such that $g^*\alpha$ is in the
...
the equivalent conditions (1) and (2) of
...
A {\it relative $r$-cycle on $X/S$} is a family $\alpha$ of $r$-cycles
on fibres of $X/S$ such the equivalent conditions
...
Then any restriction, base change, flat pullback, or proper pushforward
of $\alpha$ is a relative $r$-cycle (see proof for precise statements).
\end{lemma}
...
quasi-separated schemes and $X' = S' \times_S X$, then $g^*\alpha$
is a relative $r$-cycle on $X'/S'$.
\item If $f : X' \to X$ is a flat morphism of finite presentstion
...
Let $0 \in I$ and let $X_0 \to S_0$ be a morphism of schemes of finite
presentation. For $i \geq 0$ set $X_i = S_i \times_{S_0} X_0$ and set
$X = S \times_{S_0} X_0$. Then
$$
z(X/S, r) = \colim_{i \geq 0} z(X_i/S_i, r)
$$
...
separated, then in (1) we may assume the morphism $X_0 \to S_0$ is
separated.
\end{lemma}
...
in the proof of (2) $\Rightarrow$ (1) to obtain $X_i \to S_i$ separated.
\end{proof}

\begin{lemma}
\label{lemma-supported-on-closed}
Let $X \to S$ be a morphism of finite presentation between
quasi-compact and quasi-separated schemes. Let $X' \subset X$
be a closed subscheme of finite presentation over $S$.
...
\begin{proof}
This lemma is clear for our definition of relative $r$-cycles
in the Noetherian case, but it is not immediate for the general case.
Write $S = \lim S_i$ as a directed limit with affine transition
morphisms with $S_i$ of finite type over $\mathbf{Z}$,
see Limits, Proposition \ref{limits-proposition-approximate}.
We can find an $i$ large enough such that there exist
a finite type morphism $X_i \to S_i$ whose base change to $S$
is $X \to S$, and a closed subscheme $X'_i \subset X_i$ whose
base change is $X' \subset X$. By the discussion in the proof
of Lemma \ref{} we find that after increasing $i$ we may
assume there exists a relative $r$-cycle $\alpha_i$ on $X_i/S_i$
whose base change by $S \to S_i$ is $\alpha$.
...
\begin{proof}
Omitted. Hint: the interesting implication is (2) $\Rightarrow$ (1).
For this, let $W \subset X$ be a closed subscheme of relative
dimension $\leq r$ over $S$. We may
write $W$ as a limit of closed subschemes of $X$ which are
of finite presentation over $S$, see Limits, Lemma
\ref{limits-lemma-closed-is-limit-closed-and-finite-presentation}.
Then by Limits, Lemma \ref{} one of these closed subschemes
is of relative dimension $\leq r$ over $S$.
Thus if $\alpha$ satisfies (2), then $\alpha$ is supported
on a closed subscheme $W \subset X$ of finite presentation over $S$
