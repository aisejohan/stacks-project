...
$I$ on $X$. Hence the nullity of $g_j^2\id_{C_s}$ implies the
...
$$
has a right adjoint. If this right adjoint exists, we will denote
...
of of finite free modules (Lemma \ref{lemma-lift-pseudo-coherent}).
...
$\mathcal{O}_{Y'}$-module $H^a(L(f \circ g)^*E)$ is invertible.
\end{enumerate}
Moreover, we have
\begin{enumerate}
\item[(3)] if $f : Y \to X$ factors as $Y \xrightarrow{g} Z \to X$, then
$H^a(Lf^*E) = g^*H^a(Lj^*E)$,
\item[(4)] if $\beta_a(x) \leq 1$ for all $x \in X$, then
...
we have $\beta_a(Lf^*E) = 1$ hence $y$ maps into $U$ by
Lemma \ref{lemma-jump-loci}. Hence $f$ as in (2) factors through $U$.
Thus we may replace $X$ by $U$ and assume that $\beta_a(x) \in \{0, 1\}$
for all $x \in X$. We will show that in this case
there is a closed subscheme $Z \subset X$ characterized
by the equivalence of (4) (a), (b), (c) and that (3) holds.
This will finish the proof because it will a fortiori show
that morphisms as in (2) factor through $Z$.

\medskip\noindent
If $x \in X$ and $\beta_a(x) = 0$, then there is an
open neighbourhood of $x$ where $\beta_a = 0$
...
$H^0(Lg^*E)$ is not an invertible $\mathcal{O}_Y$-module and
$H^0(Lg^*E)$ does not satisfy (4)(c)
since it is annihilated by a nonzero function.
And trivially it is an invertible $\mathcal{O}_Y$-module if
$g^\sharp(f_j) = 0$ for all $j$. Thus we see that over $U$ the
closed subscheme cut out by $f_1, \ldots, f_\beta$ satisfies
(3) and we have the equivalence of (4)(a), (b), and (c).
...
represent $K$ by a complex of $A$-modules $K^\bullet$ and $E$ by a finite
