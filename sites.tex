...
$$
\xymatrix{
\mathcal{F}(U) \ar[r]^{c(\phi)} \ar[d]_{\id}
&
...
\begin{proof}
This is just the map $c(\id_{u(U)})$ introduced above.
\end{proof}
...
into a site where $\{\id_U : U \to U\}$ are the coverings.
...
$\psi_j \times_{\id_U} \psi_{j'} : V_j \times_U V_{j'} \to
...
via $\alpha : J \to I$ and the maps $f'_j = f_j \times \id_{U_{i_0}}$.
...
$\{\id_U\}$ is an object of it. According to the remarks
...
$\{\id_U\}$ is an object of $\mathcal{J}_U$ we get
...
the element $\id_U \in h_U^\#(U)$ to the section $s \in \mathcal{F}(U)$.
...
a single object $\emptyset$ and a single morphism $\id_\emptyset$
...
$\id_{pt}$ and whose only covering is the covering
$\{\id_{pt}\}$. We will simply write $pt$ for this site.
...
mapping to $\id_U|_{U_{ab}}$. Thus after replacing
...
$V_c \to U$ because $s_{\alpha(c)}$ maps to $\id_U|_{U_{\alpha(c)}}$.)
...
to $\varphi_{a \circ b, \id, u_b(V_j)}$. Given $a : j \to i$, the
family of maps $\varphi_{a, \id, V_j}$ corresponds to a map
of sheaves $\varphi_a : f_a^{-1}\mathcal{F} \to f_{j, *}\mathcal{H}$.
The compatibilities between the $\varphi_{a, \id, u_a(V_i)}$
and the $\varphi_{\id, \text{id}, V_i}$ implies that $\varphi_a$
...
\begin{proof}
This is just the projection map $c(\id_{u(U)})$ above.
\end{proof}
...
$\id : u(U) \to u(U)$. Then we claim $(u(U), \eta)$
...
$\id : v(V) \to v(V)$. Then we claim $(v(V), \xi)$
...
value on $(X, u(\id_X))$, i.e., $\mathcal{F}(X)$.
...
value on $(X, u(\id_X))$, i.e., $\mathcal{F}(X)$. Thus for every
...
We have
$f_* \circ g_* = g^{-1} \circ g_* \cong \id$, and
$g^{-1} \circ f^{-1} = g^{-1} \circ g_! \cong \id$
...
$\id$ in $\Mor(\mathcal{G}_V, \mathcal{G}_V)$.
...
has an object $O_\varphi = (\varphi : V \to U, \id : V \to V)$ for every
...
value on $\id_X$, i.e., $\mathcal{F}(X/U)$.
...
$$
s_{(U', \psi)}|_{U'_i} = c_i^*(s_{(U, \id)}).
$$
Since $\mathcal{F}$ is a sheaf it follows that indeed $s_{(U', \psi)}$
is determined by $s_{(U, \id)}$. This proves uniqueness.
...
becomes $\id_{\mathcal{F}|_{\mathcal{C}/U}}$.
Therefore $(j_{\mathcal{F}, *}\mathcal{G}/\mathcal{F})(U)$
is isomorphic to the fiber of
$\id_{\mathcal{F}|_{\mathcal{C}/U}}$ under the map
$$
\Mor_{\Sh(\mathcal{C}/U)}(\mathcal{F}|_{\mathcal{C}/U},
\mathcal{G}|_{\mathcal{C}/U})
...
the element $(U, u(f)(y), \id_U)$. If $\phi$ exists as above,
then the triples $(V, y, f)$, $(V', y', f')$ determine the same triple
$(U, u(f)(y), \id_U) = (U, u(f')(y'), \text{id}_U)$.
This proves that the map
$u(U) \to (h_U)_p$, $x \mapsto \text{class of }(U, x, \id_U)$
is bijective.
\end{proof}
...
$(U, x) \mapsto (u(U), x, U, \id : u(U) \to u(U))$ and
because there is always a morphism
$$
(V, x, U, \phi : V \to u(U))
\longrightarrow
(u(U), v(\phi)(x), U, \id : u(U) \to u(U))
$$
...
Note that $j_{u(U)} \circ j_x = \id_\mathcal{S}$, and
$v_x \circ u' = v$.
...
to the section $\id_U$ of $h_U^\#$. This section corresponds to a map
...
\medskip\noindent
Assume (1). Then $j_U^{-1} j_{U, *} = \id$ by
...
$\lambda_{r + r'} =
+ \circ \lambda_r \times \lambda_{r'} \circ (\id, \text{id})$,
$\lambda_{rr'} = \lambda_r \circ \lambda_{r'}$,
$\lambda_1 = \id$, $\lambda_0 = 0 \circ (\mathcal{F} \to *)$.
\end{proof}
...
$\id_U \in h_U(U)$. Hence it gives rise to
sections of $Lh_U$ and $h_U^\# = LLh_U$ over $U$ which
we will also denote $\id_U$.
...
on $V$ such that $\varphi(\id_V)$ corresponds to
