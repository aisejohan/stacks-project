...
over $\id_V$. Dropping the $h$ from the notation we see that this
...
$\id : (\Sch/S)_{fppf} \to (\Sch/S)_{fppf}$
...
$$
such that $\pi_F \circ i_F = \id$. Moreover $\pi_{F, *} = i_F^{-1}$.
\end{lemma}
...
the object $(U, \id)$ corresponds to a quasi-coherent sheaf
$\mathcal{F}_{(U, \id)}$ on $U$. Next, the isomorphism
$\alpha : t_{small}^*\mathcal{F}_{(U, \id)} \to
s_{small}^*\mathcal{F}_{(U, \id)}$
comes from
\begin{enumerate}
\item the morphism $(R, \id_R) : (R, s) \to (R, t)$
in the category $[U/_{\!p}R]$ which produces an isomorphism
$\mathcal{F}_{(R, t)} \to \mathcal{F}_{(R, s)}$,
\item the special morphism $(R, s) \to (U, \id)$ which produces an
isomorphism
$s_{small}^*\mathcal{F}_{(U, \id)} \to \mathcal{F}_{(R, s)}$, and
\item the special morphism $(R, t) \to (U, \id)$ which produces an
isomorphism $t_{small}^*\mathcal{F}_{(U, \id)} \to \mathcal{F}_{(R, t)}$.
\end{enumerate}
...
map is just pullback along $f$ as $e_{small}^*\alpha = \id$ by
...
of $\id_x$ and $i$. Recall that the $\mathit{Isom}_\mathcal{X}(x)$
...
$G \to U$, then we see that $\id_x$ and $i$ define morphisms
...
the maps $\id_x$ and $i$ in $\mathcal{X}$.
This proves (2).
...
such that $f \circ s$ is $2$-isomorphic to $\id_\mathcal{X}$
...
that $f' \circ s' = \id_\mathcal{X}$ on the nose. Namely, if
$t : s \circ f \to \id_\mathcal{X}$ is a $2$-isomorphism
then we can set $s'(x) = (s(x), x, t_x)$. Thus we may assume that
$f \circ s = \id_\mathcal{X}$.

\medskip\noindent
In the case that $f \circ s = \id_\mathcal{X}$ the result follows
