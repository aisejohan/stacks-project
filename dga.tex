...
such that $\pi \circ s = \id_L$ and such that
$s \circ \pi$ is homotopic to $\id_{\tilde L}$.
\end{enumerate}
\end{lemma}
...
cone of $\id_K$ which is acyclic. Hence we see that
...
$[n] \circ [m] = [n + m]$ and $[0] = \id_\mathcal{D}$ (equality as
...
satisfying $[n] \circ [m] = [n + m]$ and $[0] = \id_{\mathcal{D}^{gr}}$
...
satisfying $[n] \circ [m] = [n + m]$ and $[0] = \id_\mathcal{A}$
...
$[0] = \id_\mathcal{A}$ and $[n + m] = [n] \circ [m]$
...
$\id_P \in \Hom_{K(A, \text{d})}(P, P)$ is a nonzero element
but $P$ is acyclic.
\end{remark}
...
maps the summand $F_iP$ via the difference of $\id$ and the inclusion
map into $F_{i - 1}P$).
...
$\pi \circ s = \id_P + \text{d}h + h\text{d}$ for some $h : P \to P$
...
$\pi \circ s = \id_P$. This means we obtain a direct
